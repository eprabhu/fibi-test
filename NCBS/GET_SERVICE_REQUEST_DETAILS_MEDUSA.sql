DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_SERVICE_REQUEST_DETAILS_MEDUSA`(
...
AND T1.MODULE_CODE = AV_MODULE_CODE AND
	(CASE WHEN AV_MODULE_CODE = 1 THEN
	(T1.MODULE_ITEM_KEY IN (select AWARD_ID FROM award WHERE AWARD_SEQUENCE_STATUS IN ("ACTIVE","ARCHIVE") AND AWARD_NUMBER = AV_MODULE_ITEM_KEY))
    WHEN AV_MODULE_CODE = 2 THEN
    (T1.MODULE_ITEM_KEY IN (select PROPOSAL_ID FROM proposal WHERE PROPOSAL_NUMBER = AV_MODULE_ITEM_KEY AND PROPOSAL_SEQUENCE_STATUS IN ("ACTIVE","ARCHIVE")))
	ELSE (T1.MODULE_ITEM_KEY = AV_MODULE_ITEM_KEY)
    END)
