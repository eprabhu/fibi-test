DELIMITER $$
CREATE DEFINER=`root`@`localhost` FUNCTION `FN_GET_SAP_FEED_STATUS`(
AV_AWARD_ID INT,
...
where batch_id in (select  max(BATCH_ID) from sap_award_feed where AWARD_ID = AV_AWARD_ID)
...
THEN
SELECT
    DESCRIPTION into LS_FEED_STATUS
FROM
    SAP_FEED_STATUS
WHERE
    FEED_STATUS_CODE = LS_FEED_STATUS;
END IF;
RETURN LS_FEED_STATUS;
END
$$
DELIMITER ;
