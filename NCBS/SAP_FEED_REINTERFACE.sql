DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SAP_FEED_REINTERFACE`(
AV_AWARD_ID 		INT,
...
DECLARE LI_BUDGET_STATUS_CODE	VARCHAR(3);
	...
	END;
		...
		SELECT AWARD_BUDGET_STATUS_CODE INTO LI_BUDGET_STATUS_CODE
		FROM AWARD_BUDGET_HEADER
		WHERE AWARD_ID = AV_AWARD_ID AND
		VERSION_NUMBER = LI_VERSION_NUMBER;
		IF LI_BUDGET_STATUS_CODE IS NOT NULL AND LI_BUDGET_STATUS_CODE = 11 THEN
