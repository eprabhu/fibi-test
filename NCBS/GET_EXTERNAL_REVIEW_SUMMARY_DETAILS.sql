DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_EXTERNAL_REVIEW_SUMMARY_DETAILS`(
...
IF AV_WIDGET = 'REVIEW_SUMMARY_BY_REVIEWERS' THEN
	...
	SET LS_DYN_SQL = CONCAT('(SELECT ''Pending'' AS PENDING_REVIEWS,
								(SELECT COUNT(DISTINCT T1.EXT_REVIEW_ID) FROM EXT_REVIEW T1
									...
									WHERE T2.EXT_REV_REVIEWER_FLOW_STATUS_CODE = 1 AND T5.EXT_REVIEW_STATUS_CODE NOT IN (1) ',LS_FILTER_CONDITION,' AND T1.EXT_REVIEW_SERVICE_TYPE_CODE=1 ) AS MAIN_PROPOSAL,
								(SELECT COUNT(DISTINCT T1.EXT_REVIEW_ID) FROM EXT_REVIEW T1
									LEFT JOIN EXT_REVIEW_REVIEWERS T2 ON T2.EXT_REVIEW_ID=T1.EXT_REVIEW_ID
									LEFT JOIN EXT_REVIEW_STATUS T5 ON T5.EXT_REVIEW_STATUS_CODE = T1.EXT_REVIEW_STATUS_CODE
									WHERE T2.EXT_REV_REVIEWER_FLOW_STATUS_CODE = 1 AND T5.EXT_REVIEW_STATUS_CODE NOT IN (1) ',LS_FILTER_CONDITION,' AND T1.EXT_REVIEW_SERVICE_TYPE_CODE=2 ) AS PRE_PROPOSAL,
                                 (SELECT COUNT(DISTINCT T1.EXT_REVIEW_ID) FROM EXT_REVIEW T1
									LEFT JOIN EXT_REVIEW_REVIEWERS T2 ON T2.EXT_REVIEW_ID=T1.EXT_REVIEW_ID
									LEFT JOIN EXT_REVIEW_STATUS T5 ON T5.EXT_REVIEW_STATUS_CODE = T1.EXT_REVIEW_STATUS_CODE
									WHERE T2.EXT_REV_REVIEWER_FLOW_STATUS_CODE = 1 AND T5.EXT_REVIEW_STATUS_CODE NOT IN (1) ',LS_FILTER_CONDITION,' AND T1.EXT_REVIEW_SERVICE_TYPE_CODE IN (1,2)) AS TOTAL_COUNT,
									COUNT(T1.EXT_REVIEW_ID) AS COUNT,
									1 AS REVIEWER_STATUS_CODE FROM EXT_REVIEW T1
									LEFT JOIN EXT_REVIEW_REVIEWERS T2 ON T2.EXT_REVIEW_ID=T1.EXT_REVIEW_ID
									LEFT JOIN EXT_REVIEW_STATUS T5 ON T5.EXT_REVIEW_STATUS_CODE = T1.EXT_REVIEW_STATUS_CODE
									WHERE T2.EXT_REV_REVIEWER_FLOW_STATUS_CODE = 1 AND T5.EXT_REVIEW_STATUS_CODE NOT IN (1) ',LS_FILTER_CONDITION,')
							  UNION
...
SET LS_DYN_SQL = CONCAT('SELECT ''1'' AS REVIEWERS_ASSIGNED , COUNT(EXTERNAL_REVIEWER_ID) FROM
					...
					WHERE T2.EXT_REVIEWERS_STATUS_CODE=1 AND EXT_REVIEW_STATUS_CODE NOT IN(4,7) AND EXT_REVIEW_SERVICE_TYPE_CODE=2
...
SET LS_DYN_SQL = CONCAT('SELECT REVIWER AS REVIEWER_ID, FULL_NAME,
							...
							FROM (SELECT T2.EXTERNAL_REVIEWER_ID AS REVIWER , T3.FULL_NAME AS FULL_NAME FROM EXT_REVIEW T1
								...
								WHERE T2.EXT_REVIEWERS_STATUS_CODE=1 AND EXT_REVIEW_STATUS_CODE NOT IN(2,3,4) AND EXT_REVIEW_SERVICE_TYPE_CODE IN (1,2)
