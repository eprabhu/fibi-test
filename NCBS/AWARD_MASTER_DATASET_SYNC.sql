DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `AWARD_MASTER_DATASET_SYNC`()
BEGIN
...
AND A3.AWARD_NUMBER IN (SELECT AWARD_NUMBER FROM AWARD_EXPENSE_HEADER WHERE UPDATE_TIMESTAMP BETWEEN LS_LAST_SYNC_TIMESTAMP AND LS_CURRENT_TIMESTAMP_15)
UNION
(SELECT A3.AWARD_ID,
A3.AWARD_NUMBER,
A3.ACCOUNT_NUMBER,
...
AREA_OF_RESEARCH
        ...
        IFNULL(T34.LATEST_APPROVED_BUDGET,0.00) AS LATEST_APPROVED_BUDGET,
         (CASE WHEN LATEST_APPROVED_BUDGET > 0 OR NOT NULL THEN IFNULL(ROUND((T31.CUMULATIVE_ACTUAL_EXPENSE/T34.LATEST_APPROVED_BUDGET)*100,2),0) ELSE 0 END)
		 AS UTILIZATION_RATE,
        T1.DURATION,
        T9.PERCENTAGE_OF_EFFORT,
        IFNULL(T14.CUMULATIVE_VIREMENT,0.00)   AS CUMULATIVE_VIREMENT,
        T15.DISPLAY_AT_ACAD_PROFILE,
        (CASE WHEN (TIMESTAMPDIFF(MONTH,T1.BEGIN_DATE, DATE_ADD(T1.FINAL_EXPIRATION_DATE, INTERVAL 1 DAY ))) > 0 OR NOT NULL THEN
        ...
        (CASE WHEN TOTAL_DIRECT_COST > 0 OR NOT NULL THEN
...
LEFT OUTER JOIN (SELECT S1.AWARD_ID AS AWARD_ID,
        ...
        (SELECT MAX(S3.AWARD_AMOUNT_INFO_ID) AS AWARD_AMOUNT_INFO_ID,S3.AWARD_NUMBER FROM AWARD_AMOUNT_INFO S3
