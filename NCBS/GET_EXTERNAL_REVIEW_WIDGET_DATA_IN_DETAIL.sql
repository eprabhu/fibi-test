DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_EXTERNAL_REVIEW_WIDGET_DATA_IN_DETAIL`(
...
IF AV_WIDGET IN ('PENDING_REVIEW_SUMMARY_BY_REVIEWERS','INPROGRESS_REVIEW_SUMMARY_BY_REVIEWERS','COMPLETED_REVIEW_SUMMARY_BY_REVIEWERS') THEN
	...
	SET LS_DYN_SQL = CONCAT('SELECT DISTINCT * FROM (SELECT T1.EXT_REVIEW_ID,
                                ...
                                ',LS_JOIN_CONDITION,'WHERE T6.EXT_REVIEW_STATUS_CODE NOT IN (1) GROUP BY T1.EXT_REVIEW_ID) T ',LS_EXT_REVIEWER_CONDITION,' ',AV_SORT_TYPE);
	SET @QUERY_STATEMENT = LS_DYN_SQL;
	PREPARE EXECUTABLE_STAEMENT FROM @QUERY_STATEMENT;
	EXECUTE EXECUTABLE_STAEMENT;
ELSEIF AV_WIDGET IN ('REVIEW_DRAFT', 'REVIEW_PENDING','REVIEW_INPROGRESS','COMPLETED') THEN
...
END IF;
			SET LS_DYN_SQL = CONCAT('SELECT DISTINCT * FROM (SELECT T1.EXT_REVIEW_ID,
									...
									WHERE T2.EXT_REVIEWERS_STATUS_CODE=1 AND T1.EXT_REVIEW_STATUS_CODE NOT IN(4,7) AND T1.EXT_REVIEW_SERVICE_TYPE_CODE=2
...
ELSEIF AV_WIDGET = 'ASSIGNMENT_LIMIT_WIDGET' THEN
	SET LS_FILTER_CONDITION = CONCAT(LS_FILTER_CONDITION,' AND T1.EXT_REVIEW_STATUS_CODE NOT IN(2,3,4,7)');
	IF AV_EXT_REVIEWER_ID <> '' THEN
