DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_WORKDAY_MANPOWER_RETRIGGER_DETAILS`(
...
SELECT T1.WORKDAY_MANPOWER_INTERFACE_ID, T1.AWARD_NUMBER, T4.TITLE, T1.INTERFACE_TYPE_CODE, T3.DESCRIPTION AS INTERFACE_TYPE, T1.INTERFACE_STATUS_CODE, T5.DESCRIPTION AS INTERFACE_STATUS, T1.INTERFACE_TIMESTAMP, T2.MESSAGE, T2.MESSAGE_TYPE, T1.MANPOWER_USER_ACTION_CODE
...
WHERE T1.INTERFACE_TYPE_CODE IN (6, 7, 1, 14) AND T4.AWARD_SEQUENCE_STATUS IN ("PENDING", "ACTIVE") GROUP BY T1.AWARD_NUMBER
