DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `PR_MASTER_DATASET_FULL_SYNC`()
BEGIN
...
BEGIN
	...
	SET AUTOCOMMIT = 0;
	SET LS_ERR_FLG = 'N';
    TRUNCATE TABLE PROG_RPT_HDR_RT;
    TRUNCATE TABLE PROG_RPT_KPI_RT;
    TRUNCATE TABLE PROG_RPT_MILESTONES_RT;
	INSERT INTO PROG_RPT_HDR_RT(
	  ...
	  LAST_SYNC_TIME
			...
			t0.CREATE_TIMESTAMP AS REPORT_CREATED_TIMESTAMP,
		...
		FROM
			...
			LEFT JOIN (SELECT DISTINCT
				w.MODULE_ITEM_ID AS MODULE_ITEM_ID,
					...
					(CASE
						WHEN (wd.APPROVAL_STATUS = 'B') THEN wd.APPROVAL_COMMENT
						ELSE NULL
					...
					(CASE
						WHEN (wd.APPROVAL_STATUS = 'B') THEN NULL
						ELSE IFNULL( (SELECT pr.DESCRIPTION FROM person_role_type pr WHERE wd.ROLE_TYPE_CODE = pr.ROLE_TYPE_CODE limit 1), wd.STOP_NAME)
					END) AS ROLE_TYPE
			FROM
				workflow w
		...
		WHERE ks.progress_report_id IN ( SELECT PROGRESS_REPORT_ID FROM PROG_RPT_HDR_RT)
	...
	WHERE  ip.progress_report_id IN ( SELECT PROGRESS_REPORT_ID FROM PROG_RPT_HDR_RT)
	...
	FROM    progress_report_kpi_collaboration_projects cp,
			award_progress_report_kpi_summary ks
	WHERE   cp.progress_report_id IN ( SELECT PROGRESS_REPORT_ID FROM PROG_RPT_HDR_RT)
	...
	FROM    progress_report_kpi_technology_disclosure td,
			award_progress_report_kpi_summary ks
	WHERE   td.progress_report_id IN ( SELECT PROGRESS_REPORT_ID FROM PROG_RPT_HDR_RT)
	...
	FROM    progress_report_kpi_manpower_development md,
			award_progress_report_kpi_summary ks
	WHERE md.progress_report_id IN ( SELECT PROGRESS_REPORT_ID FROM PROG_RPT_HDR_RT)
	...
	FROM    progress_report_kpi_undergraduate_student ugs,
			award_progress_report_kpi_summary ks
	WHERE ugs.progress_report_id IN ( SELECT PROGRESS_REPORT_ID FROM PROG_RPT_HDR_RT)
	...
	FROM    progress_report_kpi_conference_presentation cpt,
			award_progress_report_kpi_summary ks
	WHERE   cpt.progress_report_id IN ( SELECT PROGRESS_REPORT_ID FROM PROG_RPT_HDR_RT)
	...
	FROM    progress_report_kpi_competitive_grants cg,
			award_progress_report_kpi_summary ks
	WHERE cg.progress_report_id IN ( SELECT PROGRESS_REPORT_ID FROM PROG_RPT_HDR_RT)
	...
	FROM    progress_report_kpi_cash_funding cf,
			award_progress_report_kpi_summary ks
	WHERE cf.progress_report_id IN ( SELECT PROGRESS_REPORT_ID FROM PROG_RPT_HDR_RT)
	...
	FROM    progress_report_kpi_inkind_contributions ikc,
			award_progress_report_kpi_summary ks
	WHERE ikc.progress_report_id IN ( SELECT PROGRESS_REPORT_ID FROM PROG_RPT_HDR_RT)
	...
	FROM    progress_report_kpi_patents p,
			award_progress_report_kpi_summary ks
	WHERE p.progress_report_id IN ( SELECT PROGRESS_REPORT_ID FROM PROG_RPT_HDR_RT)
	...
	FROM    progress_report_kpi_licenses l,
			award_progress_report_kpi_summary ks
	WHERE  l.progress_report_id IN ( SELECT PROGRESS_REPORT_ID FROM PROG_RPT_HDR_RT)
	...
	FROM    progress_report_kpi_successful_startups ss,
			award_progress_report_kpi_summary ks
	WHERE ss.progress_report_id IN ( SELECT PROGRESS_REPORT_ID FROM PROG_RPT_HDR_RT)
	...
	FROM    progress_report_kpi_health_specific_outcomes hso,
			award_progress_report_kpi_summary ks
	WHERE hso.progress_report_id IN ( SELECT PROGRESS_REPORT_ID FROM PROG_RPT_HDR_RT)
	...
	FROM    progress_report_kpi_post_docs_employed pde,
			award_progress_report_kpi_summary ks
	WHERE  pde.progress_report_id IN ( SELECT PROGRESS_REPORT_ID FROM PROG_RPT_HDR_RT)
	...
	(
		...
		WHERE pgm.progress_report_id IN ( SELECT PROGRESS_REPORT_ID FROM PROG_RPT_HDR_RT)
	);
	...
	(SELECT PROGRESS_REPORT_ID FROM AWARD_PROGRESS_REPORT WHERE AWARD_ID NOT IN (SELECT AWARD_ID FROM AWARD));
	UPDATE REPORT_LAST_SYNC_TIME SET PR_LAST_SYNC_TIMESTAMP = UTC_TIMESTAMP();
