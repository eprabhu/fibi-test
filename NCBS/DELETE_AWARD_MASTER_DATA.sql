DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `DELETE_AWARD_MASTER_DATA`(AV_ID INT)
BEGIN
DECLARE LI_OLD_AWARD_ID 		DECIMAL(22,0);
...
DECLARE CONTINUE HANDLER FOR SQLEXCEPTION
				BEGIN
					...
					VALUES(LI_AWARD_ID,LS_AWARD_NUMBER,0,LS_ERROR_MSG,'DELETE_AWARD_MASTER_DATA',NULL,NULL,null,UTC_TIMESTAMP(),'quickstart');
					COMMIT;
				END;
				...
				BEGIN
					...
					SELECT AWARD_ID,AWARD_NUMBER,SEQUENCE_NUMBER,AWARD_ID
					FROM AWARD W;
					...
					END IF;
						...
						WHERE AWARD_ID = LI_AWARD_ID;
						DELETE FROM AWARD_DMP_DATASET_RT WHERE AWARD_ID IN (LI_AWARD_ID);
						DELETE FROM AWARD_AMOUNT_INFO WHERE AWARD_ID IN (LI_AWARD_ID);
						 DELETE FROM AWARD_PROGRESS_REPORT_MILESTONE  WHERE AWARD_ID IN (LI_AWARD_ID);
						DELETE FROM AWARD_AMOUNT_TRANSACTION
						WHERE TRANSACTION_ID  IN
							 (SELECT TRANSACTION_ID FROM AWARD_AMOUNT_INFO WHERE AWARD_ID IN (LI_AWARD_ID));
						DELETE FROM AWARD_AMT_FNA_DISTRIBUTION WHERE AWARD_ID (LI_AWARD_ID);
						DELETE FROM  AWARD_KEYPERSON_TIMESHEET WHERE AWARD_ID IN (LI_AWARD_ID);
						DELETE FROM AWARD_LATEST_APROVED_BUDGET_RT WHERE AWARD_ID IN (LI_AWARD_ID);
						DELETE FROM AWARD_MASTER_DATASET_RT WHERE AWARD_ID IN (LI_AWARD_ID);
						DELETE FROM AWARD_ORG_APPROVED_BUDGET_RT WHERE AWARD_ID IN (LI_AWARD_ID);
 						DELETE FROM AWARD_PROGRESS_REPORT_KPI_SUMMARY  WHERE AWARD_ID IN (LI_AWARD_ID);
						DELETE FROM AWARD_SUP_ORG_MAPPING WHERE AWARD_ID IN (LI_AWARD_ID);
                        DELETE FROM   AWARD_PROGRESS_REPORT_ATTACHMENT
                        WHERE PROGRESS_REPORT_ID IN
                       (SELECT PROGRESS_REPORT_ID FROM AWARD_PROGRESS_REPORT  WHERE AWARD_ID IN (LI_AWARD_ID));
                        DELETE FROM  AWARD_PROGRESS_REPORT_ACHIEVEMENT WHERE PROGRESS_REPORT_ID IN
                       (SELECT PROGRESS_REPORT_ID FROM AWARD_PROGRESS_REPORT  WHERE AWARD_ID IN (LI_AWARD_ID));
                        DELETE FROM  AWARD_PROGRESS_REPORT WHERE AWARD_ID IN (LI_AWARD_ID);
						DELETE FROM  AWARD_REPORT_RECPNT_NOTIFY_LOG WHERE AWARD_REPORT_TERMS_ID IN
                        (SELECT AWARD_REPORT_TERMS_ID FROM AWARD_REPORT_TERMS WHERE AWARD_ID IN (LI_AWARD_ID));
						DELETE
						...
						FROM AWARD_KPI
						WHERE AWARD_ID = LI_AWARD_ID;
						DELETE FROM AWARD_BUDGET_DET_CAL_AMT WHERE BUDGET_HEADER_ID IN
