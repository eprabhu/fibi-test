DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SAP_FEED_SOFT_LAUNCH_NO_REPORT`(
AV_BATCH_ID	INT
)
BEGIN
	...
	DECLARE LS_LEAD_UNIT_NAME			VARCHAR(100);
	...
	DECLARE LI_FLAG						INT;
		...
		BEGIN
			...
			VALUES(LI_SEQ_ERROR_LOG_ID,LS_ERROR_MSG,utc_timestamp(),'quickstart');
			COMMIT;
		END;
		BEGIN
			...
			AND T1.MODULE_ITEM_KEY IN (SELECT MAX(AWARD_ID) FROM AWARD
