DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_NEGOTIATION_DASHBOARD`(
...
END IF;
SET LS_DYN_SQL = CONCAT('SELECT DISTINCT * FROM (SELECT T1.NEGOTIATION_ID,
								T1.TITLE,
								...
								LEFT JOIN SPONSOR T21 ON T21.SPONSOR_CODE = T11.SPONSOR_CODE ',TAB_QUERY,' GROUP BY T1.NEGOTIATION_ID )T  ',LS_FILTER_CONDITION,' ',AV_SORT_TYPE,' ',LS_OFFSET_CONDITION );
