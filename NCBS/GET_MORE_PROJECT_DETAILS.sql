DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_MORE_PROJECT_DETAILS`(
...
SELECT
    ...
    ELSEIF AV_MODULE_CODE = 2 THEN
            ...
            WHERE T1.PROPOSAL_ID IN (
										...
										and s1.SEQUENCE_NUMBER in (select max(s2.SEQUENCE_NUMBER) from PROPOSAL s2 where s1.PROPOSAL_NUMBER = s2.PROPOSAL_NUMBER)
									);
