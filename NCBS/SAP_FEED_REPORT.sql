DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SAP_FEED_REPORT`(
AV_BATCH_ID	INT
)
BEGIN
...
DECLARE LI_COUNT INT;
	DECLARE CONTINUE HANDLER FOR SQLEXCEPTION
		BEGIN
			...
			VALUES(CONCAT(LS_ERROR_MSG,LI_FEED_ID),utc_timestamp(),'quickstart');
			COMMIT;
		END;
		BEGIN
			...
			WHERE FEED_ID = LI_FEED_ID AND BATCH_ID = AV_BATCH_ID;
			IF LI_FLAG > 0 THEN
				SELECT PROJECT_DEFINITION,SHORT_DESCRIPTION
				INTO LS_PROJECT_DEFINITION,LS_SHORT_DESCRIPTION
				FROM SAP_FEED_TMPL_PROJECT_DEF
				WHERE FEED_ID = LI_FEED_ID AND BATCH_ID = AV_BATCH_ID;
				SELECT COUNT(1) INTO LI_FLAG
				FROM SAP_FEED_REPORT
