DELIMITER $$
CREATE DEFINER=`ncbsfibi`@`%` PROCEDURE `CUSTOM_DATA_SYNC`()
BEGIN
...
INSERT INTO TEMP_AWARD_MASTER_DATASET (AWARD_ID)
(SELECT
A3.AWARD_ID
FROM AWARD A3
...
END IF;
        ...
        ( SELECT T0.AWARD_NUMBER, T2.INTERNAL_ORDER_CODE, T2.LINE_ITEM_COST
				...
				INNER JOIN AWARD_BUDGET_DETAIL T2 ON T1.BUDGET_HEADER_ID = T2.BUDGET_HEADER_ID
				WHERE T0.DOCUMENT_UPDATE_TIMESTAMP BETWEEN LS_LAST_SYNC_TIMESTAMP AND LS_CURRENT_TIMESTAMP_15
