DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_AGREEMENT_DASHBOARD_COUNT`(
...
SELECT TRIM(TRAILING 'AND ' FROM LS_FILTER_CONDITION) INTO LS_FILTER_CONDITION FROM DUAL;
  ELSE
	...
	SET ls_dyn_sql = CONCAT('WITH ACCESS_TMP
							...
							');
    SET ls_dyn_sql =CONCAT(ls_dyn_sql,'SELECT count(*) FROM
                                    (
									SELECT
                                        distinct
                                   ...
                                   ',LS_ADMIN_GROUP_CONDITION,LS_TAB_QUERY,LS_FILTER_CONDITION_NEW,' ) T ', LS_FILTER_CONDITION,'',LS_SORT_TYPE);
