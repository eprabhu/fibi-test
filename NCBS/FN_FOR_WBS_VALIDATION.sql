DELIMITER $$
CREATE DEFINER=`root`@`localhost` FUNCTION `FN_FOR_WBS_VALIDATION`(AV_AWARD_ID  VARCHAR(22)
...
AND COLUMN_ID  in (
					SELECT COLUMN_ID FROM CUSTOM_DATA_ELEMENTS WHERE CUSTOM_ELEMENT_NAME = 'Input GST Category'
				   )
