DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `VALIDATE_CREATE_TASK`(
...
BEGIN
	...
	IF AV_TASK_TYPE IN (2, 5) AND AV_TASK_ID = '' THEN
		...
		SELECT * FROM (
            ...
            UNION
			SELECT "W", CONCAT(MANPOWER_TASK_DESCRIPTION, " task needs to be completed before ", CONFIRM_PROJECT_TASK_DESCRIPTION, " task is complete else Position ID won't be generated") WHERE
