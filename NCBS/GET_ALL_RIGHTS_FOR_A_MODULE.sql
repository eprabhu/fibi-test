DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_ALL_RIGHTS_FOR_A_MODULE`( AV_MODULE_CODE INT(4), AV_PERSON_ID VARCHAR(40), AV_LEAD_UNIT VARCHAR(8), AV_PROJECT_ID DECIMAL(22) )
BEGIN
...
THEN
	...
	FROM
		AWARD
	WHERE
		AWARD_ID = AV_PROJECT_ID;
SELECT
	G1.HOME_UNIT_NUMBER INTO GC_LEAD_UNIT
FROM
	AWARD AWD1
	INNER JOIN
		GRANT_CALL_HEADER G1
		ON AWD1.GRANT_HEADER_ID = G1.GRANT_HEADER_ID
WHERE
	AWD1.AWARD_ID = AV_PROJECT_ID;
IF LI_FLAG > 0
THEN
	SELECT
		AWARD_ID INTO LI_ACTIVE_PROJECT_ID
	FROM
		AWARD
	WHERE
		AWARD_NUMBER IN
...
IF;
SELECT
	COUNT(S1.MODULE_ITEM_ID) into LI_COUNT
FROM
	WORKFLOW S1
	INNER JOIN
		WORKFLOW_DETAIL S2
		ON S1.WORKFLOW_ID = S2.WORKFLOW_ID
WHERE
	S1.IS_WORKFLOW_ACTIVE = 'Y'
...
THEN
	...
	FROM
		...
		INNER JOIN
			AWARD T2
			ON T1.GRANT_HEADER_ID = T2.GRANT_HEADER_ID
	WHERE
		T2.AWARD_ID = AV_PROJECT_ID
...
THEN
	...
	FROM
		...
		JOIN
			ROLE_RIGHTS RR
			ON PR.ROLE_ID = RR.ROLE_ID
		JOIN
			RIGHTS RT
			ON RR.RIGHT_ID = RT.RIGHT_ID
		LEFT JOIN
			UNIT_WITH_CHILDREN UC
			ON UC.UNIT_NUMBER = PR.UNIT_NUMBER
			AND PR.DESCEND_FLAG = 'Y'
	WHERE
		PR.PERSON_ID = AV_PERSON_ID
		...
		(
(PR.DESCEND_FLAG = 'Y'
			AND AV_LEAD_UNIT = UC.CHILD_UNIT_NUMBER)
			OR
			(
				PR.DESCEND_FLAG = 'N'
				AND PR.UNIT_NUMBER = AV_LEAD_UNIT
			)
		)
	UNION
	SELECT DISTINCT
	...
	WHERE
		T1.AWARD_ID = LI_ACTIVE_PROJECT_ID
	...
	FROM
		...
		INNER JOIN
			AWARD T4
			ON T4.AWARD_ID = T1.AWARD_ID
	WHERE
		T1.AWARD_ID = AV_PROJECT_ID
	...
	FROM
		(
			SELECT
				'VIEW_EOM_EXPENDITURE_TO_DATE' AS RIGHT_NAME
			FROM
				dual
			UNION
			SELECT
				'VIEW_AWARD_PURCHASE_DETAILS' AS RIGHT_NAME
			FROM
				dual
			UNION
			SELECT
				'VIEW_AWARD' AS RIGHT_NAME
			FROM
				dual
			UNION
			SELECT
				'VIEW_EXPENSE_TRANSACTION' AS RIGHT_NAME
			FROM
				dual
			UNION
			SELECT
				'VIEW_AWARD_PURCHASE_TRANSACTION' AS RIGHT_NAME
			FROM
				dual
			UNION
			SELECT
				'VIEW_AWARD_REVENUE_DETAILS' AS RIGHT_NAME
			FROM
				DUAL
		)
		t
...
( PR. DESCEND_FLAG = 'Y'
			...
			(
				PR. DESCEND_FLAG = 'N'
				AND PR.UNIT_NUMBER = GC_LEAD_UNIT
			)
		)
		AND RLE.ROLE_ID = 100
	...
	WHERE
		...
		(
			SELECT DISTINCT
				PERSON_ID
			FROM
				GRANT_CALL_FUNDING_SCHEME_MANAGERS
			WHERE
				FUNDING_SCHEME_CODE IN
				(
					SELECT
						TS3.FUNDING_SCHEME_CODE
					FROM
						GRANT_CALL_HEADER TS1
						INNER JOIN
							AWARD TS2
							ON TS1.GRANT_HEADER_ID = TS2.GRANT_HEADER_ID
						INNER JOIN
							SPONSOR_FUNDING_SCHEME TS3
							ON TS3.FUNDING_SCHEME_ID = TS1.FUNDING_SCHEME_ID
					WHERE
						TS2.AWARD_ID = AV_PROJECT_ID
				)
...
FROM
	...
	INNER JOIN
		GRANT_CALL_HEADER G1
		ON EPS1.GRANT_HEADER_ID = G1.GRANT_HEADER_ID
WHERE
	EPS1.PROPOSAL_ID = AV_PROJECT_ID;
IF LI_FLAG > 0
THEN
	SELECT
		COUNT(1) INTO LI_FLAG
	...
	where
		...
		(
			...
			FROM
				sr_header
			where
				ASSIGNEE_PERSON_ID = AV_PERSON_ID
				AND SR_HEADER_ID = AV_PROJECT_ID
		)
;
ELSEIF AV_MODULE_CODE = 14
THEN
	SELECT
		COUNT(1) INTO LI_FLAG
	FROM
			...
			(
				...
				FROM
					...
					INNER JOIN
						...
						ON TS3.FUNDING_SCHEME_ID = TS1.FUNDING_SCHEME_ID
				WHERE
					TS2.AWARD_ID = LI_ACTIVE_PROJECT_ID
			)
			AND PERSON_ID = AV_PERSON_ID
	)
	AND RLE.ROLE_ID = PR.ROLE_ID;
ELSEIF AV_MODULE_CODE = 16
THEN
	SELECT
		COUNT(1) INTO LI_FLAG
	FROM
	...
	WHERE
		PROGRESS_REPORT_ID = AV_PROJECT_ID;
SELECT
	G1.HOME_UNIT_NUMBER INTO GC_LEAD_UNIT
FROM
	AWARD AWD1
