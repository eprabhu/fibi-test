DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `COPY_MERGE_EPS_PROPOSAL_COMMENTS_DATA`(
...
BEGIN
	...
	DECLARE LS_IS_PUBLIC	varchar(255);
	...
	UPDATE_USER, IS_PRIVATE, IS_PUBLIC FROM EPS_PROPOSAL_COMMENTS WHERE PROPOSAL_ID IN (li_seq_proposal_id);
	 ...
	 SET LS_ERROR_FLAG = 'N';
	   ...
	   END IF;
  		 SET LS_PROC_LOC = 'EPS_PROPOSAL_COMMENTS';
			...
			SET NOT_FOUND  = 0;
            FETCH SEL_EPS_PROPOSAL_COMMENTS INTO  LI_LS_EPS_PROPOSAL_COMMENT_ID, LS_COMMENT_TYPE_CODE,LS_COMMENTS,LD_UPDATE_TIMESTAMP,LS_UPDATE_USER,LS_IS_PRIVATE,LS_IS_PUBLIC;
              IF NOT_FOUND=1 THEN
