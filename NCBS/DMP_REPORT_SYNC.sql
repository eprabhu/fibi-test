...
BEGIN
DECLARE LS_CURRENT_TIMESTAMP_15 DATETIME;
...
SELECT DATE_SUB(now(), INTERVAL 15 MINUTE) INTO LS_CURRENT_TIMESTAMP_15 FROM REPORT_LAST_SYNC_TIME;
...
WHERE A3.DOCUMENT_UPDATE_TIMESTAMP BETWEEN LS_LAST_SYNC_TIMESTAMP AND LS_CURRENT_TIMESTAMP_15
AND ((A3.AWARD_SEQUENCE_STATUS = 'PENDING' AND A3.SEQUENCE_NUMBER = 1)
OR (A3.AWARD_SEQUENCE_STATUS = 'ACTIVE')  AND EXISTS (SELECT 1
			   FROM AWARD
			   WHERE AWARD_NUMBER = A3.AWARD_NUMBER
				 ...
				 AND DOCUMENT_UPDATE_TIMESTAMP BETWEEN LS_LAST_SYNC_TIMESTAMP AND LS_CURRENT_TIMESTAMP_15));
COMMIT;
...
SELECT GROUP_CONCAT(A3.AWARD_ID) INTO AWARD_IDS FROM AWARD A3
                                                        ...
                                                        OR
                                                        A3.AWARD_SEQUENCE_STATUS IN('ACTIVE') AND EXISTS (SELECT 1
													   FROM AWARD
													   WHERE AWARD_NUMBER = A3.AWARD_NUMBER
													     AND AWARD_SEQUENCE_STATUS = 'ARCHIVE'
														 AND AWARD_VARIATION_TYPE_CODE IN ( SELECT B.TYPE_CODE
