DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_AGMENT_DASHBOARD_CATEGORY`(
...
(select max(AGREEMENT_SPONSOR_ID) FROM AGREEMENT_SPONSOR WHERE AGREEMENT_REQUEST_ID = T1.AGREEMENT_REQUEST_ID AND AGREEMENT_SPONSOR_TYPE_CODE = 1)) AS ORGANIZATION';
		SET LS_FILTER_CONDITION1 = CONCAT(LS_FILTER_CONDITION1,' WHERE T1.AGREEMENT_STATUS_CODE IN (1,6) AND
													T1.AGREEMENT_SEQUENCE_STATUS <> ''TEMPLATE'' AND (T1.UNIT_NUMBER IN(SELECT DISTINCT UNIT_NUMBER FROM ACCESS_TMP_ADMIN)OR (T1.REQUESTOR_PERSON_ID =''',AV_PERSON_ID,'''
		...
		SET LS_FILTER_CONDITION3 = CONCAT(LS_FILTER_CONDITION3,' WHERE T1.AGREEMENT_STATUS_CODE IN (1,6) AND
													T1.AGREEMENT_SEQUENCE_STATUS <> ''TEMPLATE'' AND ((T1.UNIT_NUMBER IN(SELECT DISTINCT UNIT_NUMBER FROM ACCESS_TMP)OR (T1.REQUESTOR_PERSON_ID =''',AV_PERSON_ID,''')
