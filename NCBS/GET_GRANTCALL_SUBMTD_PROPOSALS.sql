DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_GRANTCALL_SUBMTD_PROPOSALS`(
        ...
        ELSE
				...
				FROM(
						SELECT
							...
							(SUM(T4.SCORE)/(select count(*) from WORKFLOW s1
					inner join workflow_detail s2 on s1.workflow_id= s2.workflow_id
                    inner join WORKFLOW_REVIEWER_SCORE s3  on s2.WORKFLOW_DETAIL_ID = s3.WORKFLOW_DETAIL_ID
