DELIMITER $$
CREATE DEFINER=`ncbsfibi`@`%` PROCEDURE `GET_INST_PROPOSAL_DASHBOARD_COUNT`(
...
SET LS_DYN_SQL =CONCAT(LS_DYN_SQL,'SELECT DISTINCT count(*) FROM(SELECT DISTINCT  T1.PROPOSAL_ID,
                                            ...
                                            LEFT JOIN SPONSOR T31 ON T31.SPONSOR_CODE = T1.SPONSOR_CODE ',JOIN_CONDITION,' WHERE T1.PROPOSAL_SEQUENCE_STATUS IN(''ACTIVE'') AND ',TAB_QUERY,
