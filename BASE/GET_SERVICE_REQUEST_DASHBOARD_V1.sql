...
BEGIN
 ...
 SET LS_CUSTOM_DATA_JOIN_SEL = TRIM(TRAILING ',' FROM LS_CUSTOM_DATA_JOIN_SEL);
    SET @DYNAMIC_CTE = CONCAT('WITH ACCESS_TMP AS (
							...
							ASSOC_DETAILS AS (
								SELECT DISTINCT
								CAST(SR_HEADER_ID AS UNSIGNED) AS SR_HEADER_ID,
...
SET @DYNAMIC_CTE = CONCAT(@DYNAMIC_CTE, COALESCE(FN_PREPARE_DASHBOARD_JOINS(@AV_DISPLAY_LIST,  @AV_FILTER_JSON, @CTE_KEY_MAP, @CTE_MAP, 'CTE'),''));
  ...
  SELECT FN_PREPARE_DASHBOARD_JOINS(@AV_DISPLAY_LIST, @AV_FILTER_JSON, @JOIN_KEY_MAP, @JOIN_QUERY_MAP,'JOIN') INTO @JOIN_QUERY;
    ...
    SET @ASSOC_START_DATE_CRT='( ASSOC.SR_HEADER_ID IS NOT NULL AND (
                            (JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_ASSOC_START_DATE.fromDate'')) IS NULL AND ASSOC.START_DATE <= JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_ASSOC_START_DATE.toDate'')))
                            OR (JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_ASSOC_START_DATE.toDate'')) IS NULL AND ASSOC.START_DATE >= JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_ASSOC_START_DATE.fromDate'')))
                            OR (ASSOC.START_DATE BETWEEN JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_ASSOC_START_DATE.fromDate'')) AND JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_ASSOC_START_DATE.toDate''))))
                            )';
    SET @ASSOC_END_DATE_CRT='( ASSOC.SR_HEADER_ID IS NOT NULL AND (
                            (JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_ASSOC_END_DATE.fromDate'')) IS NULL AND ASSOC.END_DATE <= JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_ASSOC_END_DATE.toDate'')))
                            OR (JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_ASSOC_END_DATE.toDate'')) IS NULL AND ASSOC.END_DATE >= JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_ASSOC_END_DATE.fromDate'')))
                            OR (ASSOC.END_DATE BETWEEN JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_ASSOC_END_DATE.fromDate'')) AND JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_ASSOC_END_DATE.toDate''))))
                            )';
    SET @SR_CREATE_DATE_CRT='( ASSOC.SR_HEADER_ID IS NOT NULL AND (
	...
	SET @SR_KEY_PERSON_CRT = '(
                                ( JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_SR_KEY_PERSONS.isEmp'')) = ''true'' AND EXISTS (SELECT 1 FROM SR_PERSONS SRP LEFT JOIN JSON_TABLE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_SR_KEY_PERSONS.values''), ''$[*]'' COLUMNS(res LONGTEXT PATH ''$'')) jt ON JT.res = SRP.PERSON_ID WHERE SRP.SR_HEADER_ID = T1.SR_HEADER_ID AND JT.res IS NOT NULL  ) )
                                 OR
                                ( JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_SR_KEY_PERSONS.isEmp'')) = ''false'' AND EXISTS (SELECT 1 FROM SR_PERSONS SRP LEFT JOIN JSON_TABLE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_SR_KEY_PERSONS.values''), ''$[*]'' COLUMNS(res LONGTEXT PATH ''$'')) jt ON JT.res = SRP.ROLODEX_ID WHERE SR_HEADER_ID = T1.SR_HEADER_ID AND JT.res IS NOT NULL ) )
                            )';
	SET @CRT_SR_PI_CRT = '(
                                ( JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_SR_PI.isEmp'')) = ''true'' AND EXISTS (SELECT 1 FROM SR_PERSONS SRP LEFT JOIN JSON_TABLE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_SR_PI.values''), ''$[*]'' COLUMNS(res LONGTEXT PATH ''$'')) jt ON JT.res = SRP.PERSON_ID AND  SRP.PERSON_ROLE_ID = 3 WHERE SRP.SR_HEADER_ID = T1.SR_HEADER_ID AND JT.res IS NOT NULL ) )
                                 OR
                                ( JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_SR_PI.isEmp'')) = ''false'' AND EXISTS (SELECT 1 FROM SR_PERSONS SRP LEFT JOIN JSON_TABLE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_SR_PI.values''), ''$[*]'' COLUMNS(res LONGTEXT PATH ''$'')) jt ON JT.res = SRP.ROLODEX_ID AND SRP.PERSON_ROLE_ID = 3 WHERE SR_HEADER_ID = T1.SR_HEADER_ID  AND JT.res IS NOT NULL ) )
                            )';
	SET @CRT_LINKED_RECORD_PI_CRT = '( ASSOC.SR_HEADER_ID IS NOT NULL AND (
                                ( JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_LINKED_RECORD_PI.isEmp'')) = ''true'' AND EXISTS (SELECT 1 FROM JSON_TABLE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_LINKED_RECORD_PI.values''), ''$[*]'' COLUMNS(res LONGTEXT PATH ''$''))jt WHERE JT.res = ASSOC.PI_PERSON_ID ) )
                                 OR
                                ( JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_LINKED_RECORD_PI.isEmp'')) = ''false'' AND EXISTS (SELECT 1 FROM JSON_TABLE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_LINKED_RECORD_PI.values''), ''$[*]'' COLUMNS(res LONGTEXT PATH ''$'')) jt WHERE JT.res= ASSOC.PI_ROLODEX_PERSON_ID)))
                            )';
        SET @CRITERIA_MAP = JSON_OBJECT(
            ...
            'CRT_SR_TYPE', CONCAT('(JSON_CONTAINS(JSON_EXTRACT(@AV_FILTER_JSON,''$.CRT_SR_TYPE''), JSON_QUOTE(CAST(T1.TYPE_CODE AS CHAR))))'),
            'CRT_SR_DEPARTMENT', CONCAT('(JSON_CONTAINS(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_SR_DEPARTMENT''), JSON_QUOTE(T1.UNIT_NUMBER)))'),
            'CRT_SR_PRIORITY', CONCAT('(JSON_CONTAINS(JSON_EXTRACT(@AV_FILTER_JSON,''$.CRT_SR_PRIORITY''), JSON_QUOTE(CAST(T1.PRIORITY_ID AS CHAR))))'),
            'CRT_SR_STATUS', CONCAT('(JSON_CONTAINS(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_SR_STATUS''), JSON_QUOTE(CAST(T1.STATUS_CODE AS CHAR))))'),
			'CRT_SR_TASK_TYPE', CONCAT('(JSON_CONTAINS(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_SR_TASK_TYPE''), JSON_QUOTE(CAST(ST.task_type_code AS CHAR))))'),
			'CRT_SR_TASK_STATUS', CONCAT('(JSON_CONTAINS(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_SR_TASK_STATUS''), JSON_QUOTE(CAST(ST.task_status_code AS CHAR))))'),
            'CRT_CATEGORY', CONCAT('(JSON_CONTAINS(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_CATEGORY''), JSON_QUOTE(CAST(T1.CATEGORY_CODE AS CHAR))))'),
            'CRT_SR_PI', @CRT_SR_PI_CRT,
            'CRT_SR_KEY_PERSONS', @SR_KEY_PERSON_CRT,
            'CRT_ASSOC_SR_SPONSOR', '(EXISTS (SELECT 1 FROM sr_assoc_detail_v WHERE SR_HEADER_ID= T1.SR_HEADER_ID AND JSON_CONTAINS(JSON_EXTRACT(@AV_FILTER_JSON,''$.CRT_ASSOC_SR_SPONSOR''), JSON_QUOTE(SPONSOR_CODE))))',
            'CRT_ASSOC_START_DATE', @ASSOC_START_DATE_CRT,
            'CRT_ASSOC_END_DATE', @ASSOC_END_DATE_CRT,
            'CRT_ASSOC_SR_LINKED_ID', ' (ASSOC.MODULE_ITEM_KEY LIKE CONCAT("%",JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON,''$.CRT_ASSOC_SR_LINKED_ID'')),"%"))',
            'CRT_LINKED_RECORD_PI', @CRT_LINKED_RECORD_PI_CRT,
            'CRT_SR_CREATE_DATE', @SR_CREATE_DATE_CRT,
            'CRT_SR_ADMIN_GROUP', CONCAT('(JSON_CONTAINS(JSON_EXTRACT(@AV_FILTER_JSON,''$.CRT_SR_ADMIN_GROUP''), JSON_QUOTE(CAST(T1.ADMIN_GROUP_ID AS CHAR))))'),
            'CRT_SR_ASSI_PERSON', CONCAT('(JSON_CONTAINS(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_SR_ASSI_PERSON''), JSON_QUOTE(T1.ASSIGNEE_PERSON_ID)))'),
            'CRT_SR_REPORTER', CONCAT('(JSON_CONTAINS(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_SR_REPORTER''), JSON_QUOTE(T1.REPORTER_PERSON_ID)))'),
            'CRT_SR_ASSOC_LINKED_MODULE', '(JSON_CONTAINS(JSON_EXTRACT(@AV_FILTER_JSON,''$.CRT_SR_ASSOC_LINKED_MODULE''), JSON_QUOTE(CAST(ASSOC.MODULE_CODE AS CHAR))))'
        );
...
END IF;
    SET LS_DYN_SQL = CONCAT(@DYNAMIC_CTE,
        CASE WHEN AV_IS_COUNT = TRUE
            THEN ' SELECT TOTAL_COUNT FROM (SELECT COUNT(DISTINCT T1.SR_HEADER_ID) AS TOTAL_COUNT'
		...
		LEFT JOIN ASSOC_DETAILS ASSOC ON ASSOC.SR_HEADER_ID= T1.SR_HEADER_ID
		LEFT JOIN SR_TEMP WF ON WF.SR_HEADER_ID = T1.SR_HEADER_ID
        ',IFNULL(@JOIN_QUERY,''),'
