...
DECLARE LS_CURRENT_DATE 				VARCHAR(10);
...
DECLARE LI_MODULE_CODE					INT DEFAULT 1;
...
END IF;
 SET @LS_CURRENT_DATE = DATE_FORMAT(CURDATE(), '%m/%d/%Y');
IF LD_FINAL_EXPIRATION_DATE_JSON IS NULL AND LD_BEGIN_DATE_JSON IS NULL THEN
	SET LD_FINAL_EXPIRATION_DATE_JSON =   STR_TO_DATE(@LS_CURRENT_DATE, '%m/%d/%Y');
	SET LD_BEGIN_DATE_JSON = STR_TO_DATE(@LS_CURRENT_DATE, '%m/%d/%Y');
...
IF LD_OBLIGATED_START_DATE IS NULL AND LD_OBLIGATED_END_DATE IS NULL THEN
	SET LD_OBLIGATED_START_DATE = STR_TO_DATE(@LS_CURRENT_DATE, '%m/%d/%Y');
	SET LD_OBLIGATED_END_DATE = STR_TO_DATE(@LS_CURRENT_DATE, '%m/%d/%Y');
...
IF LS_PERSON_ID IS NULL  THEN
	SET LS_PERSON_ID = '10000000001';
    SET LS_UNIT_NUMBER = (SELECT VALUE FROM PARAMETER WHERE PARAMETER_NAME = 'ROOT_UNIT');
...
SELECT CONCAT(DATE_FORMAT(NOW(), '%y%m%d'), LPAD(DATE_FORMAT(NOW(), '%H%i%s%f') DIV 1000, 9, '0')) INTO LD_TRANSACTION_ID;
	...
	END IF;
        SELECT LI_AWARD_ID AS MODULE_ITEM_ID, LS_AWARD_NUMBER AS MODULE_ITEM_KEY, LI_MODULE_CODE AS MODULE_CODE, LS_RECIPIENT_ID, LS_ERROR_FLAG, LS_ERROR_MSG;
END;
END
$$
DELIMITER ;
