...
SET SQL_SAFE_UPDATES= 0 ;
				   ...
				   DELETE FROM FILE_DATA WHERE ID IN (
					SELECT FILE_DATA_ID FROM SR_TASK_ATTACHMENT
					WHERE SR_TASK_ID IN (SELECT TASK_ID FROM SR_TASK WHERE SR_HEADER_ID = AV_SR_HEADER_ID));
				
                   DELETE FROM SR_TASK_ATTACHMENT WHERE SR_TASK_ID IN (SELECT TASK_ID FROM SR_TASK WHERE SR_HEADER_ID = AV_SR_HEADER_ID);
                   
				   DELETE FROM SR_TASK_HISTORY WHERE TASK_ID IN (SELECT TASK_ID FROM SR_TASK WHERE SR_HEADER_ID = AV_SR_HEADER_ID);
				   
                   DELETE FROM SR_TASK WHERE SR_HEADER_ID = AV_SR_HEADER_ID;
				   
					DELETE FROM SR_HEADER WHERE SR_HEADER_ID = AV_SR_HEADER_ID;
					
					DELETE FROM INBOX WHERE MODULE_CODE = 20 AND MODULE_ITEM_KEY = AV_SR_HEADER_ID;
