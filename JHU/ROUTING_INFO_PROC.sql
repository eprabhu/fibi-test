DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `ROUTING_INFO_PROC`()
BEGIN
...
DECLARE LS_CREATION_STATUS_CODE VARCHAR(50);
...
BEGIN
    ...
    DECLARE proposal_rec cursor for
        (SELECT DISTINCT P.PROPOSAL_ID "PROPOSAL_NUMBER",
				SUBSTRING(DATE_FORMAT((prop.CREATE_TIMESTAMP + INTERVAL 6 MONTH), '%Y'),3,2) FY,
				(CASE WHEN DATE_FORMAT(prop.CREATE_TIMESTAMP, '%m')= '01' THEN '07'
					  WHEN DATE_FORMAT(prop.CREATE_TIMESTAMP, '%m')= '02' THEN '08'
					  WHEN DATE_FORMAT(prop.CREATE_TIMESTAMP, '%m')= '03' THEN '09'
					  WHEN DATE_FORMAT(prop.CREATE_TIMESTAMP, '%m')= '04' THEN '10'
					  WHEN DATE_FORMAT(prop.CREATE_TIMESTAMP, '%m')= '05' THEN '11'
					  WHEN DATE_FORMAT(prop.CREATE_TIMESTAMP, '%m')= '06' THEN '12'
					  WHEN DATE_FORMAT(prop.CREATE_TIMESTAMP, '%m')= '07' THEN '01'
					  WHEN DATE_FORMAT(prop.CREATE_TIMESTAMP, '%m')= '08' THEN '02'
					  WHEN DATE_FORMAT(prop.CREATE_TIMESTAMP, '%m')= '09' THEN '03'
					  WHEN DATE_FORMAT(prop.CREATE_TIMESTAMP, '%m')= '10' THEN '04'
					  WHEN DATE_FORMAT(prop.CREATE_TIMESTAMP, '%m')= '11' THEN '05'
					  WHEN DATE_FORMAT(prop.CREATE_TIMESTAMP, '%m')= '12' THEN '06' END) FM,
				prop.PROPOSAL_NUMBER IPN,
				...
				(CASE WHEN P.STATUS_CODE= 1 THEN 'In Progress'
					  ...
					  WHEN P.STATUS_CODE= 11 THEN 'Submitted'
					   END ) "CREATION_STATUS_CODE" ,
		...
		FROM
				...
				LEFT OUTER JOIN proposal prop
							ON 				prop.PROPOSAL_ID = D.INST_PROPOSAL_ID
	...
	GET DIAGNOSTICS CONDITION 1 @sqlstate = RETURNED_SQLSTATE,
		...
		SELECT @full_error,@errno,LI_PROPOSAL_ID ;
	END;
