DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_RESEARCH_SUMMARY_DETAILS`(
...
ELSEIF AV_WIDGET = 'AWARD_BY_SPONSOR' THEN
	...
	AND A1.BEGIN_DATE BETWEEN CONCAT(COL3-1,LS_FY_START_DATE) AND CONCAT(COL3,LS_FY_END_DATE)
...
ELSEIF AV_WIDGET = 'PROPOSAL_BY_SPONSOR_TYPE' THEN
 ...
 AND T1.PROPOSAL_ID IN (SELECT PROPOSAL_ID FROM EPS_PROPOSAL
	...
	AND S1.MODULE_CODE = 3) GROUP BY T2.SPONSOR_TYPE_CODE, T3.DESCRIPTION ',LS_ORDER_CONDITION,'');
	SET @QUERY_STATEMENT = LS_DYN_SQL;
