DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_AGRMENT_DASHBOARD_DYN_JSON`(
JSON_FORMAT JSON
)
BEGIN
...
SET LS_DYN_SQL = CONCAT(LS_DYN_SQL,'
									...
									LEFT JOIN SPONSORS SPO ON T1.AGREEMENT_REQUEST_ID = SPO.AGREEMENT_REQUEST_ID ', JOIN_CONDITION,LS_CUSTOM_DATA_JOIN, LS_ADMIN_GROUP_CONDITION,LS_TAB_QUERY,LS_FILTER_CONDITION_NEW,LS_FILTER_CONDITION, LS_SORT_TYPE, LS_OFFSET_CONDITION,') T ');
