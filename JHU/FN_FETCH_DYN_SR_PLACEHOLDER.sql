DELIMITER $$
CREATE DEFINER=`root`@`localhost` FUNCTION `FN_FETCH_DYN_SR_PLACEHOLDER`(AV_MODULE_CODE INT, AV_PROJECT_ID INT, AV_SR_HEADER_ID INT) RETURNS json
    DETERMINISTIC
BEGIN
...
DECLARE LS_PI_NAME VARCHAR(90) DEFAULT '';
    ...
    END IF;
    SELECT COALESCE(FULL_NAME, '') INTO LS_PI_NAME FROM SR_PERSONS WHERE SR_HEADER_ID = AV_SR_HEADER_ID AND PERSON_ROLE_ID = 3 AND ACTION_FLAG != 'IMPORTED_DELETED';
    RETURN  JSON_OBJECT(
