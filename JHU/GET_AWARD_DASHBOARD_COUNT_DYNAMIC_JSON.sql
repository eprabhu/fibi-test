DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_AWARD_DASHBOARD_COUNT_DYNAMIC_JSON`(
		JSON_FORMAT JSON
)
BEGIN
...
DECLARE AV_ROLE_ID		        VARCHAR(200);
...
SET TAB_QUERY = CONCAT(' WHERE T1.AWARD_SEQUENCE_STATUS NOT IN (''ARCHIVE'', ''CANCELLED'')' );
...
ELSEIF AV_TAB_TYPE = 'DRAFT_AWARDS' THEN
        SET TAB_QUERY =  CONCAT('  WHERE  ((T1.CREATE_USER = (SELECT USER_NAME FROM PERSON WHERE PERSON_ID = ''',AV_PERSON_ID,''')
                OR (T1.AWARD_DOCUMENT_TYPE_CODE = 1) OR (T2.PERSON_ID = ''',AV_PERSON_ID,'''))
