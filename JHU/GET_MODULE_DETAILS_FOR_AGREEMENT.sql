DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_MODULE_DETAILS_FOR_AGREEMENT`(
...
BEGIN
   ...
   ELSEIF AV_MODULE_CODE = '2' THEN
    ...
    WHERE
		...
		IFNULL(t6.FULL_NAME, t7.FULL_NAME) AS PI_NAME,
	    "" AS SPONSOR_NAME,
        t1.SR_HEADER_ID AS MODULE_ITEM_KEY,
		20 AS MODULE_CODE,
        t1.SR_HEADER_ID AS MODULE_ITEM_ID,
        t2.PERSON_ID AS PERSON_ID,
        "" AS SPONSOR_CODE,
		"" AS START_DATE,
		"" AS END_DATE,
        ...
        "" AS SPONSOR_AWARD_NUMBER,
		IFNULL(t6.EMAIL_ADDRESS, t7.EMAIL_ADDRESS) AS EMAIL_ADDRESS,
        t10.DISPLAY_NAME  AS DEPARTMENT,
		t2.DESIGNATION AS DESIGNATION
    FROM
        (((((((((sr_header t1
        LEFT JOIN sr_persons t2 ON (((t1.SR_HEADER_ID = t2.SR_HEADER_ID)
            AND (t2.PERSON_ROLE_ID = 3) AND (t2.PI_FLAG ='Y'))))
		LEFT JOIN sr_person_units t9 ON ((t9.SR_PERSON_ID = t2.SR_PERSON_ID)))
        LEFT JOIN UNIT t10 ON ((t10.UNIT_NUMBER = t9.UNIT_NUMBER)))
        LEFT JOIN sr_status t3 ON ((t1.STATUS_CODE = t3.STATUS_CODE)))
        LEFT JOIN unit t5 ON ((t1.UNIT_NUMBER = t5.UNIT_NUMBER)))
        LEFT JOIN PERSON t6 ON ((t2.PERSON_ID = t6.PERSON_ID)))
		LEFT JOIN ROLODEX t7 ON ((t2.ROLODEX_ID = t7.ROLODEX_ID)))))
    WHERE
        (t1.SR_HEADER_ID = AV_MODULE_ITEM_KEY);
    ...
    FROM
        ...
        LEFT JOIN ROLODEX t7 ON ((t2.ROLODEX_ID = t7.ROLODEX_ID)))
            ...
            as1.SPONSOR_CODE AS SPONSOR_CODE,
                as2.DISPLAY_NAME AS SPONSOR_NAME,
