DELIMITER $$
CREATE DEFINER=`jhufibi`@`%` PROCEDURE `MIG_AWARD_PERSONS_TO_SR`(
...
BEGIN
    ...
    DECLARE LS_AW_UNIT_NUMBER                          		VARCHAR(8);
 ...
 SET LS_ERROR_FLAG = 'N';
	...
	IF (LS_PERSON_ID is not null ) then
            ...
            THEN
				...
				(LI_INSERT_ID, utc_timestamp(6), AV_UPDATE_USER, AV_SR_HEADER_ID,(SELECT GROUP_CONCAT(DERIVED_ROLE_NAME SEPARATOR ', ')  FROM module_derived_roles WHERE AUTO_GRANT_TO_PI = 'Y' AND MODULE_CODE = 20),LS_FULL_NAME);
