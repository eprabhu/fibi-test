DELIMITER $$
CREATE DEFINER=`jhufibi`@`%` PROCEDURE `jhu_update_sap_spon_prog_person`(IN proc_sponsored_program_number VARCHAR(20)
...
BEGIN
	...
	GET DIAGNOSTICS CONDITION 1 @sqlstate = RETURNED_SQLSTATE,
        ...
        -- SET ls_file_text = concat('Invalid Dataset \n', IFNULL(ls_message_text,'\n'));
			...
			VALUES
			('SAP_SPONSORED_PROGRAM_PERSON',proc_sponsored_program_number,'SQLEXCEPTION',LS_ERROR,NULL,'N',null,null,NULL,NULL,'N',now(),'JHU_ADMIN');
	END;
      BEGIN
        DELETE FROM sap_sponsored_program_person
        ...
        THEN
          ...
          AND person_id = proc_person_id;
            IF row_count = 0
          THEN
            INSERT INTO sap_sponsored_program_person (sponsored_program_number, person_id, update_timestamp)
            VALUES (proc_sponsored_program_number, proc_person_id, proc_update_timestamp);
          END IF;
			IF proc_update_column IS NOT NULL AND proc_update_column = 'responsibility_code' THEN
				UPDATE sap_sponsored_program_person
				SET RESPONSIBILITY_CODE = proc_new_value
				WHERE sponsored_program_number = proc_sponsored_program_number
				AND person_id = proc_person_id;
			END IF;
        END IF;
