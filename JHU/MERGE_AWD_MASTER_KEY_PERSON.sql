DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `MERGE_AWD_MASTER_KEY_PERSON`(
AV_AWARD_ID 			INT,
...
DECLARE LI_AWARD_PERSON_UNIT_ID_NEXT_VAL	INT(10);
DECLARE LI_AWARD_PERSON_ATTACH_NEXT_VAL		INT(10);
...
DECLARE LI_AWARD_PERSON_TS_ID_NEXT_VAL	INT(10);
DECLARE LI_AWARD_KYPERSON_TS_CNT			INT(10);
...
DECLARE LS_DOCUMENT_UPDATE_TIMESTAMP	DATETIME;
	BEGIN
			...
			BEGIN
				...
				VALUES(AV_AWARD_ID,AV_AWARD_NUMBER,AV_SEQUENCE_NUMBER,LS_ERROR_MSG,'MERGE_AWD_MASTER_KEY_PERSON',LS_TABLE_NAME,104,AV_VARIATION_TYPE_CODE,UTC_TIMESTAMP(),AV_UPDATE_USER);
				COMMIT;
			END;
			...
			SELECT COUNT(1) INTO LI_AWARD_KYPERSON_TS_CNT
			...
			SELECT IFNULL(MAX(NEXT_VAL),0) INTO LI_AWARD_PERSON_TS_ID_NEXT_VAL
			FROM KEYPERSON_TIMESHEET_ID_GEN;
			SELECT IFNULL(MAX(NEXT_VAL),0) INTO LI_AWARD_PERSON_UNIT_ID_NEXT_VAL
			FROM AWARD_PERSON_UNIT_ID_GENERATOR;
			SELECT IFNULL(MAX(NEXT_VAL),0) INTO LI_AWARD_PERSON_ATTACH_NEXT_VAL
			FROM AWARD_PERSON_ATTACH_ID_GENERATOR;
			SELECT IFNULL(MAX(NEXT_VAL),0) INTO LI_AWARD_PERSON_ID_NEXT_VAL
			FROM AWARD_PERSON_ID_GENERATOR;
			UPDATE AWARD_PERSON_UNIT_ID_GENERATOR
			SET NEXT_VAL = LI_AWARD_PERSON_UNIT_ID_NEXT_VAL + LI_AWARD_PERSON_UNIT_CNT + 1;
			UPDATE KEYPERSON_TIMESHEET_ID_GEN
			SET NEXT_VAL = LI_AWARD_PERSON_TS_ID_NEXT_VAL + LI_AWARD_KYPERSON_TS_CNT + 1;
			UPDATE AWARD_PERSON_ATTACH_ID_GENERATOR
			SET NEXT_VAL = LI_AWARD_PERSON_ATTACH_NEXT_VAL + LI_AWARD_PERSON_ATTACHMNT_CNT + 1;
			UPDATE AWARD_PERSON_ID_GENERATOR
			SET NEXT_VAL = LI_AWARD_PERSON_ID_NEXT_VAL + LI_AWARD_PERSONS_CNT + 1;
			...
			BEGIN
				...
				EMAIL_ADDRESS, ROLODEX_ID, UNIT_NAME, PERSON_ROLE_ID, PI_FLAG, IS_MULTI_PI,
				...
				END IF;
				SET LI_AWARD_PERSON_ID_NEXT_VAL = LI_AWARD_PERSON_ID_NEXT_VAL + 1;
				...
				BEGIN
					...
					END IF;
					SET LI_AWARD_PERSON_UNIT_ID_NEXT_VAL = LI_AWARD_PERSON_UNIT_ID_NEXT_VAL + 1;
					...
					VALUES
					(
					LI_AWARD_PERSON_UNIT_ID_NEXT_VAL,
					LI_AWARD_PERSON_ID_NEXT_VAL,
					LI_MASTER_AWARD_ID,
					AV_AWARD_NUMBER,
					LI_MASTER_SEQ_NUMBER,
				...
				BEGIN
					...
					END IF;
					SET LI_AWARD_PERSON_ATTACH_NEXT_VAL = LI_AWARD_PERSON_ATTACH_NEXT_VAL + 1;
					INSERT INTO AWARD_PERSON_ATTACHMNT
					(ATTACHMENT_ID, DESCRIPTION, FILE_NAME, MIME_TYPE, FILE_DATA_ID, UPDATE_TIMESTAMP, UPDATE_USER, AWARD_PERSON_ID)
					VALUES(
					LI_AWARD_PERSON_ATTACH_NEXT_VAL,
					LS_DESCRIPTION,
					LS_FILE_NAME,
					LS_MIME_TYPE,
					LS_FILE_DATA_ID,
				...
				BEGIN
					...
					DECLARE CUR_AWARD_KEYPERSON_TIMESHEET CURSOR FOR
					SELECT
					YEAR,
					VALUE,
					TIMESHEET_TYPE,
					...
					OPEN CUR_AWARD_KEYPERSON_TIMESHEET;
					INSERT_AWARD_KEYPERSON_TIMESHEET_LOOP: LOOP
					FETCH CUR_AWARD_KEYPERSON_TIMESHEET INTO
					LS_YEAR,
					LS_VALUE,
					LS_TIMESHEET_TYPE,
					LS_ORDER_NUMBER,
					...
					LS_UPDATE_TIMESTAMP;
					IF DONE4 THEN
						LEAVE INSERT_AWARD_KEYPERSON_TIMESHEET_LOOP;
					END IF;
					SET LI_AWARD_PERSON_TS_ID_NEXT_VAL = LI_AWARD_PERSON_TS_ID_NEXT_VAL + 1;
					INSERT INTO AWARD_KEYPERSON_TIMESHEET(
					KEYPERSON_TIMESHEET_ID,
					AWARD_PERSON_ID,
					AWARD_ID,
					AWARD_NUMBER,
					YEAR,
					...
					LI_AWARD_PERSON_TS_ID_NEXT_VAL,
					LI_AWARD_PERSON_ID_NEXT_VAL,
					LI_MASTER_AWARD_ID,
					AV_AWARD_NUMBER,
					LS_YEAR,
					...
					END LOOP;
					CLOSE CUR_AWARD_KEYPERSON_TIMESHEET;
				END;
				END LOOP;
