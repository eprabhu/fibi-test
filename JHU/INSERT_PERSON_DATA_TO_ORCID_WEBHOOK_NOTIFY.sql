DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `INSERT_PERSON_DATA_TO_ORCID_WEBHOOK_NOTIFY`()
BEGIN
...
END IF;
		    ...
		    VALUES(LI_SEQ_ORCID_WEBHOOK_NOTIFY_LOG_ID, LI_ORCID_ID,NOW(), 'quickstart', 'C');
			ELSEIF LI_STATUS = 'I' AND LI_IS_WEBHOOK_ACTIVE ='Y' THEN
			INSERT INTO ORCID_WEBHOOK_NOTIFY_LOG(ORCID_WEBHOOK_NOTIFY_LOG_ID,ORCID_ID, UPDATE_TIMESTAMP, UPDATE_USER, WEBHOOK_ACTION_TYPE_CODE)
		    VALUES(LI_SEQ_ORCID_WEBHOOK_NOTIFY_LOG_ID, LI_ORCID_ID,NOW(), 'quickstart', 'D');
            END IF;
		    COMMIT;
END;
END LOOP;
