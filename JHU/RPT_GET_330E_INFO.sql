DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `RPT_GET_330E_INFO`(IN report_start_date  VARCHAR(50)
...
WHERE
  ...
  FROM   proposal p
         ...
         inner join (select p.FULL_NAME,asr_data.* from asr_data
					left join person p on UPPER(p.user_name)=asr_data.pi_user_name ) asr on asr.award_id=a.award_id   inner join proposal_status ps on ps.status_code = p.status_code
