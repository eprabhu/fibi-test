DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_SAP_CLAIM_DASHBOARD`(
...
IF AV_TAB_TYPE = 'BATCH_DETAIL' THEN
		SET JOIN_CONDITION = CONCAT('SELECT T1.FEED_ID,T1.BATCH_ID,T1.CLAIM_ID,T1.CLAIM_NUMBER,T1.INVOICE_ID,T1.FEED_STATUS_CODE,T1.FEED_TYPE_CODE,T1.USER_ACTION_CODE,T5.USER_ACTION AS USER_ACTION, T6.DESCRIPTION AS FEED_STATUS, T7.DESCRIPTION AS FEED_TYPE
                                            ,T1.UPDATE_TIMESTAMP,T1.UPDATE_USER,T8.FULL_NAME AS UPDATE_USER_NAME,T4.ACCOUNT_NUMBER,T4.AWARD_NUMBER,T1.USER_COMMENT, ((select count(1) from claim_invoice_log WHERE CLAIM_ID = T1.CLAIM_ID AND BATCH_ID = T1.BATCH_ID) =
	(select count(1) from claim_invoice_log WHERE CLAIM_ID = T1.CLAIM_ID AND BATCH_ID = T1.BATCH_ID AND STATUS = ''E'')) AS CAN_REVISION
	...
	ELSEIF AV_TAB_TYPE = 'BATCH_HISTORY' THEN
		SET JOIN_CONDITION = CONCAT('SELECT distinct T1.BATCH_ID, t2.FEED_STATUS_CODE,T1.NO_OF_RECORDS,T1.CREATE_TIMESTAMP,T1.RESPONSE_TIMESTAMP ,(SELECT COUNT(1) FROM SAP_CLAIM_FEED WHERE FEED_STATUS_CODE IN(''E'') AND BATCH_ID = T1.BATCH_ID) AS TOTAL_ERROR_INVOICE
                                   ...
                                   FROM SAP_CLAIM_FEED_BATCH T1
									...
									LEFT JOIN PERSON T8 ON T2.UPDATE_USER = T8.USER_NAME GROUP BY BATCH_ID');
		SET LS_FILTER_CONDITION = CONCAT(LS_FILTER_CONDITION,' T.FEED_STATUS_CODE in (''E'',''R'',''F'',''N'') AND ');
		IF AV_SORT_TYPE IS  NULL THEN
