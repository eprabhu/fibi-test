DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_EXT_REVIEWER_DASHBOARD`(
    ...
    AV_ACADEMIC_AREA_CODE_PRIMARY           VARCHAR(50),
    AV_AFFILATION_INSTITUITION_CODE         VARCHAR(50),
    AV_HINDEX                               VARCHAR(200),
    AV_KEYWORD_CODE                         VARCHAR(100),
...
SET SELECTED_HINDEX_VALUE = '';
    ...
    IF AV_ACADEMIC_AREA_CODE_SECONDARY IS NOT NULL  AND AV_ACADEMIC_AREA_CODE_SECONDARY <> '' THEN
        SET LS_FILTER_CONDITION = CONCAT(LS_FILTER_CONDITION,' T1.ACADEMIC_AREA_CODE_SECONDARY LIKE ''',AV_ACADEMIC_AREA_CODE_SECONDARY,''' AND ');
    END IF;
    IF AV_ACADEMIC_AREA_CODE_PRIMARY IS NOT NULL  AND AV_ACADEMIC_AREA_CODE_PRIMARY <> '' THEN
        SET LS_FILTER_CONDITION = CONCAT(LS_FILTER_CONDITION,' T1.ACADEMIC_AREA_CODE_PRIMARY LIKE ''',AV_ACADEMIC_AREA_CODE_PRIMARY,''' AND ');
    END IF;
    IF AV_AFFILATION_INSTITUITION_CODE IS NOT NULL  AND AV_AFFILATION_INSTITUITION_CODE <> '' THEN
        SET LS_FILTER_CONDITION = CONCAT(LS_FILTER_CONDITION,' T3.AFFILATION_INSTITUITION_CODE = ''',AV_AFFILATION_INSTITUITION_CODE,''' AND ');
    END IF;
    ...
    'SELECT * FROM(SELECT DISTINCT  T1.*, GROUP_CONCAT(T6.DESCRIPTION SEPARATOR ", ") AS SPECIALISM_KEYWORD, T3.DESCRIPTION AS REVIEWER_AFFILIATION ',SELECTED_FIELD_LIST, ' FROM EXTERNAL_REVIEWER T1
    LEFT OUTER JOIN EXTERNAL_REVIEWER_EXT T2 ON T2.EXTERNAL_REVIEWER_ID = T1.EXTERNAL_REVIEWER_ID
    LEFT OUTER JOIN EXT_REVIEWER_AFFILIATION T3 ON T3.AFFILATION_INSTITUITION_CODE = T1.AFFILATION_INSTITUITION_CODE
    ...
    ',LS_FILTER_CONDITION,' GROUP BY T1.EXTERNAL_REVIEWER_ID)T ',AV_SORT_TYPE,' ',LS_OFFSET_CONDITION
);
