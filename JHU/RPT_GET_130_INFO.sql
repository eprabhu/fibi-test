DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `RPT_GET_130_INFO`( IN report_start_date VARCHAR(50)
...
BEGIN
	...
	ELSE
        ...
        FROM     proposal p
				...
				INNER JOIN award a ON a.AWARD_ID = afp.AWARD_ID
												AND    a.sequence_number = (SELECT MAX(sequence_number)
																			  FROM   award
																			  WHERE  award_number = a.award_number)
				INNER JOIN award_persons ai ON ai.AWARD_ID=a.AWARD_ID
				INNER JOIN award_person_unit au ON au.AWARD_PERSON_ID = ai.AWARD_PERSON_ID
				...
				INNER JOIN award_amount_info aai ON  aai.award_number = a.award_number
				...
				INNER JOIN unit u ON u.unit_number = au.unit_number
				INNER JOIN person apr ON  apr.person_id = ai.person_id
				LEFT OUTER JOIN(  SELECT distinct ii.protocol_number
										,ii.title
										,ii.status
										,ii.expiration_date
										,ipr.full_name
