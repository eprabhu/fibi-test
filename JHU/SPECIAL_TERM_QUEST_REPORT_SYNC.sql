DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SPECIAL_TERM_QUEST_REPORT_SYNC`(AV_MODULE_ITEM_KEY INT)
BEGIN
...
DECLARE CUR_SEL CURSOR FOR
	...
	WHERE Q1.QUESTIONNAIRE_ID = QH.QUESTIONNAIRE_ID AND QH.IS_FINAL ='Y' AND QH.QUESTIONNAIRE_NUMBER = 110
	...
	LEFT JOIN(SELECT
				...
				AND T2.QUESTIONNAIRE_ID = (SELECT MAX(S.QUESTIONNAIRE_ID) FROM QUEST_ANSWER_HEADER S
											...
											AND T5.QUESTIONNAIRE_NUMBER = 110
