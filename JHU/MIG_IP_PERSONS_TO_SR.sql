DELIMITER $$
CREATE DEFINER=`jhufibi`@`%` PROCEDURE `MIG_IP_PERSONS_TO_SR`(
...
BEGIN
    ...
    DECLARE LI_IP_PERSON_ID                          		INT;
    DECLARE LI_INSERT_ID                          		 	INT;
    DECLARE LI_COUNT                          		 		INT;
    DECLARE LS_IP_UNIT_NUMBER                          		VARCHAR(8);
    ...
    DECLARE SEL_IP_PERSONS CURSOR FOR
     ...
     FROM proposal_persons where PROPOSAL_ID = AV_MODULE_ITEM_ID;
 ...
 SET LS_ERROR_FLAG = 'N';
 	SELECT ASSIGNEE_PERSON_ID, STATUS_CODE, SUBJECT , TYPE_CODE
 	INTO LS_ASSIGNEE_PERSON_ID, LI_STATUS_CODE, LS_SUBJECT , LS_TYPE_CODE
 	FROM SR_HEADER WHERE SR_HEADER_ID = AV_SR_HEADER_ID;
	IF LS_TYPE_CODE <> '61' THEN
  	...
  	SET LS_PROC_LOC = 'SEL_IP_PERSONS';
			OPEN SEL_IP_PERSONS;
            SEL_IP_PERSONS:LOOP
			SET NOT_FOUND  = 0;
            FETCH SEL_IP_PERSONS
            INTO LI_IP_PERSON_ID, LS_DEPARTMENT, LS_FULL_NAME, LS_PERSON_ID, LI_PERSON_ROLE_ID, LI_ROLODEX_ID, LI_PERCENTAGE_OF_EFFORT, LS_PI_FLAG, LS_DESIGNATION, LS_PROJECT_ROLE;
            IF NOT_FOUND = 1 THEN
            LEAVE SEL_IP_PERSONS;
            END IF;
            Select count(1) into LI_COUNT from sr_person_roles WHERE SR_HEADER_ID = AV_SR_HEADER_ID AND PERSON_ID = LS_PERSON_ID AND IS_SYSTEM_DERIVED_ROLE = 'Y';
			IF (LI_COUNT > 0)
				THEN
		...
		VALUES (LS_DEPARTMENT, LS_FULL_NAME, LS_PERSON_ID, LI_PERSON_ROLE_ID, LI_ROLODEX_ID, AV_SR_HEADER_ID, LI_PERCENTAGE_OF_EFFORT, LS_PI_FLAG, LS_DESIGNATION, 'N' , LS_PROJECT_ROLE, 'Not Applicable','IMPORTED', utc_timestamp(6), AV_UPDATE_USER);
         SET LI_INSERT_ID = last_insert_id();
        IF (select count(*)  from prop_person_units where PROPOSAL_PERSON_ID = LI_IP_PERSON_ID > 0 ) then
		...
		FROM
			prop_person_units
		WHERE
				PROPOSAL_PERSON_ID = LI_IP_PERSON_ID;
       END IF;
	IF (LS_PERSON_ID is not null ) then
	 ...
	 THEN
		   ...
		   END IF;
      END IF;
        END LOOP SEL_IP_PERSONS;
		CLOSE SEL_IP_PERSONS;
	END IF;
END
$$
DELIMITER ;
