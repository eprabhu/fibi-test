DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `RPT_GET_133E_INFO_bkp`(IN report_unit VARCHAR(8)                   )
BEGIN
    SELECT Distinct	  ip.PROPOSAL_NUMBER
			...
			,REPLACE(asr.protocol_number,' ','') PROTOCOL_NUMBER
			,p.FULL_NAME
			,p.user_name JHED_ID
			,s.SPONSOR_NAME
			,s.SPONSOR_CODE
  ...
  SELECT Distinct ip.PROPOSAL_NUMBER
        ...
        ,pm.sponsored_program_number PRIMARY_SPONSORED_PROGRAM
        ,SUBSTR(pu.unit_number, 1, 3) LEAD_UNIT
        ,REPLACE(psr.protocol_number,' ','') PROTOCOL_NUMBER
        ,p.FULL_NAME
        ,p.user_name JHED_ID
        ,s.SPONSOR_NAME
        ,s.SPONSOR_CODE
