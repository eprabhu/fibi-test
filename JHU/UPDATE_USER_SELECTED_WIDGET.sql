DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `UPDATE_USER_SELECTED_WIDGET`(
AV_WIDGET_ID INT)
BEGIN
...
OPEN C1;
	...
	INSERT INTO `USER_SELECTED_WIDGET` (`WIDGET_ID`, `SORT_ORDER`, `PERSON_ID`, `UPDATE_TIMESTAMP`, `UPDATE_USER`) VALUES (AV_WIDGET_ID, LI_SORT_ORDER, LI_PERSON_ID, NOW(), 'quickstart');
    END IF;
END;
END LOOP;
CLOSE C1;
