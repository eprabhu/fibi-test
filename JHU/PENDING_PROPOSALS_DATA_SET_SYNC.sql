DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `PENDING_PROPOSALS_DATA_SET_SYNC`()
BEGIN
...
FROM
    ...
    prop_person_units pu ON pu.proposal_number = pi.proposal_number
     ...
     and p.proposal_ID IN (SELECT
										proposal_ID
										FROM proposal
										WHERE UPDATE_TIMESTAMP>=DATE_SUB(LS_LAST_SYNC_TIMESTAMP,INTERVAL 1 HOUR)
										AND STATUS_CODE =1 and type_code in (1,4,5,6,9) )
	AND pi.pi_flag='Y'
...
END;
   call RPT_MV_AWARDS_DATA_SET_SYNC();
