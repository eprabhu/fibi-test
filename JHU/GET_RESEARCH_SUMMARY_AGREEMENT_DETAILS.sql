DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_RESEARCH_SUMMARY_AGREEMENT_DETAILS`(
...
IF AV_START_DATE = '' THEN
    SET AV_START_DATE = CASE
        WHEN MONTH(NOW()) >= 7 THEN CONCAT(DATE(CONCAT(YEAR(NOW()), '-07-01')), ' 00:00:00')
        ELSE CONCAT(DATE(CONCAT(YEAR(NOW()) - 1, '-07-01')), ' 00:00:00')
    END;
END IF;
IF AV_END_DATE = '' THEN
    SET AV_END_DATE = CASE
        WHEN MONTH(NOW()) >= 7 THEN CONCAT(DATE(CONCAT(YEAR(NOW()) + 1, '-06-30')), ' 00:00:00')
        ELSE CONCAT(DATE(CONCAT(YEAR(NOW()), '-06-30')), ' 00:00:00')
    END;
END IF;
