DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_APPROVERS_FOR_ROLE_TYPE`(
...
ELSEIF AV_ROLE_TYPE = 48 THEN
	...
	IF AV_MODULE_CODE = 20 THEN
	SELECT spr.PERSON_ID FROM SR_PERSON_ROLES spr
    INNER JOIN role_rights rr ON rr.role_id = spr.role_id
    INNER JOIN rights r ON r.RIGHT_ID = rr.RIGHT_ID AND r.RIGHT_NAME = 'SERVICE_REQUEST_WATCHER'
