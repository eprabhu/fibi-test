DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_AWARD_DASHBOARD_COUNT_SAN`(
                                ...
                                SET TAB_QUERY = CONCAT(' WHERE (T1.LEAD_UNIT_NUMBER IN(SELECT DISTINCT UNIT_NUMBER FROM PERSON_ROLE_RT WHERE RIGHT_NAME IN (''VIEW_AWARD'',''MODIFY_AWARD'')
                                                   ...
                                                   OR T1.AWARD_ID IN(SELECT AWARD_ID FROM AWARD_PERSONS
                                                        ...
                                                        OR T1.AWARD_ID IN(SELECT AWARD_ID FROM AWARD_PERSON_ROLES
                                                                                         WHERE PERSON_ID=''',AV_PERSON_ID,''')) AND T1.AWARD_SEQUENCE_STATUS NOT IN (''ARCHIVE'', ''CANCELLED'')' );
...
ELSE
                                    ...
                                    T1.STATUS_CODE,
                                                                        T7.NAME as NAME,
                                    T7.GRANT_HEADER_ID AS GRANT_HEADER_ID,
                                    T1.UPDATE_TIMESTAMP,
                                                                        T8.UNIT_NAME ,
                                    T1.AWARD_DOCUMENT_TYPE_CODE AS AWARD_DOCUMENT_TYPE,
                                    T1.AWARD_SEQUENCE_STATUS,
