DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_PROPOSAL_DASHBOARD_COUNT`(
...
SET TAB_QUERY = CONCAT(' ((T1.CREATE_USER = (SELECT USER_NAME FROM PERSON WHERE PERSON_ID = ''',AV_PERSON_ID,''')
                           OR T3.PERSON_ID = ''',AV_PERSON_ID,'''  OR T14.PERSON_ID = ''',AV_PERSON_ID,''' OR T8.PERSON_ID = ''',AV_PERSON_ID,''' AND T8.PROP_PERSON_ROLE_ID IN (3,9)) ) ' );
                 SET JOIN_CONDITION =  CONCAT(JOIN_CONDITION,'   LEFT JOIN EPS_PROPOSAL_PERSONS T14 ON T14.PROPOSAL_ID = T1.PROPOSAL_ID
												  LEFT OUTER JOIN EPS_PROPOSAL_PERSON_ROLES T3 ON T1.PROPOSAL_ID = T3.PROPOSAL_ID AND T3.ROLE_ID IN (5,6,7)');
