DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `UPDATE_CLAIM_INDIRECT_COST`(
IN AV_TYPE INT)
BEGIN
DECLARE LI_FLAG int;
...
DECLARE LS_AWARD_NUMBER VARCHAR(12);
		...
		SELECT CLAIM_ID, AWARD_ID, AWARD_NUMBER FROM CLAIM WHERE CREATE_USER = 'quickstart' and date_format(CREATE_TIMESTAMP,'%Y-%m-%d') = '2022-02-26' and (IDC_ORGINAL_AMOUNT is null or IDC_LATEST_AMOUNT is null);
