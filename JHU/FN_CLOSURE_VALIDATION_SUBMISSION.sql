DELIMITER $$
CREATE DEFINER=`jhufibi`@`%` FUNCTION `FN_CLOSURE_VALIDATION_SUBMISSION`(
...
WHERE
 REPORT_CLASS_CODE = 1 AND
    AWARD_ID = LS_AWARD_ID;
IF LS_CLAIM_COUNT = 0 AND LS_PROGRESS_COUNT = 0 THEN
RETURN 'TRUE';
END IF;
...
SELECT COUNT(*) INTO LI_PROGRESS_COUNT FROM  AWARD_PROGRESS_REPORT
    WHERE AWARD_ID = LS_AWARD_ID AND REPORT_CLASS_CODE = 1 AND PROGRESS_REPORT_STATUS_CODE = 6;
END IF;
IF LS_CLAIM_COUNT = LI_CLAIM_COUNT AND LI_PROGRESS_COUNT = LS_PROGRESS_COUNT THEN
RETURN 'TRUE';
END IF;
RETURN 'FALSE';
END
