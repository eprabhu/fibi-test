DELIMITER $$
CREATE DEFINER=`jhufibi`@`localhost` PROCEDURE `jhu_sap_awd_upd_fna_tbl`(av_grant_number  VARCHAR(12)
...
DECLARE li_fna_sequence_number INT(4) DEFAULT 0;
...
DECLARE li_fna_distribution_id 	INT;
DECLARE li_award_id int;
declare ls_error_msg varchar(4000);
  DECLARE  fna_cur CURSOR FOR
	with s as (select distinct grant_number from sap_sponsored_program
       where sponsored_program_type = 'PM' )
	  ...
	  FROM sap_grant_award g INNER JOIN  s ON  g.grant_number = s.grant_number
...
DECLARE CONTINUE HANDLER FOR SQLEXCEPTION
		...
		insert into integration_error_log (SECTION, ERROR_MESSAGE, AWARD_NUMBER, sequence_number)
				values('AWARD FNA', LS_ERROR_MSG, av_grant_number, av_sequence_number);
		END;
  BEGIN
	...
	WHERE AWARD_NUMBER = av_grant_number
	 AND   sequence_number = li_fna_sequence_number;
			...
			fna_cur_loop : LOOP
                    ...
                    select award_id into li_award_id from award where award_number = av_grant_number and sequence_number = li_fna_sequence_number  ;
					  INSERT INTO award_amt_fna_distribution (AWARD_NUMBER
																  ,sequence_number
					  ...
					  VALUES (av_grant_number
						    ...
						    ,li_fna_amount_sequence_number
							 ,li_coeus_period
							 ,date_format(ls_start_date, '%Y-%m-%d')
							 ,date_format(ls_end_date, '%Y-%m-%d')
							  ,ls_direct_cost
							 ,ls_indirect_cost
