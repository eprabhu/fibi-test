DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `INSERT_ORCID_ID_TO_WBHK_NTFY_LOG`()
BEGIN
...
VALUES( LI_ORCID_ID,UTC_TIMESTAMP(), 'quickstart', 'C');
END IF;
...
IF LI_COUNT_TWO = 0 THEN
INSERT INTO ORCID_WEBHOOK_NOTIFY_LOG(ORCID_ID, UPDATE_TIMESTAMP, UPDATE_USER, WEBHOOK_ACTION_TYPE_CODE)
VALUES(LI_ORCID_ID,UTC_TIMESTAMP(), 'quickstart', 'D');
END IF;
