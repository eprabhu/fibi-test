DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `MIGRATE_AWARD_SPONSOR_TO_AGREEMENT`(
...
BEGIN
    ...
    IF V_COUNT = 0 THEN
        ...
        SELECT
            ...
            AV_UPDATED_USER
            FROM sponsor T1
            INNER JOIN AWARD T2 ON T1.SPONSOR_CODE = T2.SPONSOR_CODE WHERE
            T2.AWARD_ID = AV_SOURCE_MODULE_ID
        ;
	END IF;
    INSERT INTO AGREEMENT_SPONSOR (
            AGREEMENT_REQUEST_ID,
        ...
        SELECT
            ...
            AV_UPDATED_USER
            FROM sponsor T1
            INNER JOIN AWARD T2 ON T1.SPONSOR_CODE = T2.PRIME_SPONSOR_CODE WHERE
            T2.AWARD_ID = AV_SOURCE_MODULE_ID
        ;
END
$$
