DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_AWARD_DASHBOARD_FOR_LOG`(
...
IF AV_TYPE = 'A'   THEN
            ...
            END IF;
                        IF AV_LEAD_UNIT_NUMBER IS NOT NULL  AND AV_LEAD_UNIT_NUMBER <> '' THEN
                         SET LS_FILTER_CONDITION = CONCAT(LS_FILTER_CONDITION,' T.UNIT_NAME LIKE ''%',AV_LEAD_UNIT_NUMBER,'%'' AND ');
                        END IF;
...
ELSEIF AV_TAB_TYPE = 'ALL_AWARDS' THEN
                                SET TAB_QUERY = CONCAT(' WHERE (T1.LEAD_UNIT_NUMBER IN(SELECT DISTINCT UNIT_NUMBER FROM PERSON_ROLE_RT WHERE RIGHT_NAME IN (''VIEW_AWARD'', ''MODIFY_AWARD'')
                                                   ...
                                                   OR T1.AWARD_ID IN(SELECT AWARD_ID FROM AWARD_PERSONS
													...
													OR T1.AWARD_ID IN(SELECT AWARD_ID FROM AWARD_PERSON_ROLES
                                                                                         WHERE PERSON_ID=''',AV_PERSON_ID,''')) AND T1.AWARD_SEQUENCE_STATUS != ''ARCHIVE''' );
...
SET LS_FILTER_CONDITION = CONCAT(' WHERE  T.AWARD_SEQUENCE_STATUS <> ''ARCHIVE'' AND ',LS_FILTER_CONDITION);
...
END IF;
INSERT INTO LOG_SQL(log_sql) values (LS_DYN_SQL);
COMMIT;
