DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `AWARD_MASTER_DATASET_SYNC_BKP`()
BEGIN
...
WHERE  ((A3.AWARD_SEQUENCE_STATUS IN('PENDING','ARCHIVE','CANCELLED') AND A3.SEQUENCE_NUMBER = 1)
...
AREA_OF_RESEARCH
        ...
        IFNULL(T34.LATEST_APPROVED_BUDGET,0.00) AS LATEST_APPROVED_BUDGET,
         (CASE WHEN LATEST_APPROVED_BUDGET IS NOT NULL THEN IFNULL(ROUND((T31.CUMULATIVE_ACTUAL_EXPENSE/T34.LATEST_APPROVED_BUDGET)*100,2),0) ELSE 0 END)
		 AS UTILIZATION_RATE,
        T1.DURATION,
