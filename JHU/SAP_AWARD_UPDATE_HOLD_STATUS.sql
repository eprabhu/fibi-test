DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SAP_AWARD_UPDATE_HOLD_STATUS`(
)
    DETERMINISTIC
BEGIN
...
DECLARE LI_FEED_ID INT;
	...
	BEGIN
		...
		VALUES(CONCAT(' SAP_AWARD_UPDATE_HOLD_STATUS ',SUBSTR(LS_ERROR_MSG,1,999)),NOW(),'quickstart');
		COMMIT;
	END;
BEGIN
