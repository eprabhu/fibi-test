DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `SAP_AWARD_UPDATE_HOLD_STATUS_MANUALLY`(
AV_AWARD_ID INT
)
    DETERMINISTIC
BEGIN
...
DECLARE LI_FEED_ID INT;
	...
	BEGIN
		...
		VALUES(CONCAT(' SAP_AWARD_UPDATE_HOLD_STATUS_MANUALLY ',SUBSTR(LS_ERROR_MSG,1,999)),NOW(),'quickstart');
	END;
