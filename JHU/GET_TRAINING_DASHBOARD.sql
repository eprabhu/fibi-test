DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GET_TRAINING_DASHBOARD`(
...
END IF;
  SET LS_DYN_SQL =CONCAT('SELECT  *  FROM(SELECT (SELECT IF(T1.IS_NON_EMPLOYEE = ''Y'',
			...
			IFNULL(T2.DESCRIPTION,T3.CERTIFICATION_NAME) AS DESCRIPTION, T1.UPDATE_TIMESTAMP ,T1.PERSON_ID, T1.IS_NON_EMPLOYEE ,T1.PERSON_TRAINING_ID,T1.FOLLOWUP_DATE,T1.TRAINING_CODE, T1.DATE_ACKNOWLEDGED, T1.SCORE, T3.SOURCE_TYPE_CODE ' ,SELECTED_FIELD_LIST,
' FROM PERSON_TRAINING T1 INNER JOIN PERSON_TRAINING_EXT T3 ON T1.PERSON_TRAINING_ID = T3.PERSON_TRAINING_ID LEFT JOIN TRAINING T2 ON T1.TRAINING_CODE = T2.TRAINING_CODE
