...
# ============================================
echo "📦 Syncing all Release CORE folders..."
...
if [ -n "$RELEASE_FOLDERS" ]; then
  for RELEASE_DIR in $RELEASE_FOLDERS; do
    ...
    if [ -n "$CORE_PATH" ] && [ -d "$CORE_PATH" ]; then
      ...
      echo "✅ Synced $RELEASE_NAME CORE to $DEST_DIR"
    else
      echo "⚠️  No CORE folder found in $RELEASE_NAME"
    fi
  done
else
  echo "⚠️  No release folders found"
fi

# ============================================
# SYNC ALL SPRINT CORE FOLDERS
# ============================================
echo "📦 Syncing all Sprint CORE folders..."
...
if [ -n "$SPRINT_FOLDERS" ]; then
  for SPRINT_DIR in $SPRINT_FOLDERS; do
    ...
    echo "Processing sprint: $SPRINT_NAME"
    ...
    if [ -n "$CORE_PATH" ] && [ -d "$CORE_PATH" ]; then
      ...
      if [ -d "$DEST_DIR" ]; then
        echo "Removing existing $SPRINT_NAME folder for clean sync..."
        rm -rf "$DEST_DIR"
      fi
      ...
      echo "✅ Synced $SPRINT_NAME CORE to $DEST_DIR"
    else
      echo "⚠️  No CORE folder found in $SPRINT_NAME"
    fi
  done
else
  echo "⚠️  No sprint folders found"
fi

# ============================================
# SYNC ALL ROUTINES FILES
# ============================================
echo "📦 Syncing all ROUTINES files from all YAML files..."

# Find all routines YAML files - ONLY from BASE/CORE (exclude client-based CORE)
...
if [ -n "$ROUTINES_YAML_FILES" ]; then
  ...
  echo "✅ Finished syncing all ROUTINES files"
