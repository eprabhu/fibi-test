...
WITH 
	...
	MY_TABS AS (
     ...
     SELECTED_TAB AS (
        SELECT TAB_TYPE_ID, DISPLAY_NAME, TAB_KEY, SORT_ORDER, IS_PRIMARY_TAB, IS_AUTO_SEARCH, SOURCE_ID, SOURCE_TAB_ID FROM MY_TABS WHERE ( AV_TAB_ID = 0 AND IS_PRIMARY_TAB = 'Y') OR (TAB_TYPE_ID = AV_TAB_ID)  LIMIT 1
     ),
     MY_FILTERS AS (
