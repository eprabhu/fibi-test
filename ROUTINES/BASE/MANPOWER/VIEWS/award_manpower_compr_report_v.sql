CREATE VIEW `award_manpower_compr_report_v` AS
    SELECT DISTINCT
        ...
        IFNULL(`t16`.`EXPENSE_AMOUNT`, 0) AS `EXPENSE_AMOUNT`,
        IFNULL(`t17`.`ACTUAL_HEAD_COUNT`, 0) AS `ACTUAL_HEAD_COUNT`,
        IFNULL(`t15`.`COMMITTED_AMOUNT`, 0) AS `COMMITTED_AMOUNT`,
        CAST(`t`.`QUANTITY` AS UNSIGNED) AS `APPROVED_HEAD_COUNT`,
        `t18`.`DESCRIPTION` AS `COST_ELEMENT`,
        `t`.`LINE_ITEM_COST` AS `LINE_ITEM_COST`,
        `t`.`JOB_REQUISITION_STATUS` AS `POSITION_ID_STATUS`,
        `t`.`PLAN_START_DATE` AS `PROPOSED_START_DATE`,
        `t`.`PLAN_END_DATE` AS `PROPOSED_END_DATE`,
        `t`.`PROPOSED_COST_ALLOCATION` AS `PROPOSED_COST_ALLOCATION`,
    ...
    FROM
        (((((((((((((((((SELECT 
                ...
                `s2`.`COST_ALLOCATION` AS `PROPOSED_COST_ALLOCATION`,
                ...
                `s5`.`JOB_REQUISITION_STATUS` AS `JOB_REQUISITION_STATUS`,
        ...
        LEFT JOIN `workday_position_requisition` `s5` ON ((`s5`.`POSITION_ID` = `s2`.`POSITION_ID`)))
        ...
        JOIN `award_budget_detail` `s9` ON (((`s9`.`BUDGET_HEADER_ID` = `s8`.`BUDGET_HEADER_ID`)
        ...
        WHERE
            (((`s2`.`PERSON_ID` IS NULL)
                ...
                NULL AS `JOB_REQUISITION_STATUS`,
                `s9`.`BUDGET_DETAILS_ID` AS `BUDGET_DETAILS_ID`,
                `s9`.`cost_element` AS `COST_ELEMENT`,
                `s9`.`LINE_ITEM_DESCRIPTION` AS `LINE_ITEM_DESCRIPTION`,
                `s9`.`LINE_ITEM_COST` AS `LINE_ITEM_COST`,
        ...
        JOIN `cost_element` `t18` ON ((`t18`.`COST_ELEMENT` = `t`.`COST_ELEMENT`)))
        ...
        GROUP BY `ta2`.`BUDGET_REFERENCE_NUMBER` UNION SELECT 
