CREATE PROCEDURE GET_MANPOWER_COST_ALLOCATION_DETAILS_COUNT(
...
END IF;
SET LS_DYN_SQL = CONCAT('SELECT DISTINCT count(*) FROM (SELECT DISTINCT T1.MANPOWER_WORKDAY_RESOURCE_ID, T1.COST_ALLOCATION, T1.WORKDAY_REFERENCE_ID, T1.POSITION_ID, T1.WBS_NUMBER, T1.CHARGE_START_DATE,
