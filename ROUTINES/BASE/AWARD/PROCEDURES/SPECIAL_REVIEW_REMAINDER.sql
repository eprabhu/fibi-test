-- `SPECIAL_REVIEW_REMAINDER`; 

CREATE PROCEDURE `SPECIAL_REVIEW_REMAINDER`( AV_DAYS INT )
BEGIN
	 SELECT 0 AS SUB_MODULE_ITEM_KEY, T1.AWARD_ID AS MODULE_ITEM_KEY, 1 AS MODULE_CODE, 0 AS SUB_MODULE_CODE, T1.PROTOCOL_NUMBER, T1.APPLICATION_DATE, T1.APPROVAL_DATE, T1.EXPIRATION_DATE, T1.COMMENTS, T2.DESCRIPTION AS REVIEW_TYPE, T3.DESCRIPTION AS APPROVAL_STATUS 
    ...
    LEFT JOIN IRB_PROTOCOL_STATUS T4 ON T2.PROTOCOL_STATUS_CODE = T4.PROTOCOL_STATUS_CODE
    ...
    INNER JOIN SPECIAL_REVIEW T3 ON T1.SPECIAL_REVIEW_CODE = T3.SPECIAL_REVIEW_CODE
    LEFT JOIN AC_PROTOCOL_STATUS T4 ON T2.PROTOCOL_STATUS_CODE = T4.PROTOCOL_STATUS_CODE
	WHERE date(T2.INITIAL_SUBMISSION_DATE) = date(DATE_ADD(utc_timestamp(),INTERVAL AV_DAYS DAY))
