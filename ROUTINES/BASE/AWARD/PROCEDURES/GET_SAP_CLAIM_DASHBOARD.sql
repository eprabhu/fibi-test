-- `GET_SAP_CLAIM_DASHBOARD`; 

CREATE PROCEDURE `GET_SAP_CLAIM_DASHBOARD`(
...
IF AV_TAB_TYPE = 'BATCH_DETAIL' THEN
		SET JOIN_CONDITION = CONCAT('SELECT T1.FEED_ID,T1.BATCH_ID,T1.CLAIM_ID,T1.CLAIM_NUMBER,T1.INVOICE_ID,T1.FEED_STATUS_CODE,T1.FEED_TYPE_CODE,T1.USER_ACTION_CODE,T5.USER_ACTION AS USER_ACTION, T6.DESCRIPTION AS FEED_STATUS, T7.DESCRIPTION AS FEED_TYPE
                                            ,T1.UPDATE_TIMESTAMP,T1.UPDATE_USER,T8.FULL_NAME AS UPDATE_USER_NAME,T4.ACCOUNT_NUMBER,T4.AWARD_NUMBER,T1.USER_COMMENT, ((select count(1) from claim_invoice_log WHERE CLAIM_ID = T1.CLAIM_ID AND BATCH_ID = T1.BATCH_ID AND STATUS = ''E'' ) > 0) AS CAN_REVISION
  ...
  ELSEIF AV_TAB_TYPE = 'PENDING_FEEDS' THEN
		...
		SET JOIN_CONDITION = CONCAT('SELECT null as BATCH_ID, T1.FEED_ID,T1.CLAIM_ID,T1.CLAIM_NUMBER,T1.INVOICE_ID,T1.FEED_STATUS_CODE,T1.FEED_TYPE_CODE,T1.USER_ACTION_CODE,T5.USER_ACTION AS USER_ACTION, T6.DESCRIPTION AS FEED_STATUS, T7.DESCRIPTION AS FEED_TYPE,
                                            T1.UPDATE_TIMESTAMP,T1.UPDATE_USER,T8.FULL_NAME AS UPDATE_USER_NAME,T4.ACCOUNT_NUMBER,T4.AWARD_NUMBER, null as USER_COMMENT,0 AS CAN_REVISION
                                    FROM SAP_CLAIM_FEED T1
									LEFT JOIN CLAIM T3 ON T3.CLAIM_ID = T1.CLAIM_ID
                                    LEFT JOIN AWARD T4 ON T4.AWARD_ID = T3.AWARD_ID
									LEFT JOIN SAP_FEED_USER_ACTIONS T5 ON T1.USER_ACTION_CODE = T5.USER_ACTION_CODE
...
SET LS_DYN_SQL =CONCAT('SELECT  T.BATCH_ID, MAX(T.NO_OF_RECORDS) AS NO_OF_RECORDS, T.CREATE_TIMESTAMP, T.RESPONSE_TIMESTAMP, MAX(T.TOTAL_ERROR_INVOICE) AS TOTAL_ERROR_INVOICE FROM(
