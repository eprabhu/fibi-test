-- `AWARD_EXPENSE_TRACKER_REFRESH_INSERT`; 

CREATE PROCEDURE `AWARD_EXPENSE_TRACKER_REFRESH_INSERT`(
AV_FILE_ID INT)
BEGIN   
...
DECLARE LS_REFERENCE_ORG_UNIT 				VARCHAR(50);
...
BEGIN
	...
	BEGIN
		...
		VALUES(LI_SEQ_ERROR_LOG_ID,LS_ACCOUNT_NUMBER,LS_INTERNAL_ORDER_CODE,LS_ERROR_MSG,'AWARD_EXPENSE_TRACKER_REFRESH',utc_timestamp(),'admin');
		COMMIT;
		SET LI_ERROR_FLAG = 1;
	END;
	SET LI_ERROR_FLAG = 0;
	BEGIN
		...
		SELECT 
			...
			TRANSACTION_NUMBER,
			FILE_TYPE
	FROM AWARD_EXPENSE_TRANSACTIONS_RT_17OCT2020 WHERE FILE_ID = AV_FILE_ID;
	...
	IF LI_ERROR_FLAG = 0 THEN
		IF LS_FILE_TYPE = 'INSERT' THEN
			SELECT IFNULL(MAX(AWARD_EXPENSE_TRANS_ID),0)+1 INTO LI_AWARD_EXPENSE_TRANS_ID FROM AWARD_EXPENSE_TRANSACTIONS;
			...
			ELSEIF LS_FILE_TYPE = 'UPDATE' THEN
