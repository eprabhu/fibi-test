CREATE PROCEDURE ADD_COLUMN_IN_AGREEMENT_CUSTOM_DATA_RT(AV_CUSTOM_DATA_ELEMENTS_ID INT, AV_ACTYPE VARCHAR(5))
PROC:BEGIN
    ...
    END IF;
     ...
     IF NOT EXISTS (
            ...
            WHERE TABLE_NAME = 'AGREEMENT_CUSTOM_DATA_RT' 
            ...
            END IF;
              ...
              SET @SQL = CONCAT('ALTER TABLE AGREEMENT_CUSTOM_DATA_RT ADD COLUMN ', LS_NEW_COLUMN_NAME, ' ', LS_DATATYPE, ' AFTER AGREEMENT_REQUEST_ID', LS_MUTLI_ANS_INDEX);
    ...
    CALL UPDATE_DASHBOARD_LOOKUPS_FOR_CUSTOM_DATA(AV_CUSTOM_DATA_ELEMENTS_ID, 13, AV_ACTYPE, LS_DATA_TYPE_CODE);

    ELSEIF AV_ACTYPE = 'D' THEN
    
    CALL UPDATE_DASHBOARD_LOOKUPS_FOR_CUSTOM_DATA(AV_CUSTOM_DATA_ELEMENTS_ID, 13, AV_ACTYPE, NULL);

         SET LS_COLUMN_NAME = CONCAT('CUST_ELEMENTS_ID_', AV_CUSTOM_DATA_ELEMENTS_ID);
        
         SELECT COUNT(*) INTO LI_COUNT 
        FROM INFORMATION_SCHEMA.COLUMNS 
        WHERE TABLE_NAME = 'AGREEMENT_CUSTOM_DATA_RT' AND COLUMN_NAME = LS_COLUMN_NAME;

        IF LI_COUNT > 0 THEN 
            SET @SQL = CONCAT('ALTER TABLE AGREEMENT_CUSTOM_DATA_RT DROP COLUMN ', LS_COLUMN_NAME);
