DELIMITER $$
CREATE DEFINER=`fibi`@`%` PROCEDURE `MIGRATE_AGREEMENT_PI_TO_AGREEMENT`(
...
BEGIN
    ...
    UNION
    SELECT ROLE_ID, 17 AS ROLE_TYPE FROM MODULE_DERIVED_ROLES WHERE MODULE_CODE = '13' AND IS_ACTIVE = 'Y' AND AUTO_GRANT_TO_COI = 'Y'
    )
    SELECT  AP.PERSON_ID, MDR.ROLE_ID AS ROLE_ID FROM AGREEMENT_PEOPLE AP
    INNER JOIN DERIVED_ROLE MDR ON MDR.ROLE_TYPE = AP.PEOPLE_TYPE_ID
    WHERE PERSON_ID IS NOT NULL AND AP.PEOPLE_TYPE_ID in (3, 17) AND AP.AGREEMENT_REQUEST_ID = AV_SOURCE_MODULE_ID;
    ...
    IF NOT EXISTS (SELECT 1 FROM AGREEMENT_PEOPLE WHERE PEOPLE_TYPE_ID IN (3, 17) AND AGREEMENT_REQUEST_ID = AV_DESTINATION_MODULE_ID) THEN
        INSERT INTO AGREEMENT_PEOPLE(
            ...
            AGREEMENT_REQUEST_ID = AV_SOURCE_MODULE_ID AND PEOPLE_TYPE_ID IN (3, 17);
    END IF;
	OPEN AGMT_ROLE_CUR;
		ROLE_LOOP: LOOP
