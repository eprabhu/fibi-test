DELIMITER $$
CREATE DEFINER=`fibi`@`%` PROCEDURE `GET_SERVICE_REQUEST_WIDGET_COUNT`(
 ...
 ELSEIF AV_WIDGET = 'SERVICE_REQUEST_RESEARCH_SUMMARY_COUNT' THEN
    ...
    SR_COUNTS AS (
        SELECT
            ...
            SUM(CASE WHEN T.STATUS_CODE IN (1, 12, 14, 2, 3) OR (T.STATUS_CODE = 4 AND T.ASSIGNEE_PERSON_ID IS NULL AND T.ADMIN_GROUP_ID IS NULL) THEN 1 ELSE 0 END) AS OPENED,
            SUM(CASE WHEN T.STATUS_CODE = 8 THEN 1 ELSE 0 END) AS CLOSED,
            SUM(CASE WHEN T.STATUS_CODE = 5 THEN 1 ELSE 0 END) AS RESOLVED,
            SUM(CASE WHEN T.STATUS_CODE IN (4, 6) AND (T.ASSIGNEE_PERSON_ID IS NOT NULL OR T.ADMIN_GROUP_ID IS NOT NULL) THEN 1 ELSE 0 END) AS PENDING,
        ...
        FROM (
          ...
          WHERE(
            ...
            ', JOIN_CONDITION,LS_FILTER_CONDITIONS,
									')T)
									 SELECT ''Opened'' AS SERVICE_REQUEST_RESEARCH_SUMMARY ,''Open''AS LABEL, COALESCE(OPENED, 0) AS COUNT FROM SR_COUNTS
    UNION ALL
    SELECT ''Pending'',''Pending'', COALESCE(PENDING, 0) FROM SR_COUNTS
    UNION ALL
    SELECT ''Resolved'',''Resolved'', COALESCE(RESOLVED, 0) FROM SR_COUNTS
    UNION ALL
    SELECT ''Closed'',''Closed'', COALESCE(CLOSED, 0) FROM SR_COUNTS
    UNION ALL
