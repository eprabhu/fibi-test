DELIMITER $$
CREATE DEFINER=`fibi`@`%` PROCEDURE `MIGRATE_PROPOSAL_PI_TO_AGREEMENT`(
...
BEGIN
    ...
    SELECT  PP.PERSON_ID, MDR.ROLE_ID AS ROLE_ID FROM EPS_PROPOSAL_PERSONS PP
    INNER JOIN DERIVED_ROLE MDR ON MDR.ROLE_TYPE = PP.PROP_PERSON_ROLE_ID
    WHERE PERSON_ID IS NOT NULL AND PP.PROP_PERSON_ROLE_ID in (3, 1) AND PP.PROPOSAL_ID = AV_SOURCE_MODULE_ID;
    ...
    IF NOT EXISTS (SELECT 1 FROM AGREEMENT_PEOPLE WHERE PEOPLE_TYPE_ID IN (3, 17) AND AGREEMENT_REQUEST_ID = AV_DESTINATION_MODULE_ID) THEN
        INSERT INTO AGREEMENT_PEOPLE(
            ...
            PROPOSAL_ID = AV_SOURCE_MODULE_ID AND PROP_PERSON_ROLE_ID IN (1, 3);
    END IF;
	OPEN AGMT_ROLE_CUR;
		ROLE_LOOP: LOOP
