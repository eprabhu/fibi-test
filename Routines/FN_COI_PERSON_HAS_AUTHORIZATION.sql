DELIMITER $$
CREATE DEFINER=`fibi`@`%` FUNCTION `FN_COI_PERSON_HAS_AUTHORIZATION`(
    ...
    IF AV_SUB_MODULE_CODE = 801 THEN
        ...
        WHERE PR.PERSON_ID = AV_LOGGED_IN_PERSON_ID
          AND (( PR. DESCEND_FLAG = 'Y' AND  EXISTS (SELECT 1
				...
				WHERE CR.ASSIGNEE_PERSON_ID = AV_LOGGED_IN_PERSON_ID AND CDPER.PERSON_ENTITY_ID = AV_MODULE_ITEM_KEY;
				IF LI_COUNT > 0 THEN
                    RETURN LI_AUTHORIZED;
                END IF;
				...
				WHERE O.ASSIGNEE_PERSON_ID = AV_LOGGED_IN_PERSON_ID AND ODPER.PERSON_ENTITY_ID = AV_MODULE_ITEM_KEY;
				IF LI_COUNT > 0 THEN
                    RETURN LI_AUTHORIZED;
                END IF;
