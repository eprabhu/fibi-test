DELIMITER $$
CREATE DEFINER=`fibi`@`%` PROCEDURE `UPD_DOCS_WITH_ORG_ENTITY`(
AV_DUPLICATE_ENTITY_ID                        INT(10),
AV_ORIGINAL_ENTITY_ID                         INT(10)
)
BEGIN
DECLARE LI_ORG_ENTITY_NUMBER 		INT;
SELECT ENTITY_NUMBER INTO LI_ORG_ENTITY_NUMBER FROM ENTITY WHERE ENTITY_ID = AV_ORIGINAL_ENTITY_ID;
SET SQL_SAFE_UPDATES = 0;
-- UPDATE ENTITY set VERSION_STATUS = 'ARCHIVE' where ENTITY_ID = AV_DUPLICATE_ENTITY_ID;
UPDATE PERSON_ENTITY set ENTITY_ID = AV_ORIGINAL_ENTITY_ID, ENTITY_NUMBER = LI_ORG_ENTITY_NUMBER
where ENTITY_ID = AV_DUPLICATE_ENTITY_ID;
