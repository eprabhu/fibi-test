DELIMITER $$
CREATE DEFINER=`fibi`@`%` PROCEDURE `AWARD_CLAIM_REMINDER_MONTHLY`( AV_DAYS INT )
BEGIN
                ...
                AND (DAY(date(DATE_ADD(CONVERT_TZ(utc_timestamp(),'+00:00','+8:00'),INTERVAL AV_DAYS DAY))) = '1')
                AND 0 = (
