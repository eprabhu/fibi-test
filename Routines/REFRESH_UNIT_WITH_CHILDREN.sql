DELIMITER $$
CREATE DEFINER=`fibi`@`%` PROCEDURE `REFRESH_UNIT_WITH_CHILDREN`()
    DETERMINISTIC
BEGIN
 DECLARE LS_UNIT_NUMBER VARCHAR(8);
 DECLARE LS_UNIT_NAME VARCHAR(60);
 DECLARE LI_ID INT;
 DECLARE DONE INT DEFAULT FALSE;
 DECLARE CUR CURSOR FOR
 SELECT UNIT_NUMBER,UNIT_NAME FROM UNIT_TMP;
 DECLARE CONTINUE HANDLER FOR NOT FOUND SET DONE = TRUE;
 TRUNCATE TABLE MITKC_UNIT_WITH_CHILDREN;
 OPEN CUR;
    INSERT_LOOP: LOOP
        FETCH CUR INTO
        LS_UNIT_NUMBER,
  ...
  INSERT INTO MITKC_UNIT_WITH_CHILDREN( UNIT_NUMBER, UNIT_NAME, CHILD_UNIT_NUMBER, CHILD_UNIT_NAME )
