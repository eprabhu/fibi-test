DELIMITER $$
CREATE DEFINER=`fibi`@`%` FUNCTION `FN_CHK_FCOI_REQUIRED`(
AV_PERSON_ID VARCHAR(40)
) RETURNS int
BEGIN
DECLARE LI_COUNT				  int;
DECLARE LS_PARAMETER			  VARCHAR(10);
DECLARE LS_IS_DISCLOSURE_REQUIRED_FLAG			  VARCHAR(10);
DECLARE LS_IS_USER_PRESENT			  VARCHAR(10);
SELECT VALUE INTO LS_PARAMETER FROM PARAMETER WHERE PARAMETER_NAME = 'ALLOW_FCOI_WITHOUT_PROJECT';
IF LS_PARAMETER = 'Y' THEN
	SELECT IS_DISCLOSURE_REQUIRED INTO LS_IS_DISCLOSURE_REQUIRED_FLAG
	FROM PERSON
	WHERE PERSON_ID = AV_PERSON_ID;
		IF LS_IS_DISCLOSURE_REQUIRED_FLAG = 'Y' THEN
			RETURN 1;
		ELSE
			SELECT CASE WHEN
			  EXISTS (
				SELECT 1
				FROM COI_INT_STAGE_AWARD_PERSON p1
				JOIN COI_INT_STAGE_AWARD a1 ON p1.PROJECT_NUMBER = a1.PROJECT_NUMBER
				WHERE p1.KEY_PERSON_ID = AV_PERSON_ID
				  AND p1.STATUS = 'A'
				  AND a1.PROJECT_STATUS_CODE IN (1, 6, 3)
			  )
			  OR EXISTS (
				SELECT 1
				FROM COI_INT_STAGE_DEV_PROPOSAL_PERSON p2
				JOIN COI_INT_STAGE_DEV_PROPOSAL d1 ON p2.PROPOSAL_NUMBER = d1.PROPOSAL_NUMBER
				WHERE p2.KEY_PERSON_ID = AV_PERSON_ID
				  AND p2.STATUS = 'A'
				  AND d1.PROPOSAL_STATUS_CODE IN (1, 2, 3, 12)
			  )
			  OR EXISTS (
				SELECT 1
				FROM COI_INT_STAGE_PROPOSAL_PERSON p3
				JOIN COI_INT_STAGE_PROPOSAL p4 ON p3.PROJECT_NUMBER = p4.PROJECT_NUMBER
				WHERE p3.KEY_PERSON_ID = AV_PERSON_ID
				  AND p3.STATUS = 'A'
				  AND p4.PROJECT_STATUS_CODE IN (1)
			  )
			THEN 'Y' ELSE 'N' END AS IS_USER_PRESENT INTO LS_IS_USER_PRESENT;
                IF LS_IS_USER_PRESENT = 'Y' THEN
					RETURN 1;
				END IF;
			END IF;
	-- RETURN 1;
