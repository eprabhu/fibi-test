DELIMITER $$
CREATE DEFINER=`fibi`@`%` PROCEDURE `AGT_LOC_REVIEW_END_DATE_REMAINDER`( AV_DAYS INT )
BEGIN
SELECT
    13 AS MODULE_CODE,
...
AND t3.REVIEW_END_DATE IS NOT NULL
AND (DATE_ADD(DATE(UTC_TIMESTAMP()), INTERVAL AV_DAYS DAY) = DATE(t3.REVIEW_END_DATE) );
END
$$
DELIMITER ;
