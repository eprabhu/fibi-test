DELIMITER $$
CREATE DEFINER=`fibi`@`%` PROCEDURE `MIGRATE_AWARD_SPECIAL_REVIEW`()
BEGIN
...
FROM AWARD A3
INNER JOIN AWARD_SPECIAL_REVIEW ASR ON A3.AWARD_ID = ASR.AWARD_ID
WHERE A3.AWARD_ID IN (SELECT AWARD_ID FROM AWARD_MASTER_DATASET_RT)
...
END;
 ...
 SET SQL_SAFE_UPDATES = 0;
 UPDATE AWARD_MASTER_DATASET_RT AMDR
INNER JOIN (
    SELECT
        ASR.AWARD_ID,
        MAX(T48.DESCRIPTION) AS SPECIAL_REVIEW
    FROM AWARD_SPECIAL_REVIEW ASR
    INNER JOIN SPECIAL_REVIEW T48
        ON T48.SPECIAL_REVIEW_CODE = ASR.SPECIAL_REVIEW_CODE
    INNER JOIN AWARD A
        ON A.AWARD_ID = ASR.AWARD_ID
    WHERE (
        (A.AWARD_SEQUENCE_STATUS = 'PENDING' AND A.SEQUENCE_NUMBER = 1)
        OR A.AWARD_SEQUENCE_STATUS = 'ACTIVE'
    )
    GROUP BY ASR.AWARD_ID
    HAVING COUNT(*) = 1
) SR
    ON AMDR.AWARD_ID = SR.AWARD_ID
SET AMDR.SPECIAL_REVIEW = SR.SPECIAL_REVIEW
WHERE AMDR.AWARD_ID NOT IN (
    SELECT AWARD_ID
    FROM TEMP_AWARD_MASTER_DATASET
);
 SET SQL_SAFE_UPDATES = 1;
SELECT 'MIGRATE_AWARD_SPECIAL_REVIEW executed successfully' AS SUCCESS FROM DUAL;
 END
$$
DELIMITER ;
