DELIMITER $$
CREATE DEFINER=`fibi`@`%` PROCEDURE `COI_SYNC_FCOI_DISCLOSURE`(
    ...
    IF LI_LOCK_ACQUIRED = 1 THEN
      ...
      T1.MODULE_CODE = 3 AND T1.MODULE_ITEM_KEY IN (
          ...
          AND T1.PERSON_STATUS = 'I')
                        ...
                        T1.MODULE_CODE = 3 AND T1.MODULE_ITEM_KEY IN (
							SELECT * FROM (
                SELECT DISTINCT cdp.MODULE_ITEM_KEY
                FROM coi_discl_projects cdp
                JOIN coi_disclosure cd ON cdp.DISCLOSURE_ID = cd.DISCLOSURE_ID
                JOIN coi_project_proposal_v cpp ON cdp.MODULE_ITEM_KEY = cpp.EXTERNAL_SYSTEM_REF_ID
                WHERE cd.PERSON_ID = AV_PERSON_ID
                  AND cd.PERSON_ID NOT IN (
                      SELECT KEY_PERSON_ID
                      FROM coi_project_proposal_v
                      WHERE EXTERNAL_SYSTEM_REF_ID = cdp.MODULE_ITEM_KEY
                  )
            ) AS sub2
                              )
                        )
                        ...
                        /*OR
                        (T1.MODULE_CODE = 1 AND T1.MODULE_ITEM_KEY IN (
    						SELECT T1.EXTERNAL_SYSTEM_REF_ID
    						FROM COI_PROJECT_AWARD_V T1
    						WHERE
                            T1.KEY_PERSON_ID <> AV_PERSON_ID
    					)
                        )*/
    				);
    		    COMMIT;
