DELIMITER $$
CREATE DEFINER=`fibi`@`%` PROCEDURE `AWARD_EXPENSE_TRACKER_REFRESH`(AV_FILE_ID INT)
BEGIN
	...
	INSERT INTO AWARD_EXPENSE_TEMP (EXP_RT_ID, EXP_TRX_ID,FILE_ID,AWARD_NUMBER, ACCOUNT_NUMBER)
		(WITH temp AS
			...
			AV_FILE_ID,
		   (SELECT AWARD_NUMBER FROM AWARD WHERE AWARD_SEQUENCE_STATUS = 'ACTIVE' AND ACCOUNT_NUMBER = SUBSTRING(RT.ACCOUNT_NUMBER, 1, 15) LIMIT 1),
			SUBSTRING(RT.ACCOUNT_NUMBER, 1, 15)
			FROM AWARD_EXPENSE_TRANSACTIONS_RT  RT WHERE FILE_ID = AV_FILE_ID);
	COMMIT;
