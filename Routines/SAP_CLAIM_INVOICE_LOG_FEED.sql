DELIMITER $$
CREATE DEFINER=`fibi`@`%` PROCEDURE `SAP_CLAIM_INVOICE_LOG_FEED`(
...
DECLARE LS_STATUS                     VARCHAR(1);
	...
	BEGIN
		...
		IF LS_FEED_STATUS_CODE = 'P' AND LI_SEQUENCE_NUMBER = 1 AND LS_USER_ACTION_CODE <> '3' THEN
				...
				SELECT INVOICE_ID,SEQUENCE_NUMBER,
												...
												ASSIGNMENT_FIELD,DESCRIPTION,
												GL_ACCOUNT_CODE,GRANT_CODE,
												PROFIT_CENTRE,UTC_TIMESTAMP(),'quickstart',LI_BATCH_ID,CUSTOMER_NUMBER
	...
	ELSE IF LS_FEED_STATUS_CODE = 'P' AND LI_SEQUENCE_NUMBER > 1 AND  LS_USER_ACTION_CODE <> '3' THEN
		...
		SELECT IFNULL(MAX(CLAIM_INVOICE_LOG_ID),1) INTO LI_INVOICE_LOG_ID FROM CLAIM_INVOICE_LOG WHERE claim_id = LI_CLAIM_ID;
...
IF LI_ERROR_INVOICE = 0 THEN
		...
		SELECT LI_INVOICE_ID,LI_SEQUENCE_NUMBER,
										...
										ASSIGNMENT_FIELD,DESCRIPTION,
										GL_ACCOUNT_CODE,GRANT_CODE,
										PROFIT_CENTRE,UTC_TIMESTAMP(),'quickstart',LI_BATCH_ID,OUTPUT_DOCUMENT_NUMBER,CUSTOMER_NUMBER
		...
		INSERT INTO SAP_CLAIM_FEED_BATCH_ERROR_LOG(ERROR_MESSAGE, UPDATE_TIMESTAMP,UPDATE_USER)
			VALUES((SELECT OUTPUT_DOCUMENT_NUMBER
		FROM  CLAIM_INVOICE_LOG  WHERE CLAIM_INVOICE_LOG_ID IN (SELECT MAX(CLAIM_INVOICE_LOG_ID) FROM CLAIM_INVOICE_LOG WHERE CLAIM_ID = LI_CLAIM_ID)),UTC_TIMESTAMP(),LS_UPDATE_USER);
		INSERT INTO CLAIM_INVOICE_LOG (INVOICE_ID,SEQUENCE_NUMBER,
										CLAIM_ID,CLAIM_NUMBER,
										DOCUMENT_TYPE_CODE,CURRENCY_CODE,
										DOCUMENT_HEADER_TEXT,CUSTOMER_EMAIL_ADDRESS,
		...
		SELECT INVOICE_ID,LI_SEQUENCE_NUMBER,
										...
										ASSIGNMENT_FIELD,DESCRIPTION,
										GL_ACCOUNT_CODE,GRANT_CODE,
										PROFIT_CENTRE,UTC_TIMESTAMP(),'quickstart',LI_BATCH_ID,(SELECT OUTPUT_DOCUMENT_NUMBER
		...
		IF LI_COUNT > 1 THEN
				BEGIN
					...
					BEGIN
							...
							SELECT @full_error INTO LS_ERROR_MSG;
							INSERT INTO SAP_CLAIM_FEED_BATCH_ERROR_LOG(BATCH_ID,ERROR_MESSAGE, UPDATE_TIMESTAMP, UPDATE_USER)
							VALUES(LI_BATCH_ID,LS_ERROR_MSG,UTC_TIMESTAMP(),'quickstart');
					END;
					...
					MIGRATION_CLAIM_INVOICE_CURSOR2_LOOP : LOOP
						...
						SELECT INVOICE_ID,LI_SEQUENCE_NUMBER,
														...
														ASSIGNMENT_FIELD,DESCRIPTION,
														GL_ACCOUNT_CODE,GRANT_CODE,
														PROFIT_CENTRE,UTC_TIMESTAMP(),'quickstart',LI_BATCH_ID,INPUT_DOCUMENT_NUMBER,CUSTOMER_NUMBER
						FROM  CLAIM_INVOICE_LOG  WHERE CLAIM_INVOICE_LOG_ID = LI_INVOICE_LOG_ID AND STATUS='E';
						SELECT LAST_INSERT_ID() INTO LI_CLAIM_INVOICE_LOG_ID;
						INSERT INTO `CLAIM_INVOICE_DETAILS_LOG` (
																INVOICE_ID,
																CLAIM_INVOICE_LOG_ID,
				...
				END;
			ELSE
			SELECT IFNULL(MAX(CLAIM_INVOICE_LOG_ID),1)  INTO LI_INVOICE_LOG_ID FROM CLAIM_INVOICE_LOG WHERE INVOICE_ID = LI_INVOICE_ID;
			SELECT
