DELIMITER $$
CREATE DEFINER=`fibi`@`%` FUNCTION `FN_MAP_TRAINING_TYPE`(
...
BEGIN
 ...
 DECLARE LI_TRAINING_CODE  DECIMAL(18,6);
  IF (LI_STAGE_NUMBER = 1) then
    ...
    elseif (upper(trim(substr(LS_TRAINING_GROUP,7))) = upper(substr('GCP Â¿ Social and Behavioral Research Best Practices for Clinical Research',7))) then
