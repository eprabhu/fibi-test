DELIMITER $$
CREATE DEFINER=`fibi`@`%` PROCEDURE `GET_WORKDAY_MANPOWER_RETRIGGER_DETAILS`(
...
END IF;
SET LS_DYN_SQL = CONCAT('SELECT DISTINCT *  FROM(SELECT T1.WORKDAY_MANPOWER_INTERFACE_ID, T1.AWARD_NUMBER, T4.TITLE, T1.INTERFACE_TYPE_CODE, T3.DESCRIPTION AS INTERFACE_TYPE, T1.INTERFACE_STATUS_CODE,
