-- `AGREEMENT_CUSTOM_DATA_RT_SYNC_BY_AGREEMENT_ID`; 

CREATE PROCEDURE `AGREEMENT_CUSTOM_DATA_RT_SYNC_BY_AGREEMENT_ID`(AV_MODULE_ITEM_KEY INT, AV_COPY_MODULE_ITEM_KEY INT)
PROC:BEGIN
    ...
    DECLARE SEL_COLUMN_NAME CURSOR FOR
        WITH AGREE_CUSTOM_DATA AS (
        SELECT DISTINCT CONCAT('CUST_ELEMENTS_ID_',CD.CUSTOM_DATA_ELEMENTS_ID) AS 'CUST_ELEMENT' FROM
        CUSTOM_DATA CD
		INNER JOIN CUSTOM_DATA_ELEMENT_USAGE CDU ON (CDU.MODULE_CODE = CD.MODULE_ITEM_CODE AND CDU.MODULE_CODE = 13) 
        WHERE CD.MODULE_ITEM_CODE = 13 AND CD.MODULE_ITEM_KEY = AV_MODULE_ITEM_KEY
        )
        SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS 
        INNER JOIN AGREE_CUSTOM_DATA ACD ON ACD.CUST_ELEMENT = COLUMN_NAME
        WHERE TABLE_SCHEMA  = (SELECT DATABASE())
        AND TABLE_NAME = 'AGREEMENT_CUSTOM_DATA_RT'
        AND COLUMN_NAME <> 'AGREEMENT_REQUEST_ID'
        AND COLUMN_NAME <> 'UPDATE_TIMESTAMP'
        AND COLUMN_NAME <> 'AGREEMENT_CUSTOM_DATA_RT_ID'
        ORDER BY ORDINAL_POSITION;
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET NOT_FOUND = 1;
    
    SELECT count(1) into l_count FROM CUSTOM_DATA WHERE MODULE_ITEM_KEY = AV_MODULE_ITEM_KEY and MODULE_ITEM_CODE = 13;
    IF l_count = 0 THEN 
        SET SQL_SAFE_UPDATES = 0;
        DELETE FROM AGREEMENT_CUSTOM_DATA_RT WHERE AGREEMENT_REQUEST_ID = AV_MODULE_ITEM_KEY;
    ...
    END IF;
        ...
        OPEN SEL_COLUMN_NAME;
            ...
            COLUMN_NAME_LOOP: LOOP
                ...
                SELECT  t1.CUSTOM_DATA_ELEMENTS_ID, IS_MULTI_SELECT_LOOKUP, DATA_TYPE INTO LI_COST_ELEMENT_ID, LS_IS_MULTI_SELECT_LOOKUP, LS_DATATYPE FROM CUSTOM_DATA_ELEMENTS T1
                WHERE CONCAT('CUST_ELEMENTS_ID_',T1.CUSTOM_DATA_ELEMENTS_ID) = LS_COLUMN_NAME;
                
                IF (LI_COST_ELEMENT_ID IS NOT NULL) THEN
                ...
                IF LI_CNT = 1 THEN 
                    ...
                    FROM CUSTOM_DATA WHERE MODULE_ITEM_CODE = 13 AND CUSTOM_DATA_ELEMENTS_ID = ',LI_COST_ELEMENT_ID,' AND MODULE_ITEM_KEY =',AV_MODULE_ITEM_KEY,') AS T',LI_CNT, ' RIGHT JOIN ');
                    ELSE
                
                    SET LS_TAB_FORM = CONCAT(LS_TAB_FORM,' (SELECT MODULE_ITEM_KEY, ', LS_CUST_DATA,' AS c1 
                        FROM CUSTOM_DATA WHERE MODULE_ITEM_CODE = 13 AND CUSTOM_DATA_ELEMENTS_ID = ',LI_COST_ELEMENT_ID,' AND MODULE_ITEM_KEY =',AV_MODULE_ITEM_KEY,')
        ...
        SELECT COUNT(1) INTO LI_DEL FROM AGREEMENT_CUSTOM_DATA_RT WHERE AGREEMENT_REQUEST_ID = AV_MODULE_ITEM_KEY;
            IF LI_DEL >= 1  THEN 
                SET SQL_SAFE_UPDATES = 0;
                    DELETE FROM AGREEMENT_CUSTOM_DATA_RT WHERE AGREEMENT_REQUEST_ID = AV_MODULE_ITEM_KEY ;
                SET SQL_SAFE_UPDATES = 1;
    ...
    SET @sql = CONCAT('    
            INSERT INTO AGREEMENT_CUSTOM_DATA_RT (AGREEMENT_REQUEST_ID,',LS_INSERT,'UPDATE_TIMESTAMP) ' , LS_QUERY
    );
