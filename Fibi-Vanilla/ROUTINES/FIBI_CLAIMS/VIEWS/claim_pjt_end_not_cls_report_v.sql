-- claim_pjt_end_not_cls_report_v;

CREATE VIEW `claim_pjt_end_not_cls_report_v` AS 
select distinct
   ...
   `t1`.`TOTAL_CUM_CLAIM_AMOUNT_UPTO_CLAIM` AS `TOTAL_CUM_CLAIM_AMOUNT_UPTO_CLAIM` 
from
   (
      `award` `t0` 
      left join
      ...
      join
         (
            select distinct
               ...
               `s1`.`TOTAL_CUM_CLAIM_AMOUNT_UPTO_CLAIM` AS `TOTAL_CUM_CLAIM_AMOUNT_UPTO_CLAIM` 
            from
...
(`s1`.`CLAIM_STATUS_CODE` = 4) 
                  and 
                  (
                     `s1`.`INVOICE_NUMBER` is not null
                  )
                  and 
                  (
                     `s1`.`BANK_CLEARANCE_DATE` is not null
                  )
               )
         )
         `t1` 
