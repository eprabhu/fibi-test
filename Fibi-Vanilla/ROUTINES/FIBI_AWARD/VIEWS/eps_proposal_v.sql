...
VIEW `eps_proposal_v` AS
    SELECT 
        ...
        `t25`.`DERIVED_ROLE_NAME` AS `PROPOSAL_PERMISSION`
    FROM
        ((((((((((((((((((((((((`eps_proposal` `t1`
        ...
        LEFT JOIN `EPS_PROPOSAL_PERSON_ROLES` `t24` ON ((`t24`.`PROPOSAL_ID` = `t1`.`PROPOSAL_ID`)))
        LEFT JOIN `MODULE_DERIVED_ROLES` `t25` ON ((`t25`.`ROLE_ID` = `t24`.`ROLE_ID`)))
        LEFT JOIN (SELECT 
