-- `SAP_VIREMENT_DESCREPANCY`; 

CREATE PROCEDURE `SAP_VIREMENT_DESCREPANCY`(AV_ID INT)
BEGIN
...
DECLARE LS_AVAILABLE_FUND_TYPE			VARCHAR(10);
DECLARE LI_PREV_TOTAL_COST				DECIMAL(15,2);
DECLARE LI_CURRENT_TOTAL_COST			DECIMAL(15,2);
DECLARE LI_PREV_FEED_ID					INT;
DECLARE LI_PREV_AVAILABLE_COST			DECIMAL(15,2);
DECLARE LI_CUR_AVAILABLE_COST			DECIMAL(15,2);
		...
		BEGIN
			...
			FROM SAP_AWARD_FEED WHERE BATCH_ID >= 28
			order by FEED_ID;
			...
			END IF;
				...
				IF LI_FLAG > 0 THEN
					...
					SELECT TOTAL_COST,ANTICIPATED_TOTAL INTO LI_CURRENT_TOTAL_COST,LI_CUR_AVAILABLE_COST FROM AWARD_BUDGET_HEADER T1
					WHERE T1.AWARD_ID = LI_AWARD_ID
					AND T1.SEQUENCE_NUMBER = (SELECT MAX(T3.SEQUENCE_NUMBER) FROM AWARD_BUDGET_HEADER T3
											  WHERE T1.AWARD_ID = T3.AWARD_ID AND T3.AWARD_BUDGET_STATUS_CODE = 10
											 )
				...
				IF LI_FLAG > 0 THEN
					...
					SELECT AWARD_ID,FEED_ID INTO LI_PREV_AWARD_ID,LI_PREV_FEED_ID FROM SAP_AWARD_FEED T1
					WHERE T1.AWARD_NUMBER = LS_AWARD_NUMBER 
					AND T1.FEED_ID IN (SELECT MAX(T2.FEED_ID) FROM SAP_AWARD_FEED T2 
									WHERE  T1.AWARD_NUMBER = T2.AWARD_NUMBER 
									AND T2.FEED_ID < LI_FEED_ID
					...
					IF LI_FLAG > 0 THEN
						...
						SELECT TOTAL_COST,ANTICIPATED_TOTAL INTO LI_PREV_TOTAL_COST,LI_PREV_AVAILABLE_COST FROM AWARD_BUDGET_HEADER T1
						WHERE T1.AWARD_ID = LI_PREV_AWARD_ID
						AND T1.SEQUENCE_NUMBER = (SELECT MAX(T3.SEQUENCE_NUMBER) FROM AWARD_BUDGET_HEADER T3
												  WHERE T1.AWARD_ID = T3.AWARD_ID AND T3.AWARD_BUDGET_STATUS_CODE = 10
												 )
					...
					IF LI_WRITE_FLAG = 1 THEN
						...
						IF LI_FLAG = 0  THEN
							...
							IF LI_FLAG > 0 THEN
								...
								IF IFNULL(LI_PREV_TOTAL_COST,0) <> IFNULL(LI_CURRENT_TOTAL_COST,0) THEN
							
									INSERT INTO SAP_VIREMENT_DESCREPANCY(FEED_ID, BATCH_ID, AWARD_ID, AWARD_NUMBER, ACCOUNT_NUMBER, SEQUENCE_NUMBER, BUDGET_HEADER_ID, INTERNAL_ORDER_CODE,CURRENT_LINE_ITEM_COST, PREVIOUS_LINE_ITEM_COST, AMOUNT,FUND_CODE,PREV_TOTAL_COST,CURRENT_TOTAL_COST,PREV_AVAILABLE_COST,CUR_AVAILABLE_COST)
									VALUES(LI_FEED_ID,LS_BATCH_ID,LI_AWARD_ID,LS_AWARD_NUMBER,LS_ACCOUNT_NUMBER,LI_SEQUENCE_NUMBER,LI_BUDGET_HEADER_ID,LS_IO_CODE,LI_LINE_ITEM_COST,LI_PREV_LINE_ITEM_COST,LI_BUDGET_AMOUNT,LS_FUND_CODE,LI_PREV_TOTAL_COST,LI_CURRENT_TOTAL_COST,LI_PREV_AVAILABLE_COST,LI_CUR_AVAILABLE_COST);
