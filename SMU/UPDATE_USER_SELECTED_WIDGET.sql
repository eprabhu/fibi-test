...
OPEN C1;
	...
	SELECT IFNULL(max(SELECTED_WIDGET_ID),0)+1 INTO ID FROM USER_SELECTED_WIDGET;
	INSERT INTO `USER_SELECTED_WIDGET` (`SELECTED_WIDGET_ID`, `WIDGET_ID`, `SORT_ORDER`, `PERSON_ID`, `UPDATE_TIMESTAMP`, `UPDATE_USER`) VALUES (ID, AV_WIDGET_ID, LI_SORT_ORDER, LI_PERSON_ID, NOW(), 'quickstart');
	-- UPDATE USER_SELECTED_WIDGET_ID_GENERATOR SET NEXT_VAL = ID+1;
    END IF;
END;
END LOOP;
SELECT max(SELECTED_WIDGET_ID)+1 FROM USER_SELECTED_WIDGET;
	UPDATE USER_SELECTED_WIDGET_ID_GENERATOR SET NEXT_VAL = (SELECT max(SELECTED_WIDGET_ID)+1 FROM USER_SELECTED_WIDGET);
CLOSE C1;
END
$$
DELIMITER ;
