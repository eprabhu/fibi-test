...
DECLARE C1 CURSOR FOR
	SELECT MODULE_ITEM_KEY FROM SR_HEADER WHERE CREATE_TIMESTAMP <'<Specify the timestamp to update the datas>' AND MODULE_CODE = 1 AND MODULE_ITEM_KEY IS NOT NULL AND ORIGINATING_AWARD IS NULL;
OPEN C1;
	LOOP1: LOOP BEGIN
	...
	UPDATE SR_HEADER SET ORIGINATING_AWARD = LI_AWARD_ID WHERE MODULE_ITEM_KEY = LS_MODULE_ITEM_KEY;
IF DONE THEN
	LEAVE LOOP1;
	END IF;
END;
