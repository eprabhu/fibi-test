DELIMITER $$
CREATE  PROCEDURE `GET_PROPOSAL_DASHBOARD_back`(
...
IF AV_TYPE = 'A' THEN
			...
			END IF;
			IF AV_GRANTCALL_NAME IS NOT NULL  AND AV_GRANTCALL_NAME <> '' THEN
			 SET LS_FILTER_CONDITION = CONCAT(LS_FILTER_CONDITION,' T.NAME LIKE ''%',AV_GRANTCALL_NAME,'%'' AND ');
			END IF;
...
END IF;
			SET LS_DYN_SQL =CONCAT('SELECT DISTINCT *  FROM(SELECT DISTINCT  T1.PROPOSAL_ID,
                                            ...
                                            COALESCE(T8.PERSON_ID,T8.ROLODEX_ID) AS PI_PERSON_ID,
