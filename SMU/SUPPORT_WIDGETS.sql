...
END IF;
	SET ls_dyn_sql =  CONCAT(' SELECT
							          ...
							          WHERE ( T1.PRE_REVIEW_ID NOT IN (SELECT PRE_REVIEW_ID FROM pre_review_comment)AND  T1.PRE_REVIEW_TYPE_CODE = 2 AND (((T3.HOME_UNIT_NUMBER IN(SELECT DISTINCT UNIT_NUMBER FROM PERSON_ROLE_RT WHERE RIGHT_NAME IN (''SUPPORT_ADMINISTRATION'')
																				   AND (PERSON_ID =''',AV_PERSON_ID,'''))) OR T4.UNIT_NUMBER IN(SELECT DISTINCT UNIT_NUMBER FROM fibi_base.PERSON_ROLE_RT WHERE RIGHT_NAME IN (''SUPPORT_ADMINISTRATION'')
																				   AND (PERSON_ID =''',AV_PERSON_ID,''')))or (T1.REQUESTOR_PERSON_ID =''',AV_PERSON_ID,''') OR (T1.MODULE_ITEM_KEY IN
                                          (select T1.PROPOSAL_ID from EPS_PROPOSAL_PERSON_ROLES T1
										   INNER JOIN fibi_base.ROLE_RIGHTS T2 ON T1.ROLE_ID = T2.ROLE_ID
										   INNER JOIN fibi_base.RIGHTS T3 ON T2.RIGHT_ID = T3.RIGHT_ID
