...
IF AV_TAB_TYPE = 'MY_IOI' THEN
          SET TAB_QUERY = CONCAT(' (T1.PI_PERSON_ID =''', AV_LOGPERSON_ID,'''  AND T1.STATUS_CODE = 2)');
ELSEIF AV_TAB_TYPE = 'ALL_IOI' THEN
          SET TAB_QUERY = CONCAT('( (T1.SUBMITING_UNIT_NUMBER IN (SELECT DISTINCT UNIT_NUMBER FROM PERSON_ROLE_RT WHERE RIGHT_NAME = ''VIEW_IOI'' AND PERSON_ID =  ''', AV_LOGPERSON_ID,''')
                                                                OR T15.HOME_UNIT_NUMBER  IN (SELECT DISTINCT UNIT_NUMBER FROM PERSON_ROLE_RT WHERE RIGHT_NAME = ''VIEW_IOI'' AND PERSON_ID =  ''', AV_LOGPERSON_ID,''')) AND T1.STATUS_CODE = 2)');
...
END IF;
SET LS_DYN_SQL =CONCAT('SELECT DISTINCT T10.UNIT_NAME AS SUBMITING_UNIT_NAME,T4.FULL_NAME AS PI_FULL_NAME,T1.PROJECT_TITLE,
                                                                T1.REQUESTED_DIRECT_COST,T5.FULL_NAME AS UPDATE_USER
                                ,COUNT(T2.IOI_MEMBER_ID) AS PERSON_COUNT,T1.IOI_ID,T3.STATUS_CODE,T16.FULL_NAME AS CREATE_USER,
                                                                T1.UNIT_NUMBER,
                                                                T11.UNIT_NAME
