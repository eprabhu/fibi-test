...
DECLARE LI_INVOICE_AMOUNT DECIMAL(12,2);
	SELECT
		(CASE
			...
			(CASE WHEN ((SELECT COUNT(1)
				...
				WHERE T3.AWARD_ID = T1.AWARD_ID AND t6.OVERRIDE_NEGATIVE_AMOUNT = 'Y') > 0)
				 THEN  (
						...
						(IFNULL(SUM(t.AMOUNT_FORCASTED),0)*(t.OVERHEAD_PERCENTAGE/100))-((select IDC_CUM_CLAIM_AMT_UPTO_CLAIM from claim where claim_id = T1.CLAIM_ID) -((IFNULL(SUM(t.CUM_EXPENSE_UPTO_PREV_CLAIM),0)+
	...
	SELECT SUM(t1.CLAIM_AMOUNT) INTO LI_INVOICE_AMOUNT
