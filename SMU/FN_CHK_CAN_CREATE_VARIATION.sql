...
IF AV_AWARD_VARIATION_TYPE_CODE NOT IN ('1','3','7','17','15','14','8','11','10','9','13','2','5','21','22','18') THEN
	...
	IF LI_FLAG > 0 THEN
	RETURN 'A Variation Request for this project cannot be created as the project is awaiting for SAP response';
	END IF;
END IF;
SELECT COUNT(*) INTO LI_COUNT from AWARD WHERE STATUS_CODE = 15 AND AWARD_ID = AV_AWARD_ID;
IF LI_COUNT > 0 THEN
	IF AV_AWARD_VARIATION_TYPE_CODE NOT IN (6,7,8) THEN
	RETURN 'This Variation Request cannot be created as the project has expired. Please contact your unit administrator for assistance.';
	END IF;
END IF;
SELECT COUNT(*) INTO LI_COUNT from AWARD WHERE STATUS_CODE = 5 AND AWARD_ID = AV_AWARD_ID;
IF LI_COUNT > 0 THEN
	IF AV_AWARD_VARIATION_TYPE_CODE NOT IN (6,8) THEN
	RETURN 'This Variation Request cannot be created as the project has closed. Please contact your unit administrator for assistance.';
	END IF;
END IF;
...
ELSE
	...
	ELSE
		...
		IF LI_COUNT > 0 THEN
			RETURN 'A Variation Request for this project cannot be created as the budget is awaiting for SAP response';
		ELSE
			...
			IF LI_COUNT > 0 THEN
				...
				SELECT MAX(FEED_ID) FROM SAP_AWARD_FEED WHERE AWARD_NUMBER = LS_AWARD_NUMBER AND FEED_STATUS <> 'N')
				AND USER_ACTION_CODE = 2 AND AWARD_NUMBER = LS_AWARD_NUMBER;
				IF LI_FEED_COUNT > 0 THEN
					RETURN 'TRUE';
				ELSE
					RETURN 'A Variation Request for this project cannot be created as the latest budget is "Error in Posting". Please contact your unit administrator for assistance.';
				END IF;
			ELSE
				RETURN 'TRUE';
			END IF;
