DELIMITER $$
CREATE  FUNCTION `FN_CHCK_REMAINING_FUND`(AV_AWARD_ID VARCHAR(22)) RETURNS varchar(8) CHARSET utf8mb4
    DETERMINISTIC
BEGIN
...
IF ( (SIGN(LI_OBLI_DISTRIBUTABLE_AMOUNT - LI_TOTAL_COST)  <= 0) ) THEN
RETURN 'TRUE';
ELSE
RETURN 'FALSE';
END IF;
...
END IF;
IF ( (SIGN((LI_ANT_DISTRIBUTABLE_AMOUNT + LI_COMMITMENT_AMOUNT) - LI_TOTAL_COST) <= 0) ) THEN
RETURN 'TRUE';
ELSE
RETURN 'FALSE';
END IF;
