...
DECLARE LI_PERSON_ROLES_ID  INT;
        IF AV_TYPE IN ('ASSIGN_ROLE','UPDATE_DESCEND_FLAG') THEN
                ...
                IF LI_FLAG > 0 THEN
                        SELECT DESCEND_FLAG INTO LS_DESCEND_FLAG
                        FROM PERSON_ROLES
                        WHERE PERSON_ID = AV_PERSON_ID
                        AND UNIT_NUMBER = AV_UNIT_NUMBER
                        AND ROLE_ID = AV_ROLE_ID;
                        IF LS_DESCEND_FLAG = 'N' THEN
                                DELETE FROM PERSON_ROLE_RT
                                WHERE PERSON_ID = AV_PERSON_ID
                                AND UNIT_NUMBER = AV_UNIT_NUMBER
                                AND ROLE_ID = AV_ROLE_ID;
                        ELSE
                                ...
                                WHERE PERSON_ID = AV_PERSON_ID
                                AND ROLE_ID = AV_ROLE_ID
                                AND UNIT_NUMBER IN (SELECT CHILD_UNIT_NUMBER FROM UNIT_WITH_CHILDREN
                ...
                IF LI_FLAG = 0 THEN
					...
					VALUES(LI_PERSON_ROLES_ID,AV_PERSON_ID,AV_ROLE_ID,AV_UNIT_NUMBER,AV_DESCEND_FLAG,NOW(),AV_UPDATE_USER);
				ELSE
					UPDATE PERSON_ROLES SET DESCEND_FLAG = AV_DESCEND_FLAG, UPDATE_TIMESTAMP = NOW(), UPDATE_USER = AV_UPDATE_USER WHERE PERSON_ID = AV_PERSON_ID
					AND UNIT_NUMBER = AV_UNIT_NUMBER AND ROLE_ID = AV_ROLE_ID;
				END IF;
                IF AV_DESCEND_FLAG = 'N' THEN
                        INSERT INTO PERSON_ROLE_RT(PERSON_ID, UNIT_NUMBER, RIGHT_NAME, ROLE_ID)
                        SELECT DISTINCT AV_PERSON_ID,AV_UNIT_NUMBER,T3.RIGHT_NAME,AV_ROLE_ID
                        FROM ROLE T1
                        INNER JOIN ROLE_RIGHTS T2 ON T1.ROLE_ID = T2.ROLE_ID
                        INNER JOIN RIGHTS T3 ON T2.RIGHT_ID = T3.RIGHT_ID
                        WHERE T1.ROLE_ID = AV_ROLE_ID;
                ELSEIF AV_DESCEND_FLAG = 'Y' THEN
                        INSERT INTO PERSON_ROLE_RT(PERSON_ID, UNIT_NUMBER, RIGHT_NAME, ROLE_ID)
                        SELECT DISTINCT AV_PERSON_ID,T4.CHILD_UNIT_NUMBER,T3.RIGHT_NAME,AV_ROLE_ID FROM ROLE T1
                        INNER JOIN ROLE_RIGHTS T2 ON T1.ROLE_ID = T2.ROLE_ID
                        INNER JOIN RIGHTS T3 ON T2.RIGHT_ID = T3.RIGHT_ID
                        INNER JOIN UNIT_WITH_CHILDREN T4 ON T4.UNIT_NUMBER = AV_UNIT_NUMBER
                        WHERE T1.ROLE_ID = AV_ROLE_ID;
                END IF;
        ...
        ELSEIF AV_TYPE = 'ASSIGN_RIGHT' THEN
                ...
                IF LI_FLAG = 0 THEN
                        ...
                        INTO LS_RIGHT_NAME
                        FROM RIGHTS
                        WHERE RIGHT_ID = AV_RIGHT_ID;
                        INSERT INTO PERSON_ROLE_RT(PERSON_ID, UNIT_NUMBER, RIGHT_NAME, ROLE_ID)
                        SELECT DISTINCT T1.PERSON_ID,T1.UNIT_NUMBER,LS_RIGHT_NAME,T1.ROLE_ID
                        FROM PERSON_ROLES T1
                        WHERE T1.ROLE_ID = AV_ROLE_ID
                        AND T1.DESCEND_FLAG = 'N';
                        INSERT INTO PERSON_ROLE_RT(PERSON_ID, UNIT_NUMBER, RIGHT_NAME, ROLE_ID)
                        SELECT DISTINCT T1.PERSON_ID,T2.CHILD_UNIT_NUMBER,LS_RIGHT_NAME,T1.ROLE_ID
                        FROM PERSON_ROLES T1
