...
IF AV_TAB_TYPE = 'MY_PROPOSAL' THEN
				...
				OR T14.PERSON_ID = ''',AV_PERSON_ID,''' OR T3.PERSON_ID = ''',AV_PERSON_ID,''') ' );
...
ELSEIF AV_TAB_TYPE = 'ALL_PROPOSALS' THEN
				  ...
				  OR (T1.HOME_UNIT_NUMBER IN (SELECT DISTINCT UNIT_NUMBER FROM PERSON_ROLE_RT WHERE RIGHT_NAME IN (''MODIFY_PROPOSAL'',''VIEW_PROPOSAL'',''VIEW_ANY_PROPOSAL'') AND PERSON_ID = ''',AV_PERSON_ID,''') AND (T1.GRANT_HEADER_ID IS NULL))
				  OR (T1.HOME_UNIT_NUMBER IN (SELECT DISTINCT UNIT_NUMBER FROM PERSON_ROLE_RT WHERE RIGHT_NAME IN (''MODIFY_PROPOSAL'',''VIEW_PROPOSAL'',''VIEW_ANY_PROPOSAL'') AND PERSON_ID = ''',AV_PERSON_ID,''') AND (T1.GRANT_HEADER_ID IS NOT NULL AND (T6.HOME_UNIT_NUMBER IS NULL OR T6.HOME_UNIT_NUMBER <> ''000001'')))
				  OR T6.HOME_UNIT_NUMBER IN(SELECT DISTINCT UNIT_NUMBER FROM PERSON_ROLE_RT WHERE RIGHT_NAME IN(''MODIFY_PROPOSAL'',''VIEW_PROPOSAL'',''VIEW_ANY_PROPOSAL'')
...
END IF;
                        SET LS_DYN_SQL =CONCAT('SELECT DISTINCT count(*)  FROM(SELECT DISTINCT  T1.PROPOSAL_ID,
                                                                                        T5.DESCRIPTION AS  PROPOSAL_STATUS,
                                                                                        T1.TITLE,
                                                                                        T16.UNIT_NAME AS HOME_UNIT_NAME,
                                                                                        T31.SPONSOR_CODE,
                                                                                        T31.SPONSOR_NAME,
                                                                                        T31.ACRONYM,
                                            T1.GRANT_HEADER_ID,
                                            ...
                                            T30.DESCRIPTION AS ACTIVITY_TYPE ,
                                                                                        T1.SUBMIT_USER ,
                                            T1.SUBMISSION_DATE ,
                                            T25.GRANT_CALL_CATEGORY_CODE,T6.ABBREVIATION ,T1.INTERNAL_DEADLINE_DATE ',SELECTED_FIELD_LIST,
