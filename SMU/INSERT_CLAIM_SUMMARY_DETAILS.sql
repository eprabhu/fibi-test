...
DECLARE LI_AMOUNT_REQUESTED 		DECIMAL(12,2);
DECLARE LI_LATEST_APPROVED_AMOUNT 	DECIMAL(12,2);
DECLARE LI_ORIGINAL_APPROVED_AMOUNT DECIMAL(12,2);
...
DECLARE LI_PREV_CLAIM_TOTAL_AMOUNT	DECIMAL(12,2);
...
DECLARE LI_CUM_CLAIM_UPTO_CLAIM DECIMAL(12,2);
DECLARE LI_CUM_EXP_PREV_PERIOD DECIMAL(12,2);
DECLARE LI_EXP_IN_CLAIM_PERIOD DECIMAL(12,2);
DECLARE LI_TOTAL_EXP_PREV_QUATER DECIMAL(12,2);
DECLARE LI_FUND_BALANCE DECIMAL(12,2);
DECLARE LI_COMMIT_UPTO_CLAIM DECIMAL(12,2);
DECLARE LI_EXP_FORECAST DECIMAL(12,2);
DECLARE LI_AMOUNT_REQ DECIMAL(12,2);
...
DECLARE LI_LATEST_BUD_AMNT DECIMAL(12,2);
DECLARE LI_PREV_CLAIM_AMT DECIMAL(12,2);
DECLARE LD_BANK_CLEARING_DATE DATETIME;
DECLARE LD_DOCUMENT_DATE DATETIME;
DECLARE LI_IDC_LATEST_AMOUNT DECIMAL(12,2);
DECLARE LI_IDC_ORGINAL_AMOUNT DECIMAL(12,2);
	...
	BEGIN
			...
			END IF;
				...
				IF LI_FLAG > 0 THEN
					SELECT (SUM(IFNULL(c2.AMOUNT_REQUESTED,0.00)+ ifnull(if(c1.create_user = 'quickstart',c2.PREV_CLAIM_TOTAL_AMOUNT,0.00),0.00)))
					...
					AND C2.BUDGET_CATEGORY_CODE = LS_BUDGET_CATEGORY_CODE;
				END IF;
				SELECT IFNULL(MAX(NEXT_VAL),0)+1 INTO LI_CLAIM_SUMMARY_ID FROM CLAIM_SUMMARY_ID_GENERATOR;
				...
				LI_PREV_CLAIM_TOTAL_AMOUNT
                				...
                				UPDATE CLAIM_SUMMARY_ID_GENERATOR SET NEXT_VAL = LI_CLAIM_SUMMARY_ID;
				SELECT COUNT(1) INTO LI_FLAG
					...
					BEGIN
						...
						IF LS_BUDGET_CATEGORY_CODE IN ('EOM') THEN
							...
							(
								...
								INNER JOIN AWARD_MANPOWER_RESOURCE T2 ON T2.AWARD_MANPOWER_ID = T1.AWARD_MANPOWER_ID
								INNER JOIN AWARD_MANPOWER_PAYROLL T3 ON T2.PERSON_ID = T3.EMPLOYEE_NUMBER   AND T3.INTERNAL_ORDER_CODE = T1.BUDGET_REFERENCE_NUMBER
								INNER JOIN AWARD_EXPENSE_TRANSACTIONS T5 ON T5.AWARD_EXPENSE_TRANS_ID = LI_AWARD_EXPENSE_TRANS_ID
								WHERE DATE_FORMAT(STR_TO_DATE(T3.PAYROLL_PERIOD,'%m%Y'),'%Y-%m') BETWEEN  substring(AV_STARTDATE,1,7) AND substring(AV_ENDDATE,1,7)
								AND ( (AV_STARTDATE BETWEEN (COALESCE(CHARGE_START_DATE, PLAN_START_DATE)) AND (COALESCE(CHARGE_END_DATE, PLAN_END_DATE)) )
								OR (AV_ENDDATE BETWEEN (COALESCE(CHARGE_START_DATE, PLAN_START_DATE)) AND (COALESCE(CHARGE_END_DATE, PLAN_END_DATE))))
								...
								AND  DATE_FORMAT(STR_TO_DATE(T3.PAYROLL_PERIOD,'%m%Y'),'%Y-%m') = substring(LI_FM_POSTING_DATE,1,7)
							...
							IF LI_FLAG > 0 THEN
								...
								IF DONE3 THEN
									LEAVE INSERT_LOOP3;
								END IF;
                                SELECT IFNULL(MAX(NEXT_VAL),0) INTO LI_CLAIM_SUMMARY_DTLS_ID FROM CLAIM_SUMARY_DTLS_ID_GENERATOR;
								...
								UPDATE CLAIM_SUMARY_DTLS_ID_GENERATOR SET NEXT_VAL = LI_CLAIM_SUMMARY_DTLS_ID+1;
						END LOOP;
