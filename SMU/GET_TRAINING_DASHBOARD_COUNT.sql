...
IF AV_PERSON_ID IS NOT NULL  AND AV_PERSON_ID <> '' THEN
 SET LS_FILTER_CONDITION = CONCAT(LS_FILTER_CONDITION,' T.PERSON_ID LIKE ''%',AV_PERSON_ID,'%'' AND ');
END IF;
...
END IF;
     SET LS_DYN_SQL =CONCAT('SELECT count(1)  FROM(SELECT (SELECT IF(T1.IS_NON_EMPLOYEE = ''Y'',
			...
			T2.DESCRIPTION, T1.UPDATE_TIMESTAMP,T1.PERSON_ID,T1.IS_NON_EMPLOYEE,T1.PERSON_TRAINING_ID,T1.FOLLOWUP_DATE,T1.TRAINING_CODE' ,SELECTED_FIELD_LIST,
                                    ' FROM PERSON_TRAINING T1
									INNER JOIN TRAINING T2 ON T1.TRAINING_CODE = T2.TRAINING_CODE
									INNER JOIN PERSON T3 ON T3.USER_NAME = T1.UPDATE_USER
