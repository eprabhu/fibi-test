...
DECLARE LI_OVERHEAD_PER DECIMAL(12,2);
	SELECT
	T1.CLAIM_ID,
	T1.CLAIM_NUMBER,
	T1.AWARD_ID,
		...
		ELSE
			(CASE WHEN ((SELECT COUNT(1)
				...
				WHERE T3.AWARD_ID = T1.AWARD_ID AND t6.OVERRIDE_NEGATIVE_AMOUNT = 'Y') > 0)
				 THEN  (
						select IFNULL(SUM(t.AMOUNT_REQ_CURRENT_CLAIM),0) +if( ((IFNULL(SUM(t.COMMITMENTS_UPTO_PREV_CLAIM),0)*(t.OVERHEAD_PERCENTAGE/100))+
						...
						(IFNULL(SUM(t.AMOUNT_FORCASTED),0)*(t.OVERHEAD_PERCENTAGE/100))-((select  ifnull(IDC_CUM_CLAIM_AMT_UPTO_CLAIM,0) from claim where claim_id = T1.CLAIM_ID) -((IFNULL(SUM(t.CUM_EXPENSE_UPTO_PREV_CLAIM),0)+
						IFNULL(SUM(t.AMOUNT_REQUESTED),0))*(t.OVERHEAD_PERCENTAGE/100)))))
                        from
