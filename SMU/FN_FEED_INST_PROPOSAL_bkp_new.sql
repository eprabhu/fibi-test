DELIMITER $$
CREATE  FUNCTION `FN_FEED_INST_PROPOSAL_bkp_new`(
...
SELECT MAX(ID) INTO LI_PROPOSAL_ID FROM SEQ_PROPOSAL_PROPOSAL_ID;
SELECT
	TOTAL_COST,
	TOTAL_DIRECT_COST,
	TOTAL_INDIRECT_COST
...
END;
	BEGIN
      ...
      SELECT T2.PROPOSAL_PERSON_ID,T3.UNIT_NUMBER,T1.PROP_PERSON_ROLE_ID
      ...
      WHERE T1.PROPOSAL_ID = AV_DEV_PROPOSAL_ID;
		...
		INSERT_LOOP: LOOP
			FETCH CUR_PROPOSAL_PERSON_UNITS INTO LI_PROPOSAL_PERSON_ID,LS_UNIT_NUMBER,LI_PROP_PERSON_ROLE_ID;
			IF DONE1 THEN
				LEAVE INSERT_LOOP;
			END IF;
			SELECT IFNULL(MAX(PROP_PERSON_UNIT_ID),0)+1 INTO LI_SEQ_PROPOSAL_ID FROM PROP_PERSON_UNITS;
			IF LI_PROP_PERSON_ROLE_ID = 1 THEN
				SET LS_PROP_PERSON_ROLE_ID = 'Y';
			ELSE
				SET LS_PROP_PERSON_ROLE_ID = 'N';
			END IF;
