...
BEGIN
	...
	BEGIN
		...
		VALUES  (AV_AWARD_ID,LI_AWD_REP_TERMS_ID,CONCAT(LS_ERROR),UTC_TIMESTAMP(),'quickstart');
		COMMIT;
		SET DONE1 = TRUE;
	END;
	SET LI_COUNT =0;
	...
	IF LI_COUNT = 0 THEN
		INSERT INTO AWD_REPREQ_DUEDT_LOG
			(AWARD_ID,AWARD_REPORT_TERMS_ID,ERROR_MESSAGE,UPDATE_TIMESTAMP,UPDATE_USER)
		VALUES  (LI_AWARD_ID,LI_AWD_REP_TERMS_ID,'No Record found / End date blank for input award id',UTC_TIMESTAMP(),'quickstart');
		SET DONE1 = TRUE;
	END IF;
	IF DONE1 = FALSE THEN
	   SELECT FINAL_EXPIRATION_DATE
	    INTO  LS_AWD_END_DATE
	     FROM AWARD
		WHERE AWARD_ID = AV_AWARD_ID;
