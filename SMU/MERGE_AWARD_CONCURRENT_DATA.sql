...
DECLARE LS_ROLE_EDIT VARCHAR(10);
	BEGIN
		...
		BEGIN
			...
			VALUES(AV_AWARD_ID,LS_AWARD_NUMBER,LI_SEQUENCE_NUMBER,LS_ERROR_MSG,'MERGE_AWARD_CONCURRENT_DATA',NULL,NULL,NULL,UTC_TIMESTAMP(),AV_UPDATE_USER);
			COMMIT;
		END;
        SET LI_FLAG = 0;
		SELECT
    AWARD_NUMBER, SEQUENCE_NUMBER, AWARD_VARIATION_TYPE_CODE
INTO LS_AWARD_NUMBER , LI_SEQUENCE_NUMBER , LS_AWARD_VARIATION_TYPE_CODE FROM
    AWARD
WHERE
...
WHERE
    AWARD_NUMBER = LS_AWARD_NUMBER
		...
		BEGIN
				...
				FROM MODULE_VARIABLE_SECTION
				WHERE MODULE_ITEM_KEY = AV_AWARD_ID
				AND TYPE_CODE = LS_AWARD_VARIATION_TYPE_CODE AND SECTION_CODE = 104;
				IF LI_FLAG = 0 THEN
						CALL MERGE_AWD_SECTION_UPDATE_ROLES(AV_AWARD_ID,
													LS_AWARD_NUMBER,
													LI_SEQUENCE_NUMBER,
				...
				ELSE
					CALL MERGE_AWD_SECTION_ROLES(AV_AWARD_ID,
													LS_AWARD_NUMBER,
													LI_SEQUENCE_NUMBER,
													LS_AWARD_VARIATION_TYPE_CODE,
													AV_UPDATE_USER
				...
				ELSEIF LS_SECTION_CODE in('131','132','133','134') THEN
				...
				ELSEIF LS_SECTION_CODE = '132' THEN
					CALL MERGE_AWD_SECTION_MANPOWER(AV_AWARD_ID,
													LS_AWARD_NUMBER,
													LI_SEQUENCE_NUMBER,
													LS_AWARD_VARIATION_TYPE_CODE,
