...
DECLARE C1 CURSOR FOR
	SELECT MODULE_ITEM_KEY, SR_HEADER_ID FROM SR_HEADER_TEST WHERE ORIGINATING_AWARD IS NULL;
OPEN C1;
	LOOP1: LOOP BEGIN
...
FETCH C1 INTO LS_AWARD_ID, LS_SR_HEADER_ID;
	...
	SELECT AWARD_NUMBER,SEQUENCE_NUMBER  INTO LS_AWARD_NUMBER,LS_SEQUENCE_NUMBER FROM AWARD WHERE AWARD_ID =  LS_AWARD_ID;
		...
		UPDATE SR_HEADER_TEST SET ORIGINATING_AWARD = LS_ORG_AWARD_ID where SR_HEADER_ID = LS_SR_HEADER_ID;
		end if;
        END IF;
    END IF;
IF DONE THEN
