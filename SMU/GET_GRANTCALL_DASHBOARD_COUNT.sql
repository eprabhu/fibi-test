...
IF AV_TYPE = 'A' THEN
           ...
           IF AV_SPONSOR IS NOT NULL AND AV_SPONSOR <> '' THEN
			...
			END IF;
END IF;
SELECT DISTINCT COUNT(*) INTO LS_COUNT
FROM PERSON_ROLE_RT
WHERE PERSON_ID = AV_PERSON_ID
AND RIGHT_NAME = 'CREATE_GRANT_CALL';
...
END IF;
SET TAB_QUERY =   CONCAT(TAB_QUERY,'AND (case when (T1.HOME_UNIT_NUMBER IN (SELECT DISTINCT UNIT_NUMBER FROM PERSON_ROLE_RT
															 WHERE RIGHT_NAME IN (''MODIFY_GRANT_CALL'',''VIEW_GRANT_CALL'',''PUBLISH_GRANT_CALL'') AND PERSON_ID = ''',AV_PERSON_ID,'''
															)
												or T1.CREATE_USER = (SELECT USER_NAME FROM PERSON WHERE PERSON_ID =  ''',AV_PERSON_ID,''' )
											)
									then
										T1.HOME_UNIT_NUMBER IN (SELECT DISTINCT UNIT_NUMBER FROM PERSON_ROLE_RT
															 WHERE RIGHT_NAME IN (''MODIFY_GRANT_CALL'',''VIEW_GRANT_CALL'',''PUBLISH_GRANT_CALL'') AND PERSON_ID = ''',AV_PERSON_ID,'''
															)
												or T1.CREATE_USER = (SELECT USER_NAME FROM PERSON WHERE PERSON_ID =  ''',AV_PERSON_ID,''' )
									else
...
END IF;
SET LS_DYN_SQL = CONCAT('SELECT DISTINCT COUNT(*) FROM (SELECT T1.GRANT_HEADER_ID,
								...
								',JOIN_CONDITION,'
                                WHERE ',TAB_QUERY,'
                                GROUP BY T1.GRANT_HEADER_ID )T  ',LS_FILTER_CONDITION,' ',AV_SORT_TYPE,' ',LS_OFFSET_CONDITION );
