...
DECLARE LS_RESRCH_TYPE_SUB_AREA_CODE	VARCHAR(3);
DECLARE LS_RESRCH_TYPE_AREA_CODE	VARCHAR(3);
...
SET LI_SEQUENCE_NUMBER = LI_SEQUENCE_NUMBER + 1;
	SELECT NEXT_VAL INTO LI_PROPOSAL_ID FROM SEQ_IP_ID_GNTR;
	UPDATE SEQ_IP_ID_GNTR SET NEXT_VAL = LI_PROPOSAL_ID+1;
SELECT
	TOTAL_COST,
	TOTAL_DIRECT_COST,
	TOTAL_INDIRECT_COST,
...
CLUSTER_CODE)
SELECT 	AV_UPDATE_USER,utc_timestamp(),T1.HOME_UNIT_NUMBER,T1.HOME_UNIT_NAME,LI_PROPOSAL_ID,AV_INST_PROPOSAL_NUMBER,LI_SEQUENCE_NUMBER,T1.TYPE_CODE,T1.TITLE,IFNULL(T1.SPONSOR_CODE,'300000'),
IFNULL(T1.ACTIVITY_TYPE_CODE,1),T1.START_DATE,T1.END_DATE,NULL,1,utc_timestamp(),AV_UPDATE_USER,T1.INTERNAL_DEADLINE_DATE,
...
WHERE T1.PROPOSAL_ID = AV_DEV_PROPOSAL_ID;
	SELECT NEXT_VAL INTO LI_PROPOSAL_ADMIN_DETAILS_ID FROM SEQ_PROP_ADMIN_DTLS_GNTR;
	UPDATE SEQ_PROP_ADMIN_DTLS_GNTR SET NEXT_VAL = LI_PROPOSAL_ADMIN_DETAILS_ID+1;
...
SELECT LI_PROPOSAL_ADMIN_DETAILS_ID,AV_DEV_PROPOSAL_ID,LI_PROPOSAL_ID,T1.SUBMISSION_DATE,utc_timestamp(),AV_UPDATE_USER,
...
WHERE T1.PROPOSAL_ID = AV_DEV_PROPOSAL_ID;
 BEGIN
	...
	EMAIL_ADDRESS,EPS_PROPOSAL_PERSONS.IS_MULTI_PI,EPS_PROPOSAL_PERSONS.PI_FLAG,EPS_PROPOSAL_PERSONS.DESIGNATION,EPS_PROPOSAL_PERSONS.DEPARTMENT
    ...
    INSERT_LOOP: LOOP
		FETCH CUR_PROPOSAL_PERSONS INTO LI_PROPOSAL_PERSON_ID,LS_PERSON_ID,LI_ROLODEX_ID,LS_FULL_NAME,LS_PROP_PERSON_ROLE_CODE,LI_PERCENTAGE_OF_EFFORT,LS_EMAIL_ADDRESS,ls_is_multi_pi,ls_pi_flag,LS_DESIGNATION,LS_DEPARTMENT;
        IF DONE  THEN
				LEAVE INSERT_LOOP;
		END IF;
		SELECT NEXT_VAL INTO LI_SEQ_PROPOSAL_ID FROM SEQ_IP_PERSON_ID_GNTR;
		UPDATE SEQ_IP_PERSON_ID_GNTR SET NEXT_VAL = LI_SEQ_PROPOSAL_ID+1;
		...
		BEGIN
			...
			INSERT_LOOP1: LOOP
				...
				IF DONE8 THEN
					LEAVE INSERT_LOOP1;
				END IF;
				SELECT NEXT_VAL INTO LI_SEQ_ATTACHMENT_ID FROM SEQ_IP_PERSONS_ATTACH_ID_GNTR;
				UPDATE SEQ_IP_PERSONS_ATTACH_ID_GNTR SET NEXT_VAL = LI_SEQ_ATTACHMENT_ID+1;
...
END;
      ...
      WHERE T1.PROPOSAL_ID = AV_DEV_PROPOSAL_ID;
		...
		INSERT_LOOP: LOOP
			...
			IF DONE1 THEN
				LEAVE INSERT_LOOP;
			END IF;
			SELECT NEXT_VAL INTO LI_SEQ_PROPOSAL_ID FROM SEQ_IP_PERSON_UNIT_ID_GNTR;
			UPDATE SEQ_IP_PERSON_UNIT_ID_GNTR SET NEXT_VAL = LI_SEQ_PROPOSAL_ID+1;
	...
	BEGIN
		...
		INSERT_LOOP: LOOP
			...
			IF DONE4 THEN
				LEAVE INSERT_LOOP;
			END IF;
			SELECT NEXT_VAL INTO LI_SEQ_PROP_SPL_REVIEW_ID FROM SEQ_IP_SPECIAL_REVIEW_ID_GNTR;
			UPDATE SEQ_IP_SPECIAL_REVIEW_ID_GNTR SET NEXT_VAL = LI_SEQ_PROP_SPL_REVIEW_ID+1;
	...
	BEGIN
		...
		INSERT_LOOP: LOOP
			...
			IF DONE5 THEN
				LEAVE INSERT_LOOP;
			END IF;
			SELECT NEXT_VAL INTO LI_SEQ_RESRCH_AREA_ID FROM SEQ_IP_RESRCH_AREA_ID_GNTR;
			UPDATE SEQ_IP_RESRCH_AREA_ID_GNTR SET NEXT_VAL = LI_SEQ_RESRCH_AREA_ID+1;
	...
	SELECT NEXT_VAL INTO LI_IP_BUDGET_HEADER_ID FROM SEQ_IP_BUD_HEAD_ID_GNTR;
	UPDATE SEQ_IP_BUD_HEAD_ID_GNTR SET NEXT_VAL = LI_IP_BUDGET_HEADER_ID+1;
	...
	BEGIN
				...
				INSERT_LOOP1: LOOP
					...
					IF DONE9 THEN
						LEAVE INSERT_LOOP1;
					END IF;
					SELECT NEXT_VAL INTO LI_BUDGET_PERIOD_ID FROM SEQ_IP_BUD_PERIOD_ID_GNTR;
					UPDATE SEQ_IP_BUD_PERIOD_ID_GNTR SET NEXT_VAL = LI_BUDGET_PERIOD_ID+1;
