...
LEAVE LOOP1;
END IF;
SELECT IFNULL(NEXT_VAL,0)+1 INTO LI_SEQ_ORCID_WEBHOOK_NOTIFY_LOG_ID FROM orcid_wbhk_ntfy_log_id_gnrtr;
IF LI_STATUS = 'A' THEN
...
VALUES(LI_SEQ_ORCID_WEBHOOK_NOTIFY_LOG_ID, LI_ORCID_ID,UTC_TIMESTAMP(), 'quickstart', 'C');
END IF;
INSERT INTO PERSON_ORCID_WEBHOOK_FLAG(PERSON_ID,IS_REGISTERED_FLAG, UPDATE_TIMESTAMP, UPDATE_USER)
VALUES(LI_PERSON_ID, 'Y',UTC_TIMESTAMP(), 'quickstart');
END IF;
...
IF LI_COUNT_TWO = 0 THEN
INSERT INTO ORCID_WEBHOOK_NOTIFY_LOG(ORCID_WEBHOOK_NOTIFY_LOG_ID,ORCID_ID, UPDATE_TIMESTAMP, UPDATE_USER, WEBHOOK_ACTION_TYPE_CODE)
VALUES(LI_SEQ_ORCID_WEBHOOK_NOTIFY_LOG_ID, LI_ORCID_ID,UTC_TIMESTAMP(), 'quickstart', 'D');
END IF;
UPDATE PERSON_ORCID_WEBHOOK_FLAG SET IS_REGISTERED_FLAG = 'N',UPDATE_TIMESTAMP = UTC_TIMESTAMP() WHERE PERSON_ID = LI_PERSON_ID;
END IF;
END IF;
UPDATE orcid_wbhk_ntfy_log_id_gnrtr SET NEXT_VAL = LI_SEQ_ORCID_WEBHOOK_NOTIFY_LOG_ID+1;
COMMIT;
END;
END LOOP;
CLOSE C1;
