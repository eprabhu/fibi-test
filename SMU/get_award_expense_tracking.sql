...
SELECT
	...
	NULL AS IO_CODE,
	NULL AS QUANTITY,
	0 AS ORIGINAL_APPROVED_BUDGET,
	0 AS LATEST_APPROVED_BUDGET,
	sum(T1.AMOUNT_IN_FMA_CURRENCY) as EXPENDITURE_TO_DATE,
	0 - sum(T1.AMOUNT_IN_FMA_CURRENCY) AS  BALANCE,
...
AND (T1.INTERNAL_ORDER_CODE IS NOT NULL AND T1.ACTUAL_OR_COMMITTED_FLAG = 'A'
AND  NOT EXISTS (SELECT 1 FROM(SELECT IFNULL(UPPER(TRIM(T3.INTERNAL_ORDER_CODE)),'0') AS IO_CODE
            ...
            UNION
             SELECT IFNULL(UPPER(TRIM(T7.IO_CODE)),'0') AS IO_CODE
            ...
            )T WHERE T.IO_CODE = UPPER(TRIM(T1.INTERNAL_ORDER_CODE))
	)
	OR (T1.INTERNAL_ORDER_CODE IS NULL AND T1.ACTUAL_OR_COMMITTED_FLAG = 'A')
	);
END IF;
END
$$
