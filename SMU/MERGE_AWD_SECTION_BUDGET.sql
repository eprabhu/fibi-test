...
DECLARE LS_AWARD_ACCOUNT_NUMBER	VARCHAR(100);
DECLARE LS_AWARD_FUND_CENTER		VARCHAR(255);
SET LS_COUNT =0;
			BEGIN
				...
				BEGIN
					...
					VALUES(AV_AWARD_ID,AV_AWARD_NUMBER,AV_SEQUENCE_NUMBER,LS_ERROR_MSG,'MERGE_AWD_SECTION_BUDGET',LS_TABLE_NAME,102,AV_VARIATION_TYPE_CODE,UTC_TIMESTAMP(),AV_UPDATE_USER);
					COMMIT;
				END;
				...
				SELECT ACCOUNT_NUMBER,FUND_CENTER
					INTO LS_AWARD_ACCOUNT_NUMBER,LS_AWARD_FUND_CENTER
					FROM AWARD
					WHERE AWARD_ID = AV_AWARD_ID;
				UPDATE AWARD SET ACCOUNT_NUMBER=LS_AWARD_ACCOUNT_NUMBER,FUND_CENTER=LS_AWARD_FUND_CENTER WHERE AWARD_NUMBER = AV_AWARD_NUMBER
				AND SEQUENCE_NUMBER = 0;
				...
				BEGIN
					...
					END IF;
					SELECT IFNULL(MAX(BUDGET_HEADER_ID),0) +1 INTO LI_AWD_BUDGT_HEADER_NEXT_VAL FROM AWARD_BUDGET_HEADER;
					...
					BEGIN
						...
						END IF;
						SELECT IFNULL(MAX(BUDGET_PERIOD_ID),0) +1 INTO LI_AWD_BUDGT_PERIOD_NEXT_VAL FROM AWARD_BUDGET_PERIOD;
					...
					BEGIN
						...
						END IF;
						SELECT IFNULL(MAX(BUDGET_DETAILS_ID),0) +1 INTO LI_AWD_BUDGT_DETL_ID_NEXT_VAL FROM AWARD_BUDGET_DETAIL;
