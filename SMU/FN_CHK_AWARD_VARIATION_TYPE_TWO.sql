DELIMITER $$
CREATE  FUNCTION `FN_CHK_AWARD_VARIATION_TYPE_TWO`(
...
FROM PERSON_ROLES WHERE ROLE_ID IN(100) AND PERSON_ID = LS_PERSON_ID;
IF LI_FLAG > 0 THEN
	RETURN 'FALSE';
END IF;
SELECT COUNT(*) INTO LI_FLAG
...
AND AWARD_VARIATION_TYPE_CODE NOT IN (8,9,11,12)
