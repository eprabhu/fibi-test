...
DECLARE LS_TITLE VARCHAR(600);
DECLARE LS_MODULE_ITEM_KEY VARCHAR(20);
DECLARE LS_MSG_TYPE varchar(3);
	...
	IF AV_APPROVAL_STATUS = 'W' THEN
		...
		IF LI_COUNT > 1 THEN
			...
			WHERE MODULE_ITEM_KEY = (SELECT AWARD_ID FROM AWARD
									 WHERE AWARD_NUMBER = LS_AWARD_NUMBER
									 AND AWARD_SEQUENCE_STATUS = 'ACTIVE')
			AND T1.ORIGINATING_AWARD = LS_MODULE_ITEM_ID
			AND MODULE_CODE = 1;
		ELSE
			SELECT T2.DESCRIPTION INTO LS_SERVICE_REQUEST_TYPE FROM SR_HEADER T1
			INNER JOIN SR_TYPE T2 ON T1.TYPE_CODE = T2.TYPE_CODE
			...
			AND MODULE_CODE = 1;
		END IF;
		IF LS_MODULE_CODE = 1 AND LI_SUB_MODULE_CODE = 0 AND LI_VARIATION_FLAG > 0 THEN
			SET LS_USER_MESSAGE = CONCAT(LS_SERVICE_REQUEST_TYPE,' for #',LS_MODULE_ITEM_KEY,' : ',LS_TITLE);
		ELSEIF LS_MODULE_CODE = 1 AND LI_SUB_MODULE_CODE = 0 AND LI_AWD_MODIFI_FLAG > 0 THEN
		...
		ELSEIF LS_MODULE_CODE = 1 AND LI_SUB_MODULE_CODE = 2 THEN
			...
			END IF;
		ELSEIF LS_MODULE_CODE = 16 THEN
			SELECT REPORT_CLASS_CODE INTO LS_PROGRESS_REPORT_TYPE FROM award_progress_report WHERE PROGRESS_REPORT_ID =  LS_MODULE_ITEM_ID;
			IF LS_PROGRESS_REPORT_TYPE = '2' THEN
				SET LS_USER_MESSAGE = CONCAT('Final Report #',LS_PROGRESS_REPORT_NUMBER);
			ELSE
				SET LS_USER_MESSAGE = CONCAT('Progress Report #',LS_PROGRESS_REPORT_NUMBER);
			END IF;
			ELSE
