...
EXIT_LABEL : BEGIN
   ...
   SET LS_DATE = utc_timestamp();
	...
	ELSEIF AV_MODULE_CODE = 1 AND AV_SUBMODULE_CODE = 2 THEN
		...
		END IF;
	ELSEIF AV_MODULE_CODE = 16 THEN
		select REPORT_CLASS_CODE, PROGRESS_REPORT_NUMBER into LS_PROGRESS_REPORT_TYPE, LS_PROGRESS_REPORT_NUMBER from award_progress_report where PROGRESS_REPORT_ID =  AV_MODULE_ITEM_ID;
...
IF AV_ACTION_TYPE = 'R' THEN
  ...
  SELECT COUNT(*) INTO INBOX_ACTION_COUNT
    ...
    WHERE WORKFLOW_ID = LL_WORKFLOW_ID
      AND IS_WORKFLOW_ACTIVE = 'Y';
  IF AV_MODULE_CODE IN (1,3,5,14,16) AND AV_SUBMODULE_CODE <> 2 THEN
  SELECT PERSON_ID
    INTO LS_PERSON_ID
    FROM PERSON
