...
IF AV_TAB_TYPE = 'ALL_PROPOSALS' THEN
		  SET TAB_QUERY = CONCAT(' ((T14.PERSON_ID = ''',AV_PERSON_ID,'''
		  OR T1.HOME_UNIT_NUMBER IN (SELECT DISTINCT UNIT_NUMBER FROM PERSON_ROLE_RT WHERE RIGHT_NAME IN (''MODIFY_PROPOSAL'',''VIEW_PROPOSAL'',''VIEW_ANY_PROPOSAL'',''CREATE_AWARD'',''MODIFY_INST_PROPOSAL'',''VIEW_INST_PROPOSAL'') AND PERSON_ID = ''',AV_PERSON_ID,''') ))' );
...
END IF;
			SET LS_DYN_SQL =CONCAT('SELECT DISTINCT COUNT(*)  FROM(SELECT DISTINCT  T1.PROPOSAL_ID,
											...
											LEFT JOIN SPONSOR T31 ON T31.SPONSOR_CODE = T1.SPONSOR_CODE ',JOIN_CONDITION,' WHERE T1.PROPOSAL_SEQUENCE_STATUS IN(''ACTIVE'',''PENDING'') AND ',TAB_QUERY,
