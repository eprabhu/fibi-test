...
SELECT T1.RULE_ID,T1.RULE_EXPRESSION,T1.MAP_ID FROM BUSINESS_RULES T1
                       WHERE T1.MODULE_CODE = AV_MODULE_CODE
                       AND T1.SUB_MODULE_CODE = AV_SUBMODULE_CODE
                       AND T1.RULE_TYPE = 'R' AND T1.IS_ACTIVE = 'Y'
					   AND T1.UNIT_NUMBER = LS_PARENT_UNIT_NUMBER
                       ORDER BY T1.RULE_EVALUATION_ORDER;
...
ELSE
	...
	ELSEIF AV_MODULE_CODE = 1 AND AV_SUBMODULE_CODE = 2 THEN
		...
		END IF;
	ELSEIF AV_MODULE_CODE = 16 THEN
    select REPORT_CLASS_CODE into LS_PROGRESS_REPORT_TYPE from award_progress_report where PROGRESS_REPORT_ID =  AV_MODULE_ITEM_KEY;
