													...
													ELSE
																SELECT
																		...
																		WHERE T4.AWARD_ID = T6.AWARD_ID
																	 	...
																	 	AND T1.AWARD_EXPENSE_TRANS_ID  IN (SELECT AWARD_EXPENSE_TRANS_ID
																									    FROM AWARD_EXPENSE_TRANSACTIONS WHERE (DOCUMENT_NUMBER,ITEM_NUMBER,REFERENCE_DOCUMENT_CATEGORY,REFERENCE_ORG_UNIT,ACCT_ASSIGNMENT_NUMBER,SCHEDULE_LINE_NUMBER,
																																			CONDITION_COUNTER,REFERENCE_PROCEDURE,DOCUMENT_NUMBER_FM_LINE_ITEM,TRANSACTION_NUMBER,INTERNAL_ORDER_CODE) IN
																																		(SELECT DOCUMENT_NUMBER,ITEM_NUMBER,REFERENCE_DOCUMENT_CATEGORY,REFERENCE_ORG_UNIT,ACCT_ASSIGNMENT_NUMBER,SCHEDULE_LINE_NUMBER,
																																		CONDITION_COUNTER,REFERENCE_PROCEDURE,DOCUMENT_NUMBER_FM_LINE_ITEM,TRANSACTION_NUMBER,INTERNAL_ORDER_CODE
																																		FROM expense_zero_exclude_commit_v))
																	 ORDER BY T1.FM_POSTING_DATE DESC;
													END IF;
						ELSE
