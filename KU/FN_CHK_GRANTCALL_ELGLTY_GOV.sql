DELIMITER $$
CREATE DEFINER=`ku_rms_user`@`%` FUNCTION `FN_CHK_GRANTCALL_ELGLTY_GOV`(
...
DECLARE LS_PRIMARY_TITLE_DESC VARCHAR(200);
	 BEGIN
		...
		WHERE t2.GRANT_HEADER_ID = AV_MODULE_ITEM_KEY AND t2.PROP_PERSON_ROLE_ID = 12;
        ...
        BEGIN
			...
			END IF;
                ...
                AND PROP_PERSON_ROLE_ID = 12) AND ELIGIBILITY_TARGET_TYPE_CODE = 3)) THEN
					SET LI_TARGET_FLAG = 1;
				END IF;
