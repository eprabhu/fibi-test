DELIMITER $$
CREATE DEFINER=`ku_rms_user`@`%` PROCEDURE `AWARD_MASTER_DATASET_SYNC`()
BEGIN
...
DECLARE LS_GRANT_TYPE_CODE VARCHAR(3);
DECLARE LS_SUBMIT_USER	varchar(200);
DECLARE LS_SUBMISSION_DATE DATETIME;
DECLARE LS_VERTICAL_AREA LONGTEXT;
DECLARE LS_HORIZONTAL_AREA LONGTEXT;
...
LEFT OUTER JOIN (SELECT R1.AWARD_ID,
    CONCAT(
        GROUP_CONCAT(
            CONCAT(
            '{' ,
                ...
                '| ',
                R2.DESCRIPTION,
                IFNULL(CONCAT('| ', R3.DESCRIPTION), ''),
                '}'
            )
        )
    ) AS VERTICAL_AREA FROM AWARD_RESEARCH_AREAS R1
LEFT JOIN RESEARCH_TYPE_AREA R2
    ON R2.RESRCH_TYPE_AREA_CODE = R1.RESRCH_TYPE_AREA_CODE
    AND R2.RESRCH_TYPE_CODE = 4
LEFT JOIN RESEARCH_TYPE_SUB_AREA R3
    ON R1.RESRCH_TYPE_SUB_AREA_CODE = R3.RESRCH_TYPE_SUB_AREA_CODE
GROUP BY R1.AWARD_ID
)T50 ON T1.AWARD_ID = T50.AWARD_ID
LEFT OUTER JOIN (SELECT R1.AWARD_ID,
    CONCAT(
        GROUP_CONCAT(
            CONCAT(
...
LEFT JOIN RESEARCH_TYPE_SUB_AREA R3
    ON R1.RESRCH_TYPE_SUB_AREA_CODE = R3.RESRCH_TYPE_SUB_AREA_CODE
GROUP BY R1.AWARD_ID
)T51 ON T1.AWARD_ID = T51.AWARD_ID
LEFT OUTER JOIN PERSON T49 ON T49.USER_NAME = T1.SUBMIT_USER
