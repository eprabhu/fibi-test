DELIMITER $$
CREATE DEFINER=`ku_rms_user`@`%` PROCEDURE `GET_RESEARCH_SUMMARY_DETAILS`(
...
ELSEIF AV_WIDGET = 'SUBMISSION_BASED_CLAIM_AGE' THEN
	...
	WITH ACCESS_TMP
			...
			WHERE T1.PERSON_ID = AV_PERSON_ID AND T3.RIGHT_NAME = 'VIEW_CLAIMS')
		SELECT * FROM (
	(SELECT 'Sponsor Name','Sponsor Code', CONCAT('Ã¢â€°Â¤30',COL3, ' Days'), CONCAT('31 - 60 Days',COL3) , CONCAT('61 - 90 Days',COL3) , CONCAT('91 - 120 Days',COL3) , CONCAT('121 - 180 Days',COL3) ,CONCAT('>181 Days',COL3) ,CONCAT('Total',COL3))
	UNION
...
WITH ACCESS_TMP
	...
	SELECT DISTINCT
	CASE WHEN DATEDIFF(UTC_TIMESTAMP(),FINAL_EXPIRATION_DATE) <=90 THEN 'Project ended Ã¢â€°Â¤ 90 days ago'
