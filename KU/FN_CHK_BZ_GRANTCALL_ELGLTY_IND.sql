DELIMITER $$
CREATE DEFINER=`ku_rms_user`@`%` FUNCTION `FN_CHK_BZ_GRANTCALL_ELGLTY_IND`(
...
DECLARE LS_PRIMARY_TITLE_DESC VARCHAR(200);
	 BEGIN
		...
		WHERE t1.GRANT_HEADER_ID = AV_MODULE_ITEM_KEY and t1.PROPOSAL_ID = AV_PROPOSAL_ID and t3.PROP_PERSON_ROLE_ID = 11;
		IF LI_FLAG > 0 THEN
        BEGIN
