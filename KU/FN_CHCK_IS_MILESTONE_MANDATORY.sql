DELIMITER $$
CREATE FUNCTION `FN_CHCK_IS_MILESTONE_MANDATORY`(AV_PROPOSAL_ID INT(22)) RETURNS varchar(8) CHARSET utf8mb4
    DETERMINISTIC
BEGIN
    DECLARE LI_GRANT_CALL_ID INT(22);
    DECLARE LI_CUSTOMDATA_COUNT INT(22);
    DECLARE LI_MILESTONE_COUNT INT(22);
    ...
    IF LI_GRANT_CALL_ID IS NOT NULL THEN
		SELECT COUNT(*) INTO LI_CUSTOMDATA_COUNT
								...
								AND T1.VALUE = 'YES'
                                AND T1.COLUMN_ID = (SELECT MAX(COLUMN_ID) FROM CUSTOM_DATA_ELEMENTS WHERE CUSTOM_ELEMENT_NAME = 'Proposal Milestones Mandatory Check')
       ...
       SELECT COUNT(*) INTO LI_MILESTONE_COUNT
        FROM EPS_PROPOSAL_MILESTONE
        WHERE proposal_id = AV_PROPOSAL_ID;
        IF LI_CUSTOMDATA_COUNT > 0 AND LI_MILESTONE_COUNT = 0 THEN
			RETURN  'FALSE' ;
		ELSE
			RETURN 'TRUE';
		END IF;
