DELIMITER $$
CREATE FUNCTION `FN_CHECK_PROFIT_CENTER_LENGTH`(
...
BEGIN
        ...
        AND T1.COLUMN_ID = (SELECT MAX(COLUMN_ID) FROM CUSTOM_DATA_ELEMENTS WHERE CUSTOM_ELEMENT_NAME = 'PROFIT CENTER')
        ...
        IF LI_COUNT > 0 THEN
                        ...
                        AND T1.COLUMN_VERSION_NUMBER = (SELECT MAX(T2.COLUMN_VERSION_NUMBER) FROM CUSTOM_DATA T2
                                                                                        ...
                                                                                        AND T2.MODULE_ITEM_KEY = AV_AWARD_ID
                                                                                        AND T2.MODULE_SUB_ITEM_KEY =0);
                        IF (IFNULL(TRIM(ls_value),'') = '' OR LENGTH(ls_value) <> 10) THEN
                                RETURN 'FALSE';
                         ELSE
                                    RETURN 'TRUE';
                        END IF;
