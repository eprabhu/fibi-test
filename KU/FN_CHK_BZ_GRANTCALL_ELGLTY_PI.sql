DELIMITER $$
CREATE DEFINER=`ku_rms_user`@`%` FUNCTION `FN_CHK_BZ_GRANTCALL_ELGLTY_PI`(
	 ...
	 BEGIN
        ...
        BEGIN
			...
			DECLARE CUR_GRANT_ELIGIBILITY CURSOR FOR
			    SELECT t2.GRANT_ELGBLTY_TYPE_CODE, T4.ELIGIBILITY_TARGET_TYPE_CODE, T4.TARGET_VALUE, t3.PERSON_ID,t5.DESCRIPTION FROM eps_proposal t1
				inner join GRANT_CALL_ELIGIBILITY t2 on t1.GRANT_HEADER_ID = t2.GRANT_HEADER_ID
				INNER JOIN GRANT_ELIGIBILITY_TARGET T4 ON T2.GRANT_ELIGIBILITY_ID = T4.GRANT_ELIGIBILITY_ID
			...
			END IF;
                ...
                INTO LS_IS_EXTERNAL_USER, LS_IS_RESEARCH_STAFF, LS_IS_SUPPORT_STAFF, LS_IS_FACULTY, LS_HOME_UNIT,LS_PRIMARY_TITLE FROM PERSON WHERE PERSON_ID = LS_PERSON_ID;
