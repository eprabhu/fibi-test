...
SELECT
            ...
            AND ((AWARD_DOCUMENT_TYPE_CODE =1 AND AWARD_SEQUENCE_STATUS = 'PENDING')
            OR AWARD_SEQUENCE_STATUS = 'ACTIVE');
    ELSEIF AV_MODULE_CODE = 2 THEN
            ...
            LEFT OUTER JOIN SPONSOR T6 ON T1.SPONSOR_CODE = T6.SPONSOR_CODE
            ...
            WHERE T1.PROPOSAL_ID IN (
										...
										and s1.SEQUENCE_NUMBER in (select max(s2.SEQUENCE_NUMBER) from PROPOSAL s2 where s1.PROPOSAL_ID = s2.PROPOSAL_ID)
									);
		ELSEIF AV_MODULE_CODE = 3 THEN
           SELECT
               ...
               T1.END_DATE AS END_DATE,
				...
				LEFT JOIN SPONSOR T7 ON T1.SPONSOR_CODE = T7.SPONSOR_CODE
