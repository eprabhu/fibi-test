...
IF LS_MODE = 0 THEN
	 SET LS_DYN_SQL = CONCAT('SELECT DISTINCT(T4.FULL_NAME), T1.BUDGET_REFERENCE_NUMBER AS WBS_NUMBER, T6.QUANTITY AS APPROVED_HEADCOUNT, T2.PERSON_ID,
		T3.GL_ACCOUNT_CODE, T3.PAYROLL_ID, T3.PAY_ELEMENT, T2.CHARGE_START_DATE, T2.CHARGE_END_DATE, T7.DEFAULT_JOB_TITLE, T2.POSITION_STATUS_CODE FROM AWARD_MANPOWER T1
		INNER JOIN (
...
ELSE
     SET LS_DYN_SQL = CONCAT('SELECT DISTINCT(T2.FULL_NAME), T1.WBS_NUMBER, T1.APPROVED_HEADCOUNT, T1.PERSON_ID, T3.GL_ACCOUNT_CODE,
		T3.PAYROLL_ID, T3.PAY_ELEMENT, T1.CHARGE_START_DATE, T1.CHARGE_END_DATE, T7.DEFAULT_JOB_TITLE, T1.POSITION_STATUS_CODE FROM CLAIM_MANPOWER T1
