DELIMITER $$
CREATE DEFINER=`ku_rms_user`@`%` PROCEDURE `MERGE_AWD_MASTER_PROJECT_OUTCOME`(
...
DECLARE LI_TOTAL_AWARD_PUBLICATIONS INT(10);
DECLARE LI_TOTAL_AWARD_ACHIEVEMENTS_ATTACH INT(10);
DECLARE LI_AWARD_PUBLICATION_ID	decimal(22,0);
...
DECLARE LI_AWARD_ASSOCIATION_ID		DECIMAL(22,0);
...
DECLARE LI_TOTAL_PROJECT_COST		DECIMAL(19,2);
		BEGIN
				...
				BEGIN
					...
					VALUES(AV_AWARD_ID,AV_AWARD_NUMBER,AV_SEQUENCE_NUMBER,LS_ERROR_MSG,'MERGE_AWD_MASTER_PROJECT_OUTCOME',LS_TABLE_NAME,115,AV_VARIATION_TYPE_CODE,UTC_TIMESTAMP(),AV_UPDATE_USER);
					COMMIT;
				END;
				...
				IF LI_FLAG > 0 THEN
					...
					AND SEQUENCE_NUMBER = 0;
				END IF;
				SELECT COUNT(*) INTO LI_TOTAL_AWARD_PUBLICATIONS
				FROM AWARD_PUBLICATIONS
				WHERE AWARD_ID = AV_AWARD_ID;
				SELECT IFNULL(MAX(NEXT_VAL),0) INTO LI_NEXT_VAL FROM AWARD_PUBLICATION_ID_GENERATOR;
				UPDATE AWARD_PUBLICATION_ID_GENERATOR SET NEXT_VAL = LI_NEXT_VAL + LI_TOTAL_AWARD_PUBLICATIONS + 1;
				COMMIT;
				BEGIN
				...
				IF LI_FLAG > 0 THEN
					...
					AND SEQUENCE_NUMBER = 0;
				END IF;
				SELECT COUNT(*) INTO LI_TOTAL_AWARD_ACHIEVEMENTS_ATTACH
				FROM AWARD_ACHIEVEMENTS_ATTACHMENTS
				WHERE AWARD_ID = AV_AWARD_ID;
				SELECT IFNULL(NEXT_VAL,0) INTO LI_NEXT_VAL FROM AWARD_ACHEIVEMENT_ATTACH_ID_GENERATOR;
				UPDATE AWARD_ACHEIVEMENT_ATTACH_ID_GENERATOR SET NEXT_VAL = LI_NEXT_VAL + LI_TOTAL_AWARD_ACHIEVEMENTS_ATTACH + 1;
				COMMIT;
				BEGIN
				...
				WHERE AWARD_ASSOCIATION_ID IN(SELECT AWARD_ASSOCIATION_ID FROM AWARD_ASSOCIATION
											  WHERE AWARD_ID = AV_AWARD_ID);
				SELECT IFNULL(MAX(NEXT_VAL),0) INTO LI_AWAD_ASSOC_NEXT_VAL
				FROM AWARD_ASSOCIATION_ID_GENERATOR;
				SELECT IFNULL(MAX(NEXT_VAL),0) INTO LI_AWAD_ASSOC_DTL_NEXT_VAL
				FROM AWARD_ASSOC_DETAIL_ID_GENERATOR;
				UPDATE AWARD_ASSOCIATION_ID_GENERATOR SET NEXT_VAL = LI_AWAD_ASSOC_NEXT_VAL + LI_AWAD_ASSOC_CNT + 1;
				UPDATE AWARD_ASSOC_DETAIL_ID_GENERATOR SET NEXT_VAL = LI_AWAD_ASSOC_DTL_NEXT_VAL + LI_AWAD_ASSOC_DTL_CNT + 1;
				COMMIT;
				BEGIN
					...
					END IF;
					SET LI_AWAD_ASSOC_NEXT_VAL = LI_AWAD_ASSOC_NEXT_VAL + 1;
					...
					BEGIN
						...
						END IF;
						SET LI_AWAD_ASSOC_DTL_NEXT_VAL = LI_AWAD_ASSOC_DTL_NEXT_VAL + 1;
						INSERT INTO AWARD_ASSOC_DETAIL
						(
						AWARD_ASSOC_DETAIL_ID, AWARD_ASSOCIATION_ID, title, PI_PERSON_ID, PI_ROLODEX_ID, PI_NAME, LEAD_UNIT, SPONSOR_CODE, PRIME_SPONSOR_CODE, SPONSOR_AWARD_NUMBER, CONTACT_ADMIN_PERSON_ID, SUBAWARD_ORG, UPDATE_TIMESTAMP, UPDATE_USER, FUNDING_SCHEME_CODE, STATUS_DESCRIPTION, TOTAL_PROJECT_COST
						)
						VALUES
						(
						LI_AWAD_ASSOC_DTL_NEXT_VAL,
						LI_AWAD_ASSOC_NEXT_VAL,
						LS_TITLE,
						LS_PI_PERSON_ID,
						LS_PI_ROLODEX_ID,
