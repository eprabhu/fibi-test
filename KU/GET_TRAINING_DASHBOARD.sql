...
END IF;
  SET LS_DYN_SQL =CONCAT('SELECT  *  FROM(SELECT (SELECT IF(T1.IS_NON_EMPLOYEE = ''Y'',
			...
			T2.DESCRIPTION, T1.UPDATE_TIMESTAMP ,T1.PERSON_ID, T1.IS_NON_EMPLOYEE ,T1.PERSON_TRAINING_ID,T1.FOLLOWUP_DATE,T1.TRAINING_CODE, T1.DATE_ACKNOWLEDGED' ,SELECTED_FIELD_LIST,
