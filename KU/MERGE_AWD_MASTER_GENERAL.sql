DELIMITER $$
CREATE PROCEDURE `MERGE_AWD_MASTER_GENERAL`(
...
DECLARE LS_PAYMENT_INVOICE_FREQ_CODE		VARCHAR(3);
...
DECLARE LI_TOTAL_SCIENCE_KEY_WORD			INT(10);
...
DECLARE LI_NEXT_VAL							INT(10);
		BEGIN
				...
				BEGIN
					...
					VALUES(AV_AWARD_ID,AV_AWARD_NUMBER,AV_SEQUENCE_NUMBER,LS_ERROR_MSG,'MERGE_AWD_MASTER_GENERAL',LS_TABLE_NAME,101,AV_VARIATION_TYPE_CODE,UTC_TIMESTAMP(),AV_UPDATE_USER);
					COMMIT;
				END;
				...
				BEGIN
					...
					SELECT IFNULL(MAX(NEXT_VAL),0) INTO LI_NEXT_VAL FROM AWARD_KEYWORD_ID_GENERATOR;
					SELECT COUNT(1) INTO LI_TOTAL_SCIENCE_KEY_WORD FROM AWARD_SCIENCE_KEYWORD
					WHERE AWARD_ID = AV_AWARD_ID;
					UPDATE AWARD_KEYWORD_ID_GENERATOR SET NEXT_VAL = LI_NEXT_VAL + LI_TOTAL_SCIENCE_KEY_WORD + 1;
					COMMIT;
					...
					BEGIN
						...
						(AWARD_SCIENCE_KEYWORD_ID, AWARD_ID, SCIENCE_KEYWORD_CODE, UPDATE_TIMESTAMP, UPDATE_USER, KEYWORD)
						VALUES
						(LI_NEXT_VAL,LI_MASTER_AWARD_ID,LS_SCIENCE_KEYWORD_CODE,LS_UPDATE_TIMESTAMP,LS_UPDATE_USER,LS_KEYWORD);
						END LOOP;
