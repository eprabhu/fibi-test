...
ELSEIF AV_TAB_TYPE = 'ALL_IOI' THEN
          SET TAB_QUERY = CONCAT('( (T1.SUBMITING_UNIT_NUMBER IN (SELECT DISTINCT UNIT_NUMBER FROM ACCESS_TMP)
                                                                OR T15.HOME_UNIT_NUMBER  IN (SELECT DISTINCT UNIT_NUMBER FROM ACCESS_TMP)) AND T1.STATUS_CODE = 2)');
...
END IF;
			SET LS_DYN_SQL = CONCAT('WITH ACCESS_TMP
							...
							');
SET LS_DYN_SQL =CONCAT(LS_DYN_SQL,'SELECT DISTINCT T10.UNIT_NAME AS SUBMITING_UNIT_NAME,T4.FULL_NAME AS PI_FULL_NAME,T1.PROJECT_TITLE,
