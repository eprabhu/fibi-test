CREATE PROCEDURE `GET_PROPOSAL_CERTIFICATION_JSON`(
    IN AV_PROPOSAL_ID INT,
    OUT certification_details JSON
)
BEGIN
    SELECT JSON_OBJECT(
        'certificationPersons', (
            SELECT JSON_ARRAYAGG(
                JSON_OBJECT(
                    'name', pp.full_name,
                    'designation', pp.designation,
                    'homeUnit', CONCAT(u.unit_number, ' - ', u.unit_name),
                    'emailAddress', p.email_address,
                    'certification', pp.is_person_certified,
                    'units', (
                        SELECT JSON_ARRAYAGG(
                            JSON_OBJECT(
                                'unitNumber', ppu.unit_number,
                                'leadUnitFlag', ppu.lead_unit_flag,
                                'unitName', u2.unit_name
                            )
                        )
                        FROM eps_prop_person_units ppu
                            LEFT JOIN unit u2 ON u2.unit_number = ppu.unit_number
                    ...
                    'questionnaires', (
                        ...
                        LEFT JOIN QUEST_ANSWER_HEADER T3 ON T1.QUESTIONNAIRE_ID = T3.QUESTIONNAIRE_ID
                            ...
                            AND T3.MODULE_SUB_ITEM_KEY = p.person_id
            ...
            FROM eps_proposal_persons pp
                LEFT JOIN eps_prop_person_units hup ON hup.proposal_person_id = pp.proposal_person_id AND hup.lead_unit_flag = 'Y'
                LEFT JOIN unit u ON u.unit_number = hup.unit_number
                LEFT JOIN person p ON pp.person_id = p.person_id
            WHERE pp.proposal_id = AV_PROPOSAL_ID
        )
    ) AS result INTO certification_details;
END
